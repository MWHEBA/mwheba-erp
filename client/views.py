...
def customer_list(request):
    ...
    # تعريف أعمدة الجدول
    headers = [
        {'key': 'name', 'label': 'اسم العميل', 'sortable': True, 'class': 'text-center', 'format': 'link', 'url': 'client:customer_detail'},
        {'key': 'code', 'label': 'الكود', 'sortable': True},
        {'key': 'phone', 'label': 'رقم الهاتف', 'sortable': False},
        {'key': 'email', 'label': 'البريد الإلكتروني', 'sortable': False},
        {'key': 'actual_balance', 'label': 'المديونية', 'sortable': True, 'format': 'currency', 'decimals': 2},
...
def customer_detail(request, pk):
    ...
    payments = SalePayment.objects.filter(sale__customer=customer).order_by('-payment_date')
    ...
    pricing_orders = PricingOrder.objects.filter(client=customer).order_by('-created_at')
    ...
    try:
        ...
        print(f"حساب العميل المالي: {financial_account.name if financial_account else 'لا يوجد'}")
    except Exception as e:
    ...
    invoices_headers = [
        ...
        {'key': 'number', 'label': 'رقم الفاتورة', 'sortable': True, 'class': 'text-center', 'format': 'reference', 'variant': 'highlight-code', 'app': 'sale'},
    ...
    invoices_action_buttons = [
        {'url': 'sale:sale_detail', 'icon': 'fa-eye', 'class': 'action-view', 'label': 'عرض الفاتورة'},
        {'url': 'sale:sale_add_payment', 'icon': 'fa-money-bill', 'class': 'action-paid', 'label': 'إضافة دفعة', 'condition': 'not_fully_paid'},
    ]
    ...
    payments_headers = [
        ...
        {'key': 'sale__number', 'label': 'رقم الفاتورة', 'sortable': True, 'class': 'text-center', 'template': 'components/cells/invoice_reference.html', 'width': '130px'},
    ...
    pricing_orders_headers = [
        {'key': 'id', 'label': '#', 'sortable': True, 'class': 'text-center', 'width': '60px'},
        {'key': 'order_number', 'label': 'رقم الطلب', 'sortable': True, 'class': 'text-center', 'width': '120px'},
        {'key': 'title', 'label': 'عنوان الطلب', 'sortable': True, 'class': 'text-start'},
        {'key': 'order_type', 'label': 'نوع الطلب', 'sortable': True, 'class': 'text-center', 'width': '100px'},
        {'key': 'status', 'label': 'الحالة', 'sortable': True, 'class': 'text-center', 'width': '100px'},
        {'key': 'quantity', 'label': 'الكمية', 'sortable': True, 'class': 'text-center', 'width': '80px'},
        {'key': 'created_at', 'label': 'تاريخ الطلب', 'sortable': True, 'class': 'text-center', 'format': 'date', 'width': '120px'},
        {'key': 'required_delivery_date', 'label': 'تاريخ التسليم', 'sortable': True, 'class': 'text-center', 'format': 'date', 'width': '120px'},
    ]
    
    # تعريف أزرار إجراءات جدول طلبات التسعير
    pricing_orders_action_buttons = [
        {'url': 'pricing:pricing_detail', 'icon': 'fa-eye', 'class': 'action-view', 'label': 'عرض'},
        {'url': 'pricing:pricing_edit', 'icon': 'fa-edit', 'class': 'action-edit', 'label': 'تعديل'},
        {'url': 'pricing:pricing_delete', 'icon': 'fa-trash', 'class': 'action-delete', 'label': 'حذف'}
    ]
...
def customer_change_account(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # رسالة تأكيد
            if old_account:
                messages.success(request, f'تم تغيير الحساب المحاسبي من "{old_account.name}" إلى "{customer.financial_account.name}" بنجاح')
            else:
                messages.success(request, f'تم ربط العميل بالحساب المحاسبي "{customer.financial_account.name}" بنجاح')
