...
class Customer(models.Model):
    ...
    company_name = models.CharField(_('اسم الشركة'), max_length=255, blank=True, 
    ...
    phone_primary = models.CharField(_('رقم الهاتف الأساسي'), max_length=20, blank=True,
                                   help_text=_('رقم الهاتف الأساسي للتواصل'))
    phone_secondary = models.CharField(_('رقم الهاتف الثانوي'), max_length=20, blank=True,
    ...
    client_type = models.CharField(_('نوع العميل'), max_length=20, choices=CLIENT_TYPES, 
                                 default='individual', help_text=_('تصنيف العميل حسب النوع'))
    
    # معلومات إدارة العلاقات (CRM)
    last_contact_date = models.DateTimeField(_('تاريخ آخر اتصال'), null=True, blank=True,
                                           help_text=_('تاريخ آخر تواصل مع العميل'))
    contact_frequency = models.CharField(_('تكرار الاتصال'), max_length=20, 
                                       choices=CONTACT_FREQUENCY_CHOICES, blank=True,
    ...
    def actual_balance(self):
        ...
        total_sale_payments = SalePayment.objects.filter(
            sale__customer=self
        ).aggregate(
            total=Sum('amount')
        )['total'] or 0
