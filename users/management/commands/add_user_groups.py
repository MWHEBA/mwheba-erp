    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"تم إضافة {admin_user.username} إلى مجموعة {admin_group.name}"
                    )
                )
            ...
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"تم إضافة {accountant_user.username} إلى مجموعة {accountant_group.name}"
                    )
                )
            except User.DoesNotExist:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"تم إضافة {inventory_user.username} إلى مجموعة {inventory_group.name}"
                    )
                )
            except User.DoesNotExist:
            ...
            for sales_username in ["sales1", "sales2"]:
                try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"تم إضافة {sales_user.username} إلى مجموعة {sales_group.name}"
                        )
                    )
                except User.DoesNotExist:
                    self.stdout.write(
                        self.style.WARNING(f"المستخدم {sales_username} غير موجود")
                    )

            self.stdout.write(
                self.style.SUCCESS("تمت إضافة جميع المستخدمين إلى المجموعات بنجاح")
            )
