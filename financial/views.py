...
def cash_and_bank_accounts_list(request):
    ...
    return render(
        request, "financial/advanced/cash_and_bank_accounts_list.html", context
    )
    ...
    if request.method == "POST":
        try:
            ...
            if ChartOfAccounts and ChartOfAccounts.objects.filter(code=code).exists():
                ...
                return render(
                    request, "financial/advanced/chart_of_accounts_form.html", context
                )
...
def journal_entries_create(request):
    ...
    accounting_periods = []
    if AccountingPeriod:
        accounting_periods = AccountingPeriod.objects.filter(is_active=True).order_by(
            "-start_date"
        )
...
def expense_list(request):
    ...
    paid_expenses = (
        expenses.filter(status="paid").aggregate(Sum("amount")).get("amount__sum", 0)
        or 0
    )
    pending_expenses = (
        expenses.filter(status="pending").aggregate(Sum("amount")).get("amount__sum", 0)
        or 0
    )
    ...
    monthly_expenses = (
        expenses.filter(date__gte=six_months_ago)
    ...
    # تعريف رؤوس الأعمدة للجدول الموحد
    headers = [
        {"key": "title", "label": "العنوان", "sortable": True, "width": "15%"},
        {
            "key": "created_at",
            "label": "التاريخ والوقت",
            "sortable": True,
        ...
        {"key": "category", "label": "التصنيف", "sortable": False, "width": "12%"},
        {
            "key": "amount",
            "label": "المبلغ",
            "sortable": False,
        ...
        {"key": "payee", "label": "المستفيد", "sortable": False, "width": "15%"},
        {
            "key": "reference_number",
            "label": "رقم المرجع",
            "sortable": False,
...
def income_list(request):
    ...
    received_incomes = (
        incomes.filter(status="received").aggregate(Sum("amount")).get("amount__sum", 0)
        or 0
    )
    pending_incomes = (
        incomes.filter(status="pending").aggregate(Sum("amount")).get("amount__sum", 0)
        or 0
    )
    ...
    months_passed = (
        (current_date.year - start_of_year.year) * 12
        + current_date.month
        - start_of_year.month
        + 1
    )
    ...
    action_buttons = [
        {
            "url": "financial:income_detail",
            "icon": "fa-eye",
            "label": "عرض",
            "class": "btn-primary action-view",
        },
        {
            "url": "financial:income_edit",
            "icon": "fa-edit",
            "label": "تعديل",
            "class": "btn-secondary action-edit",
        },
        {
            "url": "financial:income_mark_received",
            "icon": "fa-check-circle",
            "label": "استلام",
            "class": "btn-success action-received",
        },
        {
            "url": "financial:income_cancel",
            "icon": "fa-ban",
            "label": "إلغاء",
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if receiving_account:
                ...
                default_account = (
                    get_cash_and_bank_accounts().filter(name__icontains="خزينة").first()
                )
                if not default_account:
                    default_account = (
                        get_cash_and_bank_accounts()
                        .filter(is_cash_account=True)
                        .first()
                    )
                if not default_account:
...
def ledger_report(request):
    ...
    if account_id:
        ...
        transactions = transaction_lines.order_by(
            "transaction__date", "transaction__id"
        )
        ...
        for account in accounts:
            ...
            total_debit = transaction_lines.aggregate(Sum("debit"))["debit__sum"] or 0
            total_credit = (
                transaction_lines.aggregate(Sum("credit"))["credit__sum"] or 0
            )
...
def sales_report(request):
    ...
    """
    messages.info(
        request, "تقرير المبيعات تحت التطوير. سيتم ربطه مع نظام المبيعات قريباً."
    )
...
def purchases_report(request):
    ...
    """
    messages.info(
        request, "تقرير المشتريات تحت التطوير. سيتم ربطه مع نظام المشتريات قريباً."
    )
...
def inventory_report(request):
    ...
    """
    messages.info(
        request, "تقرير المخزون تحت التطوير. سيتم ربطه مع نظام المنتجات قريباً."
    )
...
def payment_dashboard(request):
    ...
    try:
        ...
        if SalePayment:
            ...
            stats["posted_sale_payments"] = SalePayment.objects.filter(
                status="posted"
            ).count()
            stats["pending_payments"] += SalePayment.objects.filter(
            ...
            stats["failed_sale_payments"] = SalePayment.objects.filter(
                financial_status="failed"
            ).count()

        if PurchasePayment:
            stats["total_purchase_payments"] = PurchasePayment.objects.count()
            stats["posted_purchase_payments"] = PurchasePayment.objects.filter(
                status="posted"
            ).count()
            stats["pending_payments"] += PurchasePayment.objects.filter(
                financial_status="pending"
            ).count()
            synced_purchase = PurchasePayment.objects.filter(
                financial_status="synced"
            ).count()
            stats["synced_purchase_payments"] = synced_purchase
            stats["synced_payments"] += synced_purchase
            stats["failed_purchase_payments"] = PurchasePayment.objects.filter(
                financial_status="failed"
            ).count()
        ...
        if SalePayment:
            problematic_payments["failed_sales"] = SalePayment.objects.filter(
                financial_status="failed"
            )[:5]
            problematic_payments["pending_sales"] = SalePayment.objects.filter(
                financial_status="pending"
            )[:5]

        if PurchasePayment:
            problematic_payments["failed_purchases"] = PurchasePayment.objects.filter(
                financial_status="failed"
            )[:5]
            problematic_payments["pending_purchases"] = PurchasePayment.objects.filter(
                financial_status="pending"
            )[:5]
        ...
        if SalePayment:
            recent_sale_payments = SalePayment.objects.select_related(
                "sale", "sale__customer"
            ).order_by("-created_at")[:5]

        if PurchasePayment:
            recent_purchase_payments = PurchasePayment.objects.select_related(
                "purchase", "purchase__supplier"
            ).order_by("-created_at")[:5]
    ...
    try:
                ...
                payments.append(
                    {
                        ...
                        "url": reverse(
                            "financial:payment_detail", args=["sale", payment.id]
                        ),
                    }
                )
                ...
                payments.append(
                    {
                        ...
                        "url": reverse(
                            "financial:payment_detail", args=["purchase", payment.id]
                        ),
                    }
                )
        ...
        payment_headers = [
            ...
            },
            {
                ...
                "sortable": True,
                "class": "text-center",
                "template": "components/cells/payment_type.html",
                "width": "80px",
            },
            {
                "key": "invoice_number",
            ...
            },
            {
                ...
                "sortable": True,
                "class": "text-center",
                "template": "components/cells/payment_status.html",
                "width": "100px",
            },
            {
                "key": "amount",
    ...
    try:
        ...
        if payment_type == "sale" and SalePayment:
            payment = get_object_or_404(
                SalePayment.objects.select_related("sale", "sale__customer"),
                pk=payment_id,
            )
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if PaymentEditService:
                ...
                success, message = service.edit_payment(
                    payment, request.POST, request.user
                )

                if success:
                    messages.success(request, message)
                    return redirect(
                        "financial:payment_detail",
                        payment_type=payment_type,
    ...
    try:
        ...
        if request.method == "POST":
            ...
            try:
                ...
                if (
                    hasattr(payment, "financial_transaction")
                    and payment.financial_transaction
                ):
    ...
    try:
        ...
        if AuditTrail:
            audit_entries = AuditTrail.objects.filter(
                entity_type=payment_type + "_payment", entity_id=payment_id
            ).order_by("-created_at")
...
def audit_trail_list(request):
    ...
    try:
        if not AuditTrail:
            ...
            return render(
                request, "financial/audit_trail_list.html", {"page_obj": None}
            )
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                updated_entry = ExpenseIncomeService.update_expense(
                    journal_entry, form.cleaned_data, request.user
                )
        ...
        context = {
            ...
            "breadcrumb_items": [
                ...
                {"title": "المصروفات", "url": reverse("financial:expense_list")},
                {
                    "title": f"المصروف {journal_entry.reference}",
                    "url": reverse("financial:expense_detail", args=[pk]),
                },
...
def expense_post(request, pk):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            messages.success(
                request, f"تم ترحيل المصروف {journal_entry.reference} بنجاح"
            )
            return redirect("financial:expense_detail", pk=pk)
    ...
    try:
        ...
        context = {
            ...
            "breadcrumb_items": [
                ...
                {"title": "الإيرادات", "url": reverse("financial:income_list")},
                {
                    "title": f"الإيراد {journal_entry.reference}",
                    "url": reverse("financial:income_detail", args=[pk]),
                },
                {"title": "تعديل", "active": True},
            ],
        }
...
def income_post(request, pk):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            messages.success(
                request, f"تم ترحيل الإيراد {journal_entry.reference} بنجاح"
            )
            return redirect("financial:income_detail", pk=pk)
...
def payment_sync_process_all_api(request):
    ...
    try:
        ...
        failed_ops = PaymentSyncOperation.objects.filter(status="failed").order_by(
            "-created_at"
        )
