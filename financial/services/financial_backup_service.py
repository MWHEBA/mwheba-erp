...
class FinancialBackupService:
    ...
    def __init__(self):
        self.backup_dir = getattr(
            settings,
            "FINANCIAL_BACKUP_DIR",
    ...
    def create_full_backup(self, include_logs: bool = False) -> Dict:
        ...
        try:
            ...
            for model_class, filename in models_to_backup:
                records_count = self._backup_model(model_class, backup_path, filename)
    ...
    def restore_backup(
        self, backup_path: str, selective_restore: List[str] = None
    ) -> Dict:
        """
            ...
            with transaction.atomic():
                for model_info in backup_info["models_backed_up"]:
                    ...
                    restore_info["models_restored"].append(
                        {"model": model_name, "records_restored": records_restored}
                    )
                                ...
                                backup_info.update(
                                    {
                                        ...
                                        "total_records": detailed_info.get(
                                            "total_records", 0
                                        ),
                                        "models_count": len(
                                            detailed_info.get("models_backed_up", [])
                                        ),
                                    }
                                )
            ...
            try:
                ...
                if not os.path.exists(info_file):
                    verification_result["errors"].append(
                        "ملف معلومات النسخة الاحتياطية مفقود"
                    )
                                ...
                                verification_result["files_checked"].append(
                                    {
                                        "filename": filename,
                                        "status": "valid",
                                        "expected_records": model_info.get(
                                            "records_count", 0
                                        ),
                                    }
                                )

                                verification_result["total_records"] += model_info.get(
                                    "records_count", 0
                                )

                            except Exception as e:
                                verification_result["errors"].append(
                                    f"خطأ في قراءة {filename}: {str(e)}"
                                )
                                verification_result["is_valid"] = False
