...
class RedisFinancialCache:
    ...
    def _serialize_data(self, data: Any) -> bytes:
        ...
        if isinstance(data, dict):
            ...
            data = [
                self._convert_decimals_to_string(item)
    ...
    def _deserialize_data(self, data: bytes) -> Any:
        ...
        if isinstance(result, dict):
            ...
            result = [
                self._convert_strings_to_decimal(item)
                if isinstance(item, dict)
                else item
                for item in result
            ]
        ...
        for key, value in data.items():
            if key in decimal_fields and isinstance(value, str):
                ...
                result[key] = [
                    ...
                    if isinstance(item, str) and key in decimal_fields
                    else item
                    for item in value
                ]
            else:
    ...
    def set(
        self, key_type: str, value: Any, timeout: Optional[int] = None, **kwargs
    ) -> bool:
        """
        حفظ قيمة في الكاش
        """
...
class CachedBalanceService:
    ...
    def get_account_balance(
        ...
        cached_balance = financial_cache.get(
            "balance", account_id=account_id, date_from=date_from, date_to=date_to
        )
    ...
    def get_trial_balance_cached(
        ...
        if not use_cache:
            ...
            return EnhancedBalanceService.get_trial_balance_optimized(
                date_from, date_to
            )

        # محاولة الحصول من الكاش
        cached_trial_balance = financial_cache.get(
            "trial_balance", date_from=date_from, date_to=date_to
        )
