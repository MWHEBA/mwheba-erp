...
class AdvancedBalanceValidationService:
    ...
    def validate_all_balances(
        cls, date_to: Optional[date] = None, fix_discrepancies: bool = False
    ) -> Dict:
        """
        ...
        for account in accounts:
            try:
                ...
                if validation_result["is_valid"]:
                    ...
                    results["total_discrepancy_amount"] += abs(
                        validation_result["difference"]
                    )

                    # محاولة إصلاح الفرق إذا طُلب
                    if fix_discrepancies:
                        fixed = cls._fix_balance_discrepancy(
                            account, validation_result, date_to
                        )
                        if fixed:
    ...
    def _fix_balance_discrepancy(
        cls, account: ChartOfAccounts, validation_result: Dict, date_to: date
    ) -> bool:
        """
        إصلاح فرق الرصيد
        """
    ...
    def validate_trial_balance_integrity(cls, date_to: Optional[date] = None) -> Dict:
        ...
        result = {
            ...
            "difference": difference,
            "accounts_count": len(
                [item for item in trial_balance if item.get("account_code")]
            ),
    ...
    def _find_unbalanced_entries(cls, date_to: date) -> List[Dict]:
        ...
        entries = JournalEntry.objects.filter(
            status="posted", date__lte=date_to
    ...
    def _get_last_activity_date(
        cls, account: ChartOfAccounts, date_to: date
    ) -> Optional[date]:
        """
    ...
    def reconcile_account_balance(
        ...
        calculated_balance = cls._recalculate_account_balance(
    ...
    @classmethod
    def _recalculate_account_balance(
        cls, account: ChartOfAccounts, date_to: date
    ) -> Decimal:
        """
    ...
    def _calculate_total_by_type(cls, account_type: str, date_to: date) -> Decimal:
        ...
        """
        accounts = ChartOfAccounts.objects.filter(
            account_type__category=account_type, is_leaf=True, is_active=True
        )
    ...
    def _calculate_health_score(
        ...
        if balance_validation["total_accounts"] > 0:
            error_rate = (
                balance_validation["invalid_accounts"]
                / balance_validation["total_accounts"]
            )
    ...
    def _generate_recommendations(
        cls, balance_validation: Dict, trial_balance_validation: Dict
    ) -> List[str]:
        """
