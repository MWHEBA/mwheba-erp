...
class CategoryService:
                ...
                if parent_id:
                    ...
                    if parent.type not in [category_type, "both"]:
                        raise ValidationError(
                            "نوع التصنيف الفرعية يجب أن يتوافق مع التصنيف الأب"
                        )
    ...
    def create_budget(
        ...
        try:
            with transaction.atomic():
                ...
                if existing_budget:
                    raise ValidationError(
                        "يوجد ميزانية أخرى لهذا التصنيف في نفس الفترة"
                    )
    ...
    def get_category_usage(
        ...
        try:
            ...
            total_spent = transactions.aggregate(total=Sum("amount"))[
                "total"
            ] or Decimal("0")
            ...
            budget_limit = (
                active_budget.budget_amount if active_budget else category.budget_limit
            )
            ...
            if budget_limit:
                ...
                usage_data["usage_percentage"] = (
                    (total_spent / budget_limit) * 100 if budget_limit > 0 else 0
                )
                usage_data["is_over_budget"] = total_spent > budget_limit
                usage_data["is_warning"] = (
                    usage_data["usage_percentage"] >= category.warning_threshold
                )
    ...
    def get_budget_alerts() -> List[Dict]:
        ...
        try:
            ...
            categories_with_budget = FinancialCategory.objects.filter(
                is_active=True, budget_limit__isnull=False
            )
            ...
            for category in categories_with_budget:
                usage_data = CategoryService.get_category_usage(
                    category.id, start_of_month, current_date
                )
                    ...
                    alerts.append(
                        {
                            ...
                            "excess": usage_data["total_spent"]
                            - usage_data["budget_limit"],
                            "priority": "high",
                        }
                    )
    ...
    def get_category_hierarchy(category_type: Optional[str] = None) -> List[Dict]:
        ...
        try:
            ...
            if category_type:
                root_categories = root_categories.filter(
    ...
    def get_category_report(
        start_date: datetime, end_date: datetime, category_type: Optional[str] = None
    ) -> Dict:
        """
