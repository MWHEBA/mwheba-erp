...
class ReportingService:
    ...
    def generate_trial_balance(
        date_to: Optional[datetime] = None, include_zero_balances: bool = False
    ) -> Dict:
        """
    ...
    def generate_balance_sheet(date_to: Optional[datetime] = None) -> Dict:
        ...
        try:
            ...
            balance_sheet = {
                ...
                "equity": {"items": [], "total_equity": Decimal("0")},
            }

            # الأصول
            asset_accounts = ChartOfAccounts.objects.filter(
                account_type__category="asset", is_active=True, is_leaf=True
            ).order_by("code")
            ...
            for account in asset_accounts:
                ...
                if balance != 0:
                    ...
                    if account.is_cash_account or account.is_bank_account:
                        ...
                        balance_sheet["assets"]["non_current_assets"].append(
                            account_data
                        )
            ...
            liability_accounts = ChartOfAccounts.objects.filter(
                account_type__category="liability", is_active=True, is_leaf=True
            ).order_by("code")
            ...
            if net_income != 0:
                balance_sheet["equity"]["items"].append(
                    {"code": "NET_INCOME", "name": "صافي الدخل", "balance": net_income}
                )
    ...
    def generate_income_statement(
        ...
        try:
            ...
            income_statement = {
                ...
                "period_to": date_to,
                "revenues": {"items": [], "total_revenue": Decimal("0")},
                "expenses": {"items": [], "total_expenses": Decimal("0")},
                "net_income": Decimal("0"),
            }

            # الإيرادات
            revenue_accounts = ChartOfAccounts.objects.filter(
                account_type__category="revenue", is_active=True, is_leaf=True
            ).order_by("code")
            ...
            for account in revenue_accounts:
                ...
                total_credit = lines.aggregate(total=models.Sum("credit"))[
                    "total"
                ] or Decimal("0")

                total_debit = lines.aggregate(total=models.Sum("debit"))[
                    "total"
                ] or Decimal("0")
            ...
            expense_accounts = ChartOfAccounts.objects.filter(
                account_type__category="expense", is_active=True, is_leaf=True
            ).order_by("code")
    ...
    def generate_cash_flow_statement(
        ...
        try:
            ...
            cash_flow = {
                "period_from": date_from,
                "period_to": date_to,
                "operating_activities": {"items": [], "total": Decimal("0")},
                "investing_activities": {"items": [], "total": Decimal("0")},
                "financing_activities": {"items": [], "total": Decimal("0")},
            ...
            for account in cash_accounts:
                beginning_balance = account.get_balance(
                    date_to=date_from - timedelta(days=1), include_opening=True
                )
                cash_flow["beginning_cash"] += beginning_balance

            # حساب الرصيد النقدي في نهاية الفترة
            for account in cash_accounts:
                ending_balance = account.get_balance(
                    date_to=date_to, include_opening=True
                )
                cash_flow["ending_cash"] += ending_balance

            # صافي التدفق النقدي
            cash_flow["net_cash_flow"] = (
                cash_flow["ending_cash"] - cash_flow["beginning_cash"]
            )
            ...
            cash_flow["operating_activities"]["items"].append(
                {"description": "صافي الدخل", "amount": net_income}
            )
    ...
    def generate_account_ledger(
        ...
        try:
            ...
            ledger_data = {
                ...
                "totals": {"total_debit": Decimal("0"), "total_credit": Decimal("0")},
            }
    ...
    def _calculate_net_income(
        date_to: datetime, date_from: Optional[datetime] = None
    ) -> Decimal:
        """
        ...
        try:
            ...
            total_revenue = (
                revenue_lines.aggregate(total=models.Sum("credit"))["total"]
                or Decimal("0")
            ) - (
                revenue_lines.aggregate(total=models.Sum("debit"))["total"]
                or Decimal("0")
            )
            ...
            total_expenses = (
                expense_lines.aggregate(total=models.Sum("debit"))["total"]
                or Decimal("0")
            ) - (
                expense_lines.aggregate(total=models.Sum("credit"))["total"]
                or Decimal("0")
            )
    ...
    def generate_financial_summary(
        ...
        try:
            ...
            total_assets = (
                ...
                .get("assets", {})
                .get("total_assets", Decimal("0"))
            )
            total_liabilities = (
                summary["balance_sheet"]
                .get("liabilities", {})
                .get("total_liabilities", Decimal("0"))
            )
            total_equity = (
                summary["balance_sheet"]
                .get("equity", {})
                .get("total_equity", Decimal("0"))
            )
            ...
            summary["key_metrics"] = {
                ...
                "profit_margin": (
                    ...
                    .get("revenues", {})
                    .get("total_revenue", Decimal("1"))
                )
                if summary["income_statement"]
                .get("revenues", {})
                .get("total_revenue", Decimal("0"))
                != 0
                else None,
            }
            ...
            if not summary["balance_sheet"].get("is_balanced", True):
                summary["alerts"].append(
                    {"type": "error", "message": "الميزانية العمومية غير متوازنة"}
                )

            if net_income < 0:
                summary["alerts"].append(
                    {"type": "warning", "message": "صافي الدخل سالب للفترة"}
                )
