...
class ExpenseIncomeService:
    ...
    def get_or_create_accounting_period(date):
        ...
        try:
            ...
            if not period:
                ...
                if date.month == 12:
                    end_date = date.replace(year=date.year + 1, month=1, day=1) - timezone.timedelta(days=1)
                else:
                    end_date = date.replace(month=date.month + 1, day=1) - timezone.timedelta(days=1)
    ...
    def update_account_balances(journal_entry):
        ...
        try:
            for line in journal_entry.lines.all():
                ...
                logger.debug(f"تم تحديث رصيد الحساب {account.code}: {account.current_balance}")
    ...
    def reverse_account_balances(journal_entry):
        ...
        try:
            for line in journal_entry.lines.all():
                ...
                logger.debug(f"تم عكس تحديث رصيد الحساب {account.code}: {account.current_balance}")
    ...
    def get_expense_statistics(date_from=None, date_to=None):
        ...
        try:
            ...
            for entry in queryset:
                amount = sum(line.debit_amount for line in entry.lines.all() if line.debit_amount > 0)
    ...
    def get_income_statistics(date_from=None, date_to=None):
        ...
        try:
            ...
            for entry in queryset:
                amount = sum(line.credit_amount for line in entry.lines.all() if line.credit_amount > 0)
