    ...
    def create_expense(data, user):
        ...
        try:
            with transaction.atomic():
                ...
                # الحصول على التصنيف المالي (إذا كان متوفر في البيانات)
                financial_category = data.get('financial_category')
                ...
                lines = [
                    ...
                    JournalEntryLineData(
                        account_code=data["payment_account"].code,
                        debit=Decimal("0.00"),
                        credit=data["amount"],
                ...
                journal_entry = gateway.create_journal_entry(
                    ...
                    source_id=0,  # Will be updated if expense model exists
                    lines=lines,
                    idempotency_key=f"JE:financial:Expense:{reference}:create",
                    user=user,
                ...
                if notes and notes != data.get("notes", ""):
    ...
    def create_income(data, user):
        ...
        try:
            with transaction.atomic():
                ...
                # Prepare notes
                notes = data.get("notes", "")
                if data.get("parent") and Parent:
                    notes += f"\nالعميل: {data.get('customer', data.get('parent')).name}"
                
                # Prepare journal entry lines
                lines = [
                    JournalEntryLineData(
                        account_code=data["receipt_account"].code,
                        debit=data["amount"],
                        credit=Decimal("0.00"),
                        description=f"استلام إيراد: {data['description']}"
                    ),
                    JournalEntryLineData(
                        account_code=data["income_account"].code,
                        debit=Decimal("0.00"),
                        credit=data["amount"],
                ...
                journal_entry = gateway.create_journal_entry(
                    source_module='financial',
                    source_model='Income',
                    source_id=0,  # Will be updated if income model exists
                    lines=lines,
                    idempotency_key=f"JE:financial:Income:{reference}:create",
                    user=user,
                    entry_type='automatic',
                    description=data["description"],
                    reference=reference,
