...
class AdvancedReportsService:
    ...
    def generate_comprehensive_trial_balance(
        ...
        cached_result = financial_cache.get('trial_balance_comprehensive', 
        ...
        with connection.cursor() as cursor:
            ...
            for row in cursor.fetchall():
                ...
                for key in ['total_debit', 'total_credit', 'balance', 'debit_balance', 'credit_balance']:
                ...
                category_totals[category]['total_credit'] += account_data['total_credit']
                category_totals[category]['debit_balance'] += account_data['debit_balance']
                category_totals[category]['credit_balance'] += account_data['credit_balance']
        ...
        result = {
            ...
            'is_balanced': abs(grand_totals['debit_balance'] - grand_totals['credit_balance']) < Decimal('0.01'),
            'balance_difference': grand_totals['debit_balance'] - grand_totals['credit_balance']
        }
        
        # حفظ في الكاش
        financial_cache.set('trial_balance_comprehensive', result, timeout=cls.CACHE_TIMEOUT,
                           date_from=date_from, date_to=date_to, include_zero=include_zero_balances)
    ...
    def generate_income_statement(
        ...
        if comparative_period:
            ...
            comparative_data = cls._calculate_income_statement_data(comparative_from, comparative_to)
            ...
            # حساب التغييرات
            result['changes'] = cls._calculate_income_changes(current_period, comparative_data)
    ...
    def _calculate_income_changes(cls, current: Dict, comparative: Dict) -> Dict:
        ...
        return {
            ...
            'revenue_change_percent': (
                (current['total_revenue'] - comparative['total_revenue']) / comparative['total_revenue'] * 100
                if comparative['total_revenue'] > 0 else Decimal('0')
            ),
            'expense_change': current['total_expenses'] - comparative['total_expenses'],
            'expense_change_percent': (
                (current['total_expenses'] - comparative['total_expenses']) / comparative['total_expenses'] * 100
                if comparative['total_expenses'] > 0 else Decimal('0')
            ),
            'net_income_change': current['net_income'] - comparative['net_income'],
            'net_income_change_percent': (
                (current['net_income'] - comparative['net_income']) / comparative['net_income'] * 100
                if comparative['net_income'] != 0 else Decimal('0')
            )
        }
    ...
    def _classify_cash_flow(cls, description: str, reference: str) -> str:
        ...
        if any(keyword in description_lower for keyword in ['مبيعات', 'عميل', 'إيراد']):
            return 'operating'
        elif any(keyword in description_lower for keyword in ['مشتريات', 'مورد', 'مصروف']):
            return 'operating'
        elif any(keyword in description_lower for keyword in ['استثمار', 'أصل', 'معدات']):
            return 'investing'
    ...
    def generate_accounts_aging_report(
        cls,
        account_type: str,  # 'receivables' or 'payables'
        as_of_date: date
    ) -> Dict:
        """
