...
class PurchasesReportService:
    ...
    def get_purchases_by_account(self):
        ...
        if total_purchases > 0:
            for item in purchases_data:
                try:
                    item["percentage"] = (item["amount"] / total_purchases * 100).quantize(Decimal("0.1"))
                except:
    ...
    def get_purchases_statistics(self):
        ...
        try:
            avg_daily_purchases = (total_purchases / Decimal(str(days_count))).quantize(Decimal("0.1")) if days_count > 0 else Decimal("0")
        except:
        ...
        # متوسط قيمة المعاملة
        try:
            avg_transaction_value = (total_purchases / Decimal(str(total_transactions))).quantize(Decimal("0.1")) if total_transactions > 0 else Decimal("0")
        except:
