...
class BalanceService:
    ...
    def _calculate_balance(
        ...
        aggregates = JournalEntryLine.objects.filter(query).aggregate(
            total_debit=models.Sum("debit"), total_credit=models.Sum("credit")
        )
    ...
    def get_trial_balance(
        ...
        accounts_query = ChartOfAccounts.objects.filter(is_leaf=True, is_active=True)
        ...
        for account in accounts_query:
            ...
            if balance != 0:  # عرض الحسابات ذات الرصيد فقط
                ...
                credit_balance = (
                    abs(balance)
                    if account.nature == "credit" and balance > 0
                    else Decimal("0")
                )
    ...
    def get_accounts_summary(
        account_type: Optional[str] = None, date_to: Optional[date] = None
    ) -> List[Dict]:
        """
