    ...
    def classify_inventory(self):
        ...
        for item in inventory_data:
            if total_value > 0:
                ...
                item["cumulative_percentage"] = (cumulative_value / total_value * 100).quantize(Decimal("0.1"))
    ...
    def get_category_statistics(self):
        ...
        def calculate_stats(category_items):
            ...
            return {
                "count": count,
                "value": value,
                "percentage_count": (Decimal(count) / len(inventory_data) * 100).quantize(Decimal("0.1")) if inventory_data else Decimal("0"),
                "percentage_value": (value / total_value * 100).quantize(Decimal("0.1")) if total_value > 0 else Decimal("0"),
                "avg_value": (value / count).quantize(Decimal("0.1")) if count > 0 else Decimal("0"),
            }
