...
class EnhancedBalanceService:
    ...
    def get_account_balance_optimized(
        ...
        if date_to is None:
            ...
            balance = cls._get_historical_balance_optimized(
                account_obj, date_from, date_to
            )
    ...
    def _get_current_balance_with_cache(
        cls, account: ChartOfAccounts, force_refresh: bool = False
    ) -> Decimal:
        """
        ...
        try:
            ...
            if (
                force_refresh
                or not balance_cache.is_valid
                or balance_cache.needs_refresh
            ):
            ...
            logger.error(
                f"خطأ في الحصول على الرصيد من الكاش للحساب {account.code}: {str(e)}"
            )
    ...
    def _get_historical_balance_optimized(
        ...
        snapshot = (
            BalanceSnapshot.objects.filter(account=account, snapshot_date__lte=date_to)
        ...
        try:
            ...
            if account_types:
                accounts_query = accounts_query.filter(
                    account_type__category__in=account_types
                )
            ...
            for account in accounts:
                ...
                if abs(balance_details["balance"]) > Decimal(
                    "0.01"
                ):  # تجاهل الأرصدة الصفرية
    ...
    def create_balance_snapshot(
        cls, account: ChartOfAccounts, snapshot_date: date = None
    ) -> bool:
        """
        ...
        try:
            ...
            snapshot, created = BalanceSnapshot.objects.update_or_create(
                ...
                defaults={
                    ...
                    "last_transaction_id": last_transaction.journal_entry.id
                    if last_transaction
                    else None,
                },
            )
    ...
    def _get_account_object(
        cls, account: Union[str, int, ChartOfAccounts]
    ) -> ChartOfAccounts:
        """
        الحصول على كائن الحساب
        """
    ...
    def _generate_cache_key(
        cls, account_id: int, date_from: Optional[date], date_to: Optional[date]
    ) -> str:
        """
        توليد مفتاح الكاش
        """
    ...
    def _calculate_balance_traditional(cls, account: ChartOfAccounts) -> Decimal:
        ...
        ).aggregate(total_debit=models.Sum("debit"), total_credit=models.Sum("credit"))
            ...
            for account in accounts:
                try:
                    ...
                    try:
                        ...
                        is_cash_account = (
                            "نقدي" in account.account_type.name.lower()
                            or "صندوق" in account.account_type.name.lower()
                        )
