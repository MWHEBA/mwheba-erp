...
class AccountingIntegrationService:
    ...
    def create_sale_journal_entry(
        ...
        try:
            with transaction.atomic():
                ...
                if not accounts:
                    logger.error("لا يمكن العثور على الحسابات المحاسبية المطلوبة للمبيعات")
                    return None
    ...
    def create_purchase_journal_entry(
        ...
        try:
            with transaction.atomic():
                ...
                if not accounts:
                    logger.error("لا يمكن العثور على الحسابات المحاسبية المطلوبة للمشتريات")
                    return None
        ...
        try:
            with transaction.atomic():
                ...
                if payment_type == 'sale_payment':
                    ...
                    # مدين الصندوق/البنك
                    account_debit = accounts['cash'] if payment.payment_method == 'cash' else accounts['bank']
                    # دائن العملاء
                    ...
                    # دائن الصندوق/البنك
                    account_credit = accounts['cash'] if payment.payment_method == 'cash' else accounts['bank']
    ...
    def _get_required_accounts_for_sale(cls) -> Dict[str, ChartOfAccounts]:
        ...
        try:
            accounts = {}
            required_codes = ['sales_revenue', 'cost_of_goods_sold', 'inventory', 
                            'accounts_receivable', 'cash', 'bank']
    ...
    def _get_required_accounts_for_purchase(cls) -> Dict[str, ChartOfAccounts]:
        ...
        try:
            accounts = {}
            required_codes = ['inventory', 'accounts_payable', 'cash', 'bank', 'purchase_expense']
    ...
    def _calculate_sale_cost(cls, sale) -> Decimal:
        ...
        try:
            ...
            if total_cost == 0:
                ...
                for item in sale.items.all():
                    logger.warning(f"  - المنتج: {item.product.name}, التكلفة: {item.product.cost_price}, الكمية: {item.quantity}")
    ...
    def _create_default_account_types(cls) -> Dict[str, AccountType]:
        ...
        types_data = [
            ...
            {'code': 'LIABILITY', 'name': 'خصوم', 'category': 'liability', 'nature': 'credit'},
            {'code': 'REVENUE', 'name': 'إيرادات', 'category': 'revenue', 'nature': 'credit'},
            {'code': 'EXPENSE', 'name': 'مصروفات', 'category': 'expense', 'nature': 'debit'},
        ]
