# financial/services/parent_supplier_balances_service.py
"""
خدمة تقارير أرصدة أولياء الأمور والموردين
...
class ParentSupplierBalancesService:
    """خدمة تقارير أرصدة أولياء الأمور والموردين"""
    ...
    def generate_parent_balances_report(self) -> Dict[str, Any]:
        """
        إنشاء تقرير أرصدة العملاء (بديل عن أولياء الأمور)
    ...
    def _calculate_parent_balance(self, parent) -> Dict[str, Any]:
        """حساب رصيد عميل معين حسب فترات الاستحقاق (للتوافق مع الكود القديم)"""
        return self._calculate_client_balance(parent)
    ...
    def _calculate_client_balance(self, client) -> Dict[str, Any]:
        ...
        try:
            ...
            return self._empty_balance_data(str(client), client.id)
        
        # جلب المبيعات المؤكدة للعميل حتى تاريخ التقرير
        ...
        for sale in sales:
            remaining_balance = getattr(sale, 'amount_due', Decimal('0'))
        ...
        return {
            ...
            'account_code': getattr(client, 'code', '') or str(client.id),
            'current': current,
            'days_1_30': days_1_30,
    ...
    def _generate_parent_from_journal_entries(self) -> Dict[str, Any]:
        """
        إنشاء تقرير أرصدة أولياء الأمور من القيود المحاسبية مباشرة
        """
        from financial.models.chart_of_accounts import ChartOfAccounts
        from financial.models.journal_entry import JournalEntryLine
        
        # جلب حسابات العملاء فقط (كود 10300)
        try:
            base_account = ChartOfAccounts.objects.get(code='10300', is_active=True)
    ...
    def _generate_supplier_from_journal_entries(self) -> Dict[str, Any]:
        ...
        # جلب حسابات الموردين فقط (كود 20100)
        try:
            base_account = ChartOfAccounts.objects.get(code='20100', is_active=True)
            # جلب الحساب الرئيسي وجميع الحسابات الفرعية
            liability_accounts = ChartOfAccounts.objects.filter(
                code__startswith='20100',
                is_active=True
            )
        except ChartOfAccounts.DoesNotExist:
