...
class PaymentIntegrationService:
    ...
    def process_payment(
        ...
        """
        operation_id = (
            f"{payment_type}_{payment.id}_{timezone.now().strftime('%Y%m%d_%H%M%S')}"
        )
        ...
        try:
            ...
            error_message = str(e)
            logger.error(
                f"فشل في معالجة الدفعة - العملية: {operation_id} - الخطأ: {error_message}"
            )
    ...
    def _validate_payment_data(cls, payment, payment_type: str):
        ...
        if not (
            payment.financial_account.is_cash_account
            or payment.financial_account.is_bank_account
        ):
    ...
    def _execute_payment_integration(
        cls, payment, payment_type: str, user: Optional[User], operation_id: str
    ) -> Dict[str, Any]:
    ...
    def _create_journal_entry(
        cls, payment, payment_type: str, user: Optional[User]
    ) -> JournalEntry:
    ...
    def get_integration_status(cls, payment) -> Dict[str, Any]:
        ...
        return {
            ...
            "journal_entry_number": payment.financial_transaction.number
    ...
    def bulk_sync_payments(
        cls, payments: List, payment_type: str, user: Optional[User] = None
    ) -> Dict[str, Any]:
        ...
        for payment in payments:
            try:
                ...
                results["failed"] += 1
                results["errors"].append({"payment_id": payment.id, "error": str(e)})
