...
from ..models.transactions import FinancialTransaction, ExpenseTransaction, IncomeTransaction
    ...
    def create_income_transaction(
        ...
        try:
            with transaction.atomic():
                ...
                if amount >= Decimal('1000.00') or (category and category.requires_approval):
    ...
    def create_expense_transaction(
        ...
        try:
            with transaction.atomic():
                ...
                logger.info(f"تم إنشاء معاملة مصروف: {expense.title} - {expense.amount}")
                return expense
    ...
    def create_transfer_transaction(
        ...
        try:
            with transaction.atomic():
                ...
                from_account = ChartOfAccounts.objects.get(code=from_account_code, is_active=True)
                to_account = ChartOfAccounts.objects.get(code=to_account_code, is_active=True)
                
                # التحقق من كفاية الرصيد
                from_balance = from_account.get_balance()
                if from_balance < amount:
                    raise ValidationError(f"رصيد الحساب {from_account.name} غير كافي للتحويل")
                ...
                logger.info(f"تم إنشاء معاملة تحويل: {transfer.title} - {transfer.amount}")
                return transfer
        ...
        try:
            with transaction.atomic():
                ...
                if not equity_account:
                    raise ValidationError("لا يوجد حساب حقوق ملكية لإنشاء الأرصدة الافتتاحية")
                
                for balance_data in balances_data:
                    account_code = balance_data['account_code']
                    balance = Decimal(str(balance_data['balance']))
                    description = balance_data.get('description', f'رصيد افتتاحي - {account_code}')
