...
class PaymentSyncService:
    ...
    def sync_payment(
        ...
        """
        # إنشاء عملية التزامن
        sync_operation = self._create_sync_operation(
            payment_obj, operation_type, user
        )
        ...
        try:
            ...
            logger.info(f"تم تزامن الدفعة {payment_obj} بنجاح - العملية {sync_operation.operation_id}")
    ...
    def _create_sync_operation(
        ...
        payment_data = {
            ...
            'payment_date': payment_obj.payment_date.isoformat() if hasattr(payment_obj, 'payment_date') else None,
    ...
    def _execute_sync_rules(
        ...
        for rule in rules:
            try:
                ...
                self._log_sync_error(sync_operation, f"rule_execution_{rule.id}", str(e))
                raise
    ...
    def _sync_to_customer_payment(
        ...
        with transaction.atomic():
            ...
            if sync_operation.operation_type == 'create_payment' and not existing_payment:
                ...
                existing_payment.notes = f"دفعة محدثة على فاتورة مبيعات رقم {payment_obj.sale.number}"
    ...
    def _sync_to_supplier_payment(
        ...
        with transaction.atomic():
            ...
            if sync_operation.operation_type == 'create_payment' and not existing_payment:
                ...
                existing_payment.payment_date = payment_obj.payment_date
                existing_payment.payment_method = payment_obj.payment_method
                existing_payment.notes = f"دفعة محدثة على فاتورة مشتريات رقم {payment_obj.purchase.number}"
                existing_payment.save()
    ...
    def _handle_sync_error(
        ...
        logger.error(f"فشل في تزامن الدفعة - العملية {sync_operation.operation_id}: {error_message}")
    ...
    def get_sync_statistics(self) -> Dict[str, Any]:
        ...
        completed_operations = PaymentSyncOperation.objects.filter(status='completed').count()
        failed_operations = PaymentSyncOperation.objects.filter(status='failed').count()
        pending_operations = PaymentSyncOperation.objects.filter(status='pending').count()
