...
class TransactionForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ['account', 'transaction_type', 'amount', 'date', 'description', 'reference', 'reference_number', 'to_account'] if Transaction else ['name']
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            self.fields['to_account'].queryset = ChartOfAccounts.objects.filter(
                is_active=True,
                is_leaf=True
            ).filter(
                models.Q(is_cash_account=True) | 
...
class ExpenseForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ['title', 'account', 'payment_account', 'amount', 'date', 'payment_date', 'category', 'description', 'reference', 'reference_number'] if Expense else ['name']
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        if Expense:  # فقط إذا كان النموذج متوفراً
            self.fields['account'].queryset = ChartOfAccounts.objects.filter(is_active=True)
            self.fields['payment_account'].queryset = ChartOfAccounts.objects.filter(is_active=True)
        ...
        if Expense:
            try:
                ...
                default_account = ChartOfAccounts.objects.filter(name__icontains='خزينة', is_active=True).first()
                if not default_account:
                    default_account = ChartOfAccounts.objects.filter(is_cash_account=True, is_active=True).first()
...
class IncomeForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ['title', 'receiving_account', 'amount', 'date', 'received_date', 'category', 'description', 'reference', 'reference_number'] if Income else ['name']
        widgets = {
            'title': forms.TextInput(attrs={'class': 'form-control'}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if Income:  # فقط إذا كان النموذج متوفراً
            self.fields['receiving_account'].queryset = ChartOfAccounts.objects.filter(is_active=True)
...
class CashMovementForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ['account', 'movement_type', 'amount', 'description', 'movement_date', 'beneficiary', 'receipt_number', 'notes']
        widgets = {
...
class CashMovementFilterForm(forms.Form):
    ...
    search = forms.CharField(
        required=False,
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'بحث في الوصف أو المستفيد أو الرقم المرجعي'})
    )
...
class CategoryForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['name'].error_messages = {
            'required': 'يرجى إدخال اسم التصنيف'
        }
