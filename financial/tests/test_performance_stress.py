...
class PerformanceTestCase(TransactionTestCase):
    ...
    def test_balance_calculation_performance(self):
        ...
        self.assertLess(execution_time, 1.0, 
                       f"حساب الأرصدة بطيء جداً: {execution_time:.3f}s")
    ...
    def test_trial_balance_performance(self):
        ...
        self.assertLess(execution_time, 3.0,
                       f"ميزان المراجعة بطيء جداً: {execution_time:.3f}s")
    ...
    def _create_bulk_journal_entries(self, count: int):
        ...
        # إنشاء البنود
        for entry in JournalEntry.objects.filter(description__startswith='قيد اختبار أداء'):
...
class StressTestCase(TransactionTestCase):
    ...
    def _create_stress_test_data(self):
        ...
        accounts_data = []
        for i in range(100):
            accounts_data.append(ChartOfAccounts(
                code=f'1{i+1:03d}',
                name=f'حساب ضغط {i+1}',
    ...
    def test_database_connection_stress(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            futures = [
                executor.submit(perform_database_operations) 
                for _ in range(50)
            ]
    ...
    def test_memory_usage_under_load(self):
        ...
        self.assertLess(memory_increase, 100, 
                       f"استهلاك ذاكرة مفرط: {memory_increase:.1f} MB")
...
class PaymentSyncStressTestCase(TransactionTestCase):
    ...
    def test_concurrent_payment_sync(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            futures = [
                executor.submit(sync_payment, i) 
                for i in range(20)
            ]
...
class ReportsPerformanceTestCase(TestCase):
    ...
    def test_multiple_reports_generation(self):
        ...
        reports_to_generate = [
            ('trial_balance', lambda: AdvancedReportsService.generate_comprehensive_trial_balance()),
            ('balance_sheet', lambda: AdvancedReportsService.generate_balance_sheet(date.today())),
            ('income_statement', lambda: AdvancedReportsService.generate_income_statement(
