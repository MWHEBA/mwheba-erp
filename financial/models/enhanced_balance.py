...
class BalanceSnapshot(models.Model):
    ...
    last_transaction_id = models.PositiveIntegerField(_('آخر معاملة'), null=True, blank=True)
...
class AccountBalanceCache(models.Model):
    ...
    available_balance = models.DecimalField(_('الرصيد المتاح'), max_digits=15, decimal_places=2, default=0)
    pending_balance = models.DecimalField(_('الرصيد المعلق'), max_digits=15, decimal_places=2, default=0)
    ...
    # إحصائيات
    total_debits = models.DecimalField(_('إجمالي المدين'), max_digits=15, decimal_places=2, default=0)
    total_credits = models.DecimalField(_('إجمالي الدائن'), max_digits=15, decimal_places=2, default=0)
    transactions_count = models.PositiveIntegerField(_('عدد المعاملات'), default=0)
    ...
    def refresh_balance(self, force: bool = False) -> bool:
        ...
        try:
            ...
            # حساب الرصيد الجديد
            balance_info = EnhancedBalanceService.calculate_account_balance_detailed(self.account)
            ...
            logger.error(f"خطأ في تحديث كاش الرصيد للحساب {self.account.code}: {str(e)}")
            self.is_valid = False
            self.save()
            return False
...
class BalanceAuditLog(models.Model):
    ...
    old_balance = models.DecimalField(_('الرصيد القديم'), max_digits=15, decimal_places=2, null=True, blank=True)
    new_balance = models.DecimalField(_('الرصيد الجديد'), max_digits=15, decimal_places=2)
    
    # معلومات السياق
    transaction_id = models.PositiveIntegerField(_('معرف المعاملة'), null=True, blank=True)
    journal_entry_id = models.PositiveIntegerField(_('معرف القيد'), null=True, blank=True)
...
class BalanceReconciliation(models.Model):
    ...
    calculated_balance = models.DecimalField(_('الرصيد المحسوب'), max_digits=15, decimal_places=2)
    external_balance = models.DecimalField(_('الرصيد الخارجي'), max_digits=15, decimal_places=2, null=True, blank=True)
    
    # الفروقات
    system_difference = models.DecimalField(_('فرق النظام'), max_digits=15, decimal_places=2, default=0)
    external_difference = models.DecimalField(_('فرق خارجي'), max_digits=15, decimal_places=2, null=True, blank=True)
    ...
    def calculate_differences(self):
        ...
        if abs(self.system_difference) < Decimal('0.01'):
            if self.external_balance is None or abs(self.external_difference) < Decimal('0.01'):
