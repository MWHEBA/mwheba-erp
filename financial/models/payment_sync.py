...
class PaymentSyncOperation(models.Model):
    ...
    # نوع العملية وحالتها
    operation_type = models.CharField(_('نوع العملية'), max_length=20, choices=OPERATION_TYPES)
    ...
    content_type = models.ForeignKey(ContentType, on_delete=models.CASCADE, verbose_name=_('نوع الكائن'))
    object_id = models.PositiveIntegerField(_('معرف الكائن'))
    content_object = GenericForeignKey('content_type', 'object_id')
    
    # بيانات العملية
    payment_data = models.JSONField(_('بيانات الدفعة'), default=dict)
    sync_targets = models.JSONField(_('أهداف المزامنة'), default=list)  # قائمة بالجداول المطلوب المزامنة معها
    ...
    created_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True,
                                 verbose_name=_('أنشئ بواسطة'), related_name='payment_sync_operations')
...
class PaymentSyncLog(models.Model):
    ...
    sync_operation = models.ForeignKey(PaymentSyncOperation, on_delete=models.CASCADE,
                                     verbose_name=_('عملية التزامن'), related_name='logs')
    ...
    def __str__(self):
        return f"{self.action} - {self.target_model} - {'نجح' if self.success else 'فشل'}"
...
class PaymentSyncRule(models.Model):
    ...
    # مصدر التزامن
    source_model = models.CharField(_('النموذج المصدر'), max_length=20, choices=SOURCE_CHOICES)
    trigger_event = models.CharField(_('حدث التشغيل'), max_length=20, choices=TRIGGER_CHOICES)
    
    # أهداف التزامن
    sync_to_customer_payment = models.BooleanField(_('مزامنة مع دفعات العملاء'), default=False)
    sync_to_supplier_payment = models.BooleanField(_('مزامنة مع دفعات الموردين'), default=False)
    sync_to_journal_entry = models.BooleanField(_('مزامنة مع القيود المحاسبية'), default=False)
    sync_to_balance_cache = models.BooleanField(_('مزامنة مع كاش الأرصدة'), default=False)
    ...
    created_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True,
                                 verbose_name=_('أنشئ بواسطة'), related_name='payment_sync_rules')
...
class PaymentSyncError(models.Model):
    ...
    sync_operation = models.ForeignKey(PaymentSyncOperation, on_delete=models.CASCADE,
                                     verbose_name=_('عملية التزامن'), related_name='errors')
    ...
    resolved_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True,
                                  verbose_name=_('حل بواسطة'), related_name='resolved_sync_errors')
