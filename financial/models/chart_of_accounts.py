...
class AccountType(models.Model):
    ...
    parent = models.ForeignKey('self', on_delete=models.CASCADE, null=True, blank=True, 
                              verbose_name=_('النوع الأب'), related_name='children')
    ...
    created_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, 
                                 verbose_name=_('أنشئ بواسطة'), related_name='account_types_created')
...
class ChartOfAccounts(models.Model):
    ...
    code_validator = RegexValidator(
        regex=r'^\d{4,8}$',
        message=_('كود الحساب يجب أن يكون من 4 إلى 8 أرقام')
    )
    
    code = models.CharField(_('كود الحساب'), max_length=8, unique=True, validators=[code_validator])
    name = models.CharField(_('اسم الحساب'), max_length=200)
    name_en = models.CharField(_('الاسم بالإنجليزية'), max_length=200, blank=True, null=True)
    
    account_type = models.ForeignKey(AccountType, on_delete=models.PROTECT, 
                                   verbose_name=_('نوع الحساب'), related_name='accounts')
    
    parent = models.ForeignKey('self', on_delete=models.CASCADE, null=True, blank=True,
                              verbose_name=_('الحساب الأب'), related_name='children')
    
    level = models.PositiveIntegerField(_('المستوى'), default=1)
    is_leaf = models.BooleanField(_('حساب نهائي'), default=True, 
    ...
    opening_balance = models.DecimalField(_('الرصيد الافتتاحي'), max_digits=15, decimal_places=2, 
                                        default=0.00, help_text=_('الرصيد الافتتاحي للحساب'))
    ...
    is_system_account = models.BooleanField(_('حساب نظام'), default=False,
                                          help_text=_('الحسابات النظامية لا يمكن حذفها'))
    ...
    swift_code = models.CharField(_('رمز السويفت'), max_length=11, blank=True, null=True)
    
    # حدود الحساب
    credit_limit = models.DecimalField(_('حد الائتمان'), max_digits=15, decimal_places=2, 
                                     null=True, blank=True, help_text=_('الحد الأقصى للسحب على المكشوف'))
    minimum_balance = models.DecimalField(_('الحد الأدنى للرصيد'), max_digits=15, decimal_places=2, 
                                        null=True, blank=True, help_text=_('الحد الأدنى المطلوب للرصيد'))
    
    # إعدادات التنبيهات
    low_balance_alert = models.BooleanField(_('تنبيه الرصيد المنخفض'), default=False)
    low_balance_threshold = models.DecimalField(_('عتبة الرصيد المنخفض'), max_digits=15, decimal_places=2, 
    ...
    updated_at = models.DateTimeField(_('تاريخ التحديث'), auto_now=True)
    created_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True,
                                 verbose_name=_('أنشئ بواسطة'), related_name='chart_accounts_created')
    ...
    def get_balance(self, date_from=None, date_to=None, include_opening=True):
        ...
        if include_opening and self.opening_balance:
            ...
            opening_date = self.opening_balance_date or date(2020, 1, 1)  # تاريخ افتراضي
    ...
    def get_transactions_summary(self, date_from=None, date_to=None):
        ...
        return {
            ...
            'net_movement': (summary['total_debit'] or Decimal('0')) - (summary['total_credit'] or Decimal('0')),
    ...
    def update_balance(self, amount, operation='add'):
        ...
        print(f"Updating balance for account {self.id} - {self.name}: Current={self.opening_balance}, Amount={amount}, Operation={operation}")
        ...
        if operation == 'add':
            ...
            if current_balance >= amount:
                ...
                print(f"Warning: Attempted to subtract {amount} from {current_balance} for account {self.id} - {self.name}")
                new_balance = 0
        else:
        ...
        try:
            ...
            with transaction.atomic():
                ...
                print(f"Successfully updated balance for account {self.id} - {self.name}: New balance={new_balance}")
                return True
    ...
    def reconcile(self, bank_statement_balance, reconciliation_date=None):
        ...
        try:
            ...
            if difference != 0:
                ...
                return (True, f"تمت التسوية مع إنشاء قيد بقيمة {difference}", difference)
            else:
    ...
    def get_transaction_summary(self, date_from=None, date_to=None):
        ...
        return {
            ...
            'net_movement': (summary['total_debit'] or 0) - (summary['total_credit'] or 0)
        }
...
class AccountGroup(models.Model):
    ...
    description = models.TextField(_('الوصف'), blank=True, null=True)
    accounts = models.ManyToManyField('ChartOfAccounts', verbose_name=_('الحسابات'), 
                                    related_name='groups', blank=True)
    
    is_active = models.BooleanField(_('نشط'), default=True)
    created_at = models.DateTimeField(_('تاريخ الإنشاء'), auto_now_add=True)
    created_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True,
                                 verbose_name=_('أنشئ بواسطة'), related_name='account_groups_created')
