...
class FinancialCategory(models.Model):
    ...
    parent = models.ForeignKey('self', on_delete=models.CASCADE, null=True, blank=True, 
                              verbose_name=_('التصنيف الأب'), related_name='children')
    level = models.PositiveIntegerField(_('المستوى'), default=1)
    
    # الحدود المالية
    budget_limit = models.DecimalField(_('حد الميزانية'), max_digits=12, decimal_places=2, 
                                     null=True, blank=True, help_text=_('الحد الأقصى للإنفاق في هذا التصنيف'))
    warning_threshold = models.DecimalField(_('عتبة التحذير'), max_digits=5, decimal_places=2, 
                                          default=80.00, help_text=_('نسبة التحذير من حد الميزانية'))
    
    # الإعدادات
    is_active = models.BooleanField(_('نشط'), default=True)
    is_system = models.BooleanField(_('فئة نظام'), default=False, help_text=_('فئة أساسية لا يمكن حذفها'))
    ...
    created_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, 
                                  verbose_name=_('أنشئ بواسطة'), related_name='created_categories')
    ...
    def get_budget_usage(self, start_date=None, end_date=None):
        ...
        usage_percentage = (total_spent / self.budget_limit) * 100 if self.budget_limit > 0 else 0
...
class CategoryBudget(models.Model):
    ...
    category = models.ForeignKey(FinancialCategory, on_delete=models.CASCADE, 
                               verbose_name=_('التصنيف'), related_name='budgets')
    ...
    budget_amount = models.DecimalField(_('مبلغ الميزانية'), max_digits=12, decimal_places=2)
    ...
    updated_at = models.DateTimeField(_('تاريخ التحديث'), auto_now=True)
    created_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, 
                                  verbose_name=_('أنشئ بواسطة'), related_name='created_budgets')
