...
class Command(BaseCommand):
    help = (
        "فحص تكامل البيانات المالية والتأكد من صحة الربط بين الفواتير والدفعات والقيود"
    )

    def add_arguments(self, parser):
        parser.add_argument(
            "--fix", action="store_true", help="محاولة إصلاح المشاكل المكتشفة تلقائياً"
        )
        parser.add_argument(
            "--detailed", action="store_true", help="عرض تفاصيل مفصلة للمشاكل"
        )
        parser.add_argument("--export", type=str, help="تصدير النتائج إلى ملف CSV")
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("🔍 بدء فحص تكامل البيانات المالية...\n"))
    ...
    def check_invoice_journal_entries(self):
        ...
        sales_without_entries = Sale.objects.filter(
            status="confirmed", journal_entry__isnull=True
        )
        ...
        if sales_without_entries.exists():
            ...
            self.report_data.append(
                {"type": "فواتير مبيعات بدون قيود", "count": count, "severity": "متوسط"}
            )
        ...
        if purchases_without_entries.exists():
            ...
            if self.detailed:
                for purchase in purchases_without_entries[:10]:
                    self.stdout.write(
                        f"   - فاتورة {purchase.number} - {purchase.date}"
                    )
    ...
    def check_payment_financial_status(self):
        ...
        problematic_sale_payments = SalePayment.objects.filter(
            status="posted", financial_status__in=["pending", "failed"]
        )
        ...
        total_problematic = (
            problematic_sale_payments.count() + problematic_purchase_payments.count()
        )
        ...
        if total_problematic > 0:
            ...
            self.stdout.write(
                self.style.ERROR(
                    f"❌ {total_problematic} دفعة مرحّلة لكن غير مربوطة مالياً"
                )
            )
            ...
            if self.detailed:
                for payment in problematic_sale_payments[:5]:
                    self.stdout.write(
                        f"   - دفعة مبيعات #{payment.id} - فاتورة {payment.sale.number}"
                    )
                for payment in problematic_purchase_payments[:5]:
                    self.stdout.write(
                        f"   - دفعة مشتريات #{payment.id} - فاتورة {payment.purchase.number}"
                    )
        ...
        synced_without_entry_sales = SalePayment.objects.filter(
            financial_status="synced", financial_transaction__isnull=True
        )
        ...
        total_synced_without_entry = (
            synced_without_entry_sales.count() + synced_without_entry_purchases.count()
        )
        ...
        if total_synced_without_entry > 0:
            ...
            self.stdout.write(
                self.style.ERROR(
                    f"❌ {total_synced_without_entry} دفعة مربوطة لكن بدون قيد محاسبي"
                )
            )
    ...
    def check_journal_entry_balance(self):
        ...
        if unbalanced_entries:
            ...
            self.stdout.write(self.style.ERROR(f"❌ {count} قيد محاسبي غير متوازن"))
            ...
            self.report_data.append(
                {"type": "قيود غير متوازنة", "count": count, "severity": "عالي جداً"}
            )
    ...
    def check_orphaned_records(self):
        ...
        if orphaned_entries.exists():
            ...
            self.issues_found += count
            self.stdout.write(self.style.WARNING(f"⚠️  {count} قيد محاسبي بدون مرجع"))

            self.report_data.append(
                {"type": "قيود بدون مرجع", "count": count, "severity": "منخفض"}
            )

        # بنود قيود بدون حساب صحيح
        invalid_lines = JournalEntryLine.objects.filter(account__isnull=True)

        if invalid_lines.exists():
            count = invalid_lines.count()
            self.issues_found += count
            self.stdout.write(self.style.ERROR(f"❌ {count} بند قيد بدون حساب صحيح"))

            self.report_data.append(
                {"type": "بنود قيود بدون حساب", "count": count, "severity": "عالي"}
            )
    ...
    def check_duplicate_entries(self):
        ...
        duplicates = (
            JournalEntry.objects.values("reference", "date")
            .annotate(count=Count("id"))
            .filter(count__gt=1, reference__isnull=False)
            .exclude(reference="")
        )
    ...
    def check_account_balances(self):
        ...
        if negative_cash_accounts:
            ...
            self.stdout.write(self.style.WARNING(f"⚠️  {count} حساب نقدي برصيد سالب"))
            ...
            self.report_data.append(
                {"type": "حسابات نقدية برصيد سالب", "count": count, "severity": "متوسط"}
            )
    ...
    def display_summary(self):
        ...
        for severity in severity_order:
            issues_of_severity = [
                r for r in self.report_data if r["severity"] == severity
            ]
    ...
    def export_results(self):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"❌ خطأ في تصدير النتائج: {str(e)}"))
