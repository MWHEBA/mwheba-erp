...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write(self.style.WARNING('🔍 وضع المعاينة - لن يتم تنفيذ أي تغييرات'))
    ...
    def process_suppliers(self, dry_run):
        ...
        if total == 0:
            self.stdout.write(self.style.SUCCESS('✅ جميع الموردين لديهم حسابات محاسبية'))
            return
            ...
            try:
                if not dry_run:
                    with transaction.atomic():
                        account = SupplierCustomerAccountService.create_supplier_account(supplier)
                        self.stdout.write(
                            self.style.SUCCESS(
    ...
    def process_customers(self, dry_run):
        ...
        for customer in customers:
            try:
                if not dry_run:
                    with transaction.atomic():
                        account = SupplierCustomerAccountService.create_customer_account(customer)
                        self.stdout.write(
                            self.style.SUCCESS(
