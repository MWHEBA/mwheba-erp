    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if permissions_created > 0:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"🎉 تم إنشاء {permissions_created} صلاحية جديدة بنجاح!"
                    )
                )
            else:
                self.stdout.write(self.style.WARNING("جميع الصلاحيات موجودة مسبقاً"))
            ...
            self.stdout.write(self.style.ERROR(f"❌ خطأ في إنشاء الصلاحيات: {str(e)}"))
            raise e
