...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--operation-id", type=int, help="معرف العملية المحددة")
            ...
            for op in failed_ops:
                ...
                if errors.exists():
                    ...
                    for error in errors:
                        ...
                        self.stdout.write(
                            f"  - {error.error_type}: {error.error_message[:80]}... [{status}]"
                        )
    ...
    def list_pending_operations(self):
        ...
        try:
            ...
            pending_ops = PaymentSyncOperation.objects.filter(
                status="pending"
            ).order_by("-created_at")

            # العمليات قيد المعالجة
            processing_ops = PaymentSyncOperation.objects.filter(
                status="processing"
            ).order_by("-created_at")
            ...
            if pending_ops.exists():
                ...
                for op in pending_ops:
                    ...
                    if elapsed.total_seconds() > 300:  # أكثر من 5 دقائق
                        self.stdout.write(
                            self.style.ERROR("  ⚠️ معلقة لفترة طويلة - قد تحتاج تدخل")
                        )
                    else:
                ...
                for op in processing_ops:
                    ...
                    if op.started_at:
                        ...
                        if processing_time.total_seconds() > 600:  # أكثر من 10 دقائق
                            self.stdout.write(
                                self.style.ERROR("  ⚠️ معالجة طويلة - قد تكون معلقة")
                            )
                        else:
            ...
            self.stdout.write(
                "• إذا كانت العملية معلقة أكثر من 10 دقائق، قد تحتاج إعادة تشغيل"
            )
    ...
    def suggest_solution(self, operation):
        ...
        if operation.operation_type == "test_payment":
            self.stdout.write(
                self.style.WARNING("  💡 الحل: هذه عملية اختبار، يمكن حذفها")
            )
        elif "import" in str(operation.payment_data).lower():
            self.stdout.write(
                self.style.WARNING(
                    "  💡 الحل: مشكلة في استيراد النماذج، تحقق من التكامل"
                )
            )
        elif operation.retry_count >= operation.max_retries:
            self.stdout.write(
                self.style.WARNING("  💡 الحل: تم استنفاد المحاولات، يحتاج تدخل يدوي")
            )
        else:
        ...
        try:
            ...
            if operation_id:
                ...
                if not operations.exists():
                    self.stdout.write(
                        self.style.ERROR(
                            f"العملية #{operation_id} غير موجودة أو ليست فاشلة"
                        )
                    )
                    return
            else:
            ...
            if not operations.exists():
                self.stdout.write(
                    self.style.WARNING("لا توجد عمليات قابلة لإعادة المحاولة")
                )
                return
            ...
            for operation in operations:
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"  ✓ تم إعادة تعيين العملية #{operation.id}"
                        )
                    )
                    success_count += 1
                    ...
                    self.stdout.write(
                        self.style.ERROR(f"  ✗ فشل في العملية #{operation.id}: {e}")
                    )

            self.stdout.write(
                self.style.SUCCESS(f"تم إعادة تعيين {success_count} عملية")
            )
