...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🚀 بدء إعداد النظام المحاسبي الأساسي...')
        )
    ...
    def verify_system_integrity(self):
        ...
        try:
            ...
            if accounts and len(accounts) >= 4:
                ...
                self.stdout.write('  ⚠️ خدمة التكامل المحاسبي تحتاج حسابات إضافية للمبيعات')
                
            accounts = AccountingIntegrationService._get_required_accounts_for_purchase()
            if accounts and len(accounts) >= 4:
                self.stdout.write('  ✓ خدمة التكامل المحاسبي جاهزة للمشتريات')
            else:
                self.stdout.write('  ⚠️ خدمة التكامل المحاسبي تحتاج حسابات إضافية للمشتريات')
