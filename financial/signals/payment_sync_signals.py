    ...
    if created and not getattr(instance, '_synced_from_sale', False):
        try:
            ...
            if sync_service:
                ...
                logger.warning(f"لا يمكن تزامن دفعة العميل {instance.id} - الخدمة غير متاحة")
    ...
    try:
        ...
        if sync_service:
            sync_service.sync_payment(
                payment_obj=instance,
                operation_type='delete_payment',
                user=None
            )
            logger.info(f"تم تزامن حذف دفعة المبيعات: {instance.id}")
        else:
            logger.warning(f"لا يمكن تزامن حذف دفعة المبيعات {instance.id} - الخدمة غير متاحة")
...
def sync_purchase_payment_on_delete(sender, instance, **kwargs):
    ...
    try:
        ...
        if sync_service:
            ...
            logger.warning(f"لا يمكن تزامن حذف دفعة المشتريات {instance.id} - الخدمة غير متاحة")
...
def check_payment_sync_status():
    ...
    try:
        ...
        successful_operations = PaymentSyncOperation.objects.filter(status='completed').count()
