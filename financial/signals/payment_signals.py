...
class PaymentSignalHandler:
    ...
    def handle_payment_created(sender, instance, created, **kwargs):
        ...
        if not hasattr(instance, 'id'):
            logger.debug(f"تم تخطي Signal لكائن غير صالح في handle_payment_created: {type(instance).__name__}")
            return
        ...
        try:
            ...
            logger.error(f"خطأ في معالجة إنشاء الدفعة {getattr(instance, 'id', 'unknown')}: {str(e)}")
    ...
    def handle_payment_updated(sender, instance, created, **kwargs):
        ...
        # التحقق من نوع الكائن
        if not hasattr(instance, 'id'):
            logger.debug(f"تم تخطي Signal لكائن غير صالح في handle_payment_updated: {type(instance).__name__}")
            return
        ...
        try:
            ...
            logger.error(f"خطأ في معالجة تحديث الدفعة {getattr(instance, 'id', 'unknown')}: {str(e)}")
    ...
    def handle_payment_deleted(sender, instance, **kwargs):
        ...
        # التحقق من نوع الكائن
        if not hasattr(instance, 'id'):
            logger.debug(f"تم تخطي Signal لكائن غير صالح في handle_payment_deleted: {type(instance).__name__}")
            return
        ...
        try:
            ...
            logger.error(f"خطأ في معالجة حذف الدفعة {getattr(instance, 'id', 'unknown')}: {str(e)}")
    ...
    def _sync_payment_async(instance, operation_type):
        ...
        payment_model_names = ['SalePayment', 'PurchasePayment', 'CustomerPayment', 'SupplierPayment']
...
try:
    ...
    def sale_payment_saved(sender, instance, created, **kwargs):
        ...
        if created:
            PaymentSignalHandler.handle_payment_created(sender, instance, created, **kwargs)
        else:
            PaymentSignalHandler.handle_payment_updated(sender, instance, created, **kwargs)
