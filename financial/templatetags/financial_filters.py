...
def get_entry_type(lines):
    ...
    if len(lines) == 2:
        for line in lines:
            if line.debit > 0 and ('خزينة' in line.account.name or 'نقدية' in line.account.name):
                return {"type": "إيراد نقدي", "class": "bg-success"}
            elif line.credit > 0 and ('خزينة' in line.account.name or 'نقدية' in line.account.name):
