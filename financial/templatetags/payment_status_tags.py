...
def payment_status_summary(payment):
    ...
    return format_html(
        ...
        '{} {} {}'
        '</div>',
        posting_badge, financial_badge, 
        f'<small class="text-muted d-block mt-1">{extra_text}</small>' if extra_text else ""
    )
...
def payment_financial_details(payment):
    ...
    if payment.posted_by:
        details.append(f'رحّله: {payment.posted_by.get_full_name() or payment.posted_by.username}')
