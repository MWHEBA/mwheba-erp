    ...
    if request.method == "POST":
        ...
        if db_settings["ENGINE"] == "django.db.backends.sqlite3":
            ...
            response = HttpResponse(
                zip_buffer.getvalue(), content_type="application/octet-stream"
            )
            response[
                "Content-Disposition"
            ] = f'attachment; filename="db_backup_{backup_date}.zip"'
            return response
        ...
        # التعامل مع قاعدة بيانات MySQL
            ...
            try:
                ...
                messages.error(
                    request, f_("حدث خطأ أثناء عملية النسخ الاحتياطي: {str(e)}")
                )
                return redirect("utils:backup_database")

        else:
            messages.error(
                request, _("نوع قاعدة البيانات غير مدعوم للنسخ الاحتياطي التلقائي.")
            )
            return redirect("utils:backup_database")
...
def restore_database(request):
    ...
    if request.method == "POST" and request.FILES.get("backup_file"):
        ...
        # التحقق من نوع الملف
        if not (
            backup_file.name.endswith(".zip")
            or backup_file.name.endswith(".sql")
            or backup_file.name.endswith(".sqlite3")
        ):
            messages.error(
                request,
                _(
                    "نوع الملف غير مدعوم. يجب أن يكون الملف بصيغة .zip أو .sql أو .sqlite3"
                ),
            )
        ...
        try:
            ...
            if backup_file.name.endswith(".zip"):
                ...
                for file_name in os.listdir(temp_dir):
                    ...
                    if (
                        ...
                        and db_settings["ENGINE"] == "django.db.backends.sqlite3"
                    ):
                    ...
                    # استعادة قاعدة بيانات MySQL
                        ...
                        and db_settings["ENGINE"] == "django.db.backends.mysql"
                    ):
                        # تنفيذ استعادة MySQL
                        restore_command = [
                            "mysql",
                        ...
                        subprocess.run(
                            " ".join(restore_command), shell=True, check=True
                        )
                        messages.success(request, _("تم استعادة قاعدة البيانات بنجاح"))
                        return redirect("utils:restore_database")
                ...
                messages.error(
                    request, _("لم يتم العثور على ملف قاعدة بيانات صالح في الأرشيف")
                )
                return redirect("utils:restore_database")

            # ملف SQL مباشر
            elif (
                backup_file.name.endswith(".sql")
                and db_settings["ENGINE"] == "django.db.backends.mysql"
            ):
                # حفظ الملف مؤقتًا
            ...
            # ملف SQLite مباشر
                ...
                backup_file.name.endswith(".sqlite3")
                and db_settings["ENGINE"] == "django.db.backends.sqlite3"
            ):
            ...
            # نوع ملف غير مدعوم
                ...
                messages.error(
                    request, _("نوع قاعدة البيانات غير متوافق مع ملف النسخة الاحتياطية")
                )
                return redirect("utils:restore_database")
