"""
دوال مساعدة لإرسال الإيميلات في نظام Corporate ERP
"""
...
def get_email_settings_from_db():
    ...
    try:
        ...
        logger.warning(f"فشل في استيراد موديول الإعدادات: {e}")
        return None
    except AttributeError as e:
        logger.warning(f"خطأ في الوصول لخصائص الإعدادات: {e}")
        return None
    except Exception as e:
        logger.warning(f"خطأ غير متوقع في قراءة إعدادات الإيميل: {e}")
        return None
...
def get_email_connection():
    ...
    if db_settings:
        ...
        try:
            ...
            logger.error(f"❌ فشل في المصادقة مع خادم SMTP: {str(e)}")
            raise
        except SMTPConnectError as e:
            logger.error(f"❌ فشل في الاتصال بخادم SMTP: {str(e)}")
            raise
        except SMTPServerDisconnected as e:
            logger.error(f"❌ انقطع الاتصال مع خادم SMTP: {str(e)}")
            raise
        except Exception as e:
            logger.error(f"❌ خطأ غير متوقع في الاتصال بخادم SMTP: {str(e)}")
            raise
...
def send_email(
    ...
    try:
        ...
        logger.error(f"خطأ في المصادقة عند إرسال الإيميل: {str(e)}")
        if not fail_silently:
            raise
        return False
    except SMTPConnectError as e:
        logger.error(f"خطأ في الاتصال عند إرسال الإيميل: {str(e)}")
        if not fail_silently:
            raise
        return False
    except SMTPServerDisconnected as e:
        ...
        logger.error(f"خطأ غير متوقع في إرسال الإيميل: {str(e)}")
        if not fail_silently:
            raise
        return False
...
# Payment confirmation email function removed - Sale module has been replaced
# Use product payment notifications instead
