...
def get_system_timezone():
    """
    الحصول على كائن المنطقة الزمنية المحددة في إعدادات النظام
    """
    try:
...
def get_system_now():
    """
    الحصول على التاريخ والوقت الحالي بالمنطقة الزمنية المحددة في إعدادات النظام
    """
    try:
        from core.models import SystemSetting
        timezone_name = SystemSetting.get_timezone()
    ...
    return timezone.now().astimezone(system_tz)


def get_system_today():
    """
    الحصول على التاريخ الحالي بالمنطقة الزمنية المحددة في إعدادات النظام
    """
    return get_system_now().date()


def get_system_today_string():
    """
    الحصول على التاريخ الحالي كـ string بصيغة YYYY-MM-DD للاستخدام في forms
    """
    return get_system_today().strftime("%Y-%m-%d")
...
def format_currency(value, currency="", decimal_places=2):
    """
    تنسيق قيمة عملة مع إزالة العلامات العشرية للأرقام الصحيحة
...
def get_current_fiscal_year():
    ...
    end_date = datetime.date(
        start_year + 1, fiscal_year_start_month, 1
...
def format_age_in_years_months(age_in_months):
    """
    تحويل العمر من شهور إلى سنين وشهور

    المعلمات:
    age_in_months (int): العمر بالشهور

    تُرجع: نص منسق للعمر بالسنين والشهور
    """
    if not age_in_months or age_in_months < 0:
        return "0 شهر"
    ...
    if years == 0:
        ...
        if years == 1:
            ...
            year_text = f"{years} سنوات"
        else:
            year_text = f"{years} سنة"
        ...
        if months == 1:
            ...
            month_text = f"{months} شهور"
        else:
            month_text = f"{months} شهر"
