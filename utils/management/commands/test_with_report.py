...
class TestReportRunner(DiscoverRunner):
    ...
    def _generate_report(self):
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            # معلومات الرأس
            f.write("=" * 80 + "\n")
            f.write(f"تقرير اختبارات نظام Corporate ERP\n")
            f.write(
                f'تاريخ التشغيل: {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}\n'
            )
            f.write("=" * 80 + "\n\n")
            ...
            f.write("-" * 50 + "\n")
            f.write(
                f"عدد الاختبارات الناجحة: {self.test_result.testsRun - len(self.test_result.errors) - len(self.test_result.failures)}\n"
            )
    ...
    def _generate_xml_report(self, xml_file_path):
        ...
        testsuite.set("name", "Corporate ERP Tests")
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "test_labels", nargs="*", help="حزم ونماذج ودوال الاختبار التي سيتم تشغيلها"
        )
