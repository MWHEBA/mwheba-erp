...
# تم نقل النموذج إلى utils/importers.py لتجنب التضارب
from utils.importers import ImporterTestModel
...
class ImporterTests(TestCase):
    ...
    def test_process_imported_data_with_required_fields(self):
        ...
        result = process_imported_data(
            data_with_missing, self.field_mapping, self.required_fields
        )
    ...
    def test_import_from_excel(self):
        ...
        with patch("pandas.read_excel") as mock_read_excel:
            ...
            result = import_from_excel(
                mock_file, self.field_mapping, self.required_fields
            )
    ...
    def test_bulk_create_from_import(self):
        ...
        with patch.object(transaction, "atomic", lambda func: func):
            created, updated, errors = bulk_create_from_import(
                mock_model, self.test_data
            )
            ...
            created, updated, errors = bulk_create_from_import(
                mock_model, self.test_data, ["name"]
            )
    ...
    def test_validate_import_data(self):
        ...
        validators = {
            ...
            else ValidationError("السعر يجب أن يكون رقمًا"),
        }
...
class BaseImporterTest(TestCase):
    ...
    def setUp(self):
        """إعداد بيئة الاختبار"""
        # تهيئة المستورد
        self.importer = BaseImporter(model_class=ImporterTestModel)
    ...
    def test_map_column_to_field(self):
        ...
        importer = BaseImporter(model_class=ImporterTestModel, mapping={"الاسم": "name"})
    ...
    def test_validate_field(self):
        ...
        importer = BaseImporter(
            model_class=ImporterTestModel, validators={"name": validate_name}
        )
    ...
    def test_process_row(self):
        ...
        processed_data = importer.process_row(row_data)
        self.assertEqual(
            processed_data, {"name": "منتج اختبار"}
        )  # القيمة السالبة تم تجاهلها
        self.assertEqual(len(importer.errors), 1)

    @patch("utils.importers.ImporterTestModel.objects.create")
    def test_create_object(self, mock_create):
        """اختبار إنشاء كائن جديد"""
        # محاكاة إنشاء كائن ناجح
        mock_create.return_value = MagicMock(spec=ImporterTestModel)
    ...
    def test_update_object(self):
        ...
        # إنشاء كائن وهمي
        obj = MagicMock(spec=ImporterTestModel)
    ...
    def test_import_data(self):
        ...
        self.importer.create_object = MagicMock(return_value=MagicMock(spec=ImporterTestModel))
...
class PandasImporterTest(TestCase):
    ...
    def setUp(self):
        ...
        # تهيئة المستورد
        self.importer = PandasImporter(model_class=ImporterTestModel)
    ...
    def test_from_excel(self):
        ...
        importer = PandasImporter.from_excel(
            self.temp_excel_path, model_class=ImporterTestModel
        )
    ...
    def test_from_csv(self):
        ...
        importer = PandasImporter.from_csv(self.temp_csv_path, model_class=ImporterTestModel)
    ...
    def test_from_dataframe(self):
        ...
        importer = PandasImporter.from_dataframe(self.df, model_class=ImporterTestModel)
    ...
    def test_clean_data(self):
        ...
        importer = PandasImporter.from_dataframe(df, model_class=ImporterTestModel)
        importer.clean_data()
    ...
    def test_clean_data_with_mapping(self):
        ...
        importer = PandasImporter.from_dataframe(
            df, model_class=ImporterTestModel, mapping={"الاسم": "name", "القيمة": "value"}
        )
    ...
    @patch("utils.importers.ImporterTestModel.objects.get")
    def test_find_existing_object(self, mock_get):
        """اختبار البحث عن كائن موجود"""
        # محاكاة العثور على كائن
        mock_obj = MagicMock(spec=ImporterTestModel)
        mock_get.return_value = mock_obj

        # تهيئة مستورد بحقول تفرد
        importer = PandasImporter(model_class=ImporterTestModel, unique_fields=["name"])
    ...
    def test_find_existing_object_not_found(self, mock_get):
        ...
        mock_get.side_effect = ImporterTestModel.DoesNotExist()
    ...
    def test_find_existing_object_multiple(self, mock_get):
        ...
        mock_get.side_effect = ImporterTestModel.MultipleObjectsReturned()
    ...
    def test_import_all(self, mock_process, mock_create, mock_update, mock_find):
        ...
        # السطر الثالث: كائن موجود
        mock_find.side_effect = [mock_obj, None, mock_obj]

        # محاكاة إنشاء وتحديث الكائنات مع تحديث العدادات
        def mock_create_side_effect(data):
            self.importer.created_count += 1
            return MagicMock(spec=ImporterTestModel)
        
        def mock_update_side_effect(obj, data):
            self.importer.updated_count += 1
            return MagicMock(spec=ImporterTestModel)
        ...
        self.assertEqual(mock_update.call_count, 2)  # كائنان موجودان
        self.assertEqual(mock_create.call_count, 1)  # كائن واحد جديد

        # التحقق من النتائج
        self.assertEqual(len(created), 1)
        self.assertEqual(len(updated), 2)  # كائنان محدثان
        self.assertEqual(len(errors), 0)

        # التحقق من العدادات
        self.assertEqual(self.importer.created_count, 1)
        self.assertEqual(self.importer.updated_count, 2)  # كائنان محدثان
        self.assertEqual(self.importer.skipped_count, 0)  # لا توجد كائنات متخطاة

    @patch("utils.importers.PandasImporter.from_excel")
    def test_import_excel_to_model(self, mock_from_excel):
        ...
        created = [MagicMock(spec=ImporterTestModel)]
        updated = [MagicMock(spec=ImporterTestModel)]
        errors = []
        stats = {"created": 1, "updated": 1, "skipped": 0, "errors": 0}

        mock_importer.import_all.return_value = (created, updated, errors)
        ...
        mock_importer.import_all.assert_called_once_with(update_existing=True)

        # التحقق من النتائج
        self.assertEqual(result, (created, updated, errors, stats))

    @patch("utils.importers.PandasImporter.from_csv")
    def test_import_csv_to_model(self, mock_from_csv):
