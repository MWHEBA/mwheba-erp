...
class DateRangeStatsTest(TestCase):
    ...
    def test_get_date_range_stats(self, mock_timezone):
        ...
        result = get_date_range_stats(
            queryset, date_field="created_at", value_field="amount", range_type="daily"
        )

        # التحقق من النتائج
        queryset.filter.assert_called_with(created_at__date=today.date())
...
class StatsCalculationsTest(TestCase):
    ...
    def test_calculate_growth_rate(self):
        ...
        # اختبار النمو السلبي
        self.assertEqual(
            calculate_growth_rate(80, 100), -20.0
        )  # (80 - 100) / 100 * 100
