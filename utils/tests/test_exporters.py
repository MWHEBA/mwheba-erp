...
class ExporterTests(TestCase):
    ...
    def test_generate_excel_response(self, mock_export):
        ...
        response = generate_excel_response(
            self.test_data, self.field_mapping, "test_file"
        )
    ...
    def test_generate_json_response(self, mock_export):
        ...
        self.assertEqual(
            response["Content-Disposition"], 'attachment; filename="test_file.json"'
        )


# إنشاء نموذج اختبار للاستخدام في الاختبارات
class ExporterTestModel(models.Model):
...
class ExportFunctionsTest(TestCase):
    ...
    def test_export_queryset_to_pandas(self, mock_from_queryset):
        ...
        mock_from_queryset.assert_called_once_with(
            queryset, fields=["name", "value"], annotations=None
        )
