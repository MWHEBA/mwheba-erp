...
class PandasExporter:
    ...
    def to_json(self, filename=None):
        ...
        if not filename:
            filename = (
                f"export_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
    ...
    def to_html(self, filename=None):
        ...
        if not filename:
            filename = (
                f"export_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
            )
...
def export_queryset_to_pandas(queryset, fields=None, annotations=None):
    ...
    PandasExporter: كائن من نوع PandasExporter
    """
    return PandasExporter.from_queryset(
        queryset, fields=fields, annotations=annotations
    )
