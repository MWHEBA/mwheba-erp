...
def get_date_range_stats(
    ...
    if range_type == "daily":
        ...
        filter_kwargs[f"{date_field}__date__gte"] = (
            start_date.date() if hasattr(start_date, "date") else start_date
        )
        filter_kwargs[f"{date_field}__date__lte"] = (
    ...
    # حساب الإحصائيات
    stats = filtered_queryset.aggregate(total=Sum(value_field), count=Count("id"))
...
def get_period_comparison(
    queryset, date_field, value_field, range_type="monthly", custom_period=None
):
    """
    ...
    # حساب معدل النمو
    growth_rate = calculate_growth_rate(current_stats["total"], previous_stats["total"])
...
def get_top_items(queryset, group_by, value_field, order_field="total", limit=5):
    ...
    """
    result = (
        queryset.values(group_by, f"{group_by}__name")
        .annotate(total=Sum(value_field), count=Count("id"))
