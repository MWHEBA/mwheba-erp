...
# استخدام ReportLab بدلاً من WeasyPrint
try:
...
def export_to_pdf(data, fields, headers=None, title="تقرير", orientation="portrait"):
    """
    تصدير بيانات إلى ملف PDF باستخدام ReportLab
    ...
    if not REPORTLAB_AVAILABLE:
        raise ImportError("ReportLab library is not installed. Please install it: pip install reportlab")
    
    # إنشاء buffer للـ PDF
    buffer = BytesIO()
    
    # تحديد حجم واتجاه الصفحة
    ...
    # إعداد بيانات الجدول
    table_data = [headers]  # الصف الأول: العناوين
...
class ExcelExporter:
    ...
    def __init__(self, filename=None, sheet_name=None):
        ...
        self.filename = (
            filename
            or f"export_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        )
    ...
    def add_data(self, data_rows):
        ...
        for row_data in data_rows:
            for col, cell_data in enumerate(row_data):
                ...
                if isinstance(cell_data, datetime.date) or isinstance(
                    cell_data, datetime.datetime
                ):
                    self.worksheet.write(
                        self.current_row, col, cell_data, self.date_format
                    )
                elif isinstance(cell_data, (int, float)):
                    self.worksheet.write(
                        self.current_row, col, cell_data, self.number_format
                    )
                else:
                    self.worksheet.write(
                        self.current_row, col, cell_data, self.data_format
                    )
...
class CSVExporter:
    ...
    def __init__(self, filename=None):
        ...
        self.filename = (
            filename
            or f"export_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
...
def export_queryset_to_excel(
    ...
    return exporter.add_queryset(
        queryset, fields=fields, headers=headers, annotations=annotations
    ).save()


def export_queryset_to_csv(
    queryset, filename=None, fields=None, headers=None, annotations=None
):
    """
