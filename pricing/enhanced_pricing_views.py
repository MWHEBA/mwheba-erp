...
def enhanced_plate_price_api(request):
    ...
    try:
        ...
        return JsonResponse({
            'success': False,
            'error': f'خطأ في حساب سعر الزنكات: {str(e)}'
        })
...
def enhanced_total_cost_api(request):
    ...
    try:
        ...
        # إعداد بيانات الورق
        paper_data = None
        if all(key in data for key in ['paper_type', 'paper_size', 'weight', 'quantity']):
            paper_data = {
        ...
        if 'press_type' in data:
            printing_data = {
                ...
                'supplier_id': int(data.get('supplier')) if data.get('supplier') else None
            }
...
def enhanced_suppliers_by_service_api(request):
    ...
    try:
        ...
        suppliers = SimplePricingCalculator.get_available_suppliers_for_service(service_type)
...
def pricing_calculator_test_api(request):
    ...
    try:
        ...
        return JsonResponse({
            'success': False,
            'error': f'خطأ في اختبار حاسبة التسعير: {str(e)}'
        })
