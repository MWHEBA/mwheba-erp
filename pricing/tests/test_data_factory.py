...
from pricing.models import (
    PricingOrder, PaperType, PaperSize, CoatingType, 
    FinishingType, PlateSize, VATSetting, PricingSupplierSelection
)
from supplier.models import (
    Supplier, SupplierType, SupplierServiceTag, SpecializedService, 
    PaperServiceDetails, DigitalPrintingDetails, 
    PlateServiceDetails, ServicePriceTier
)
...
class PricingTestDataFactory:
    ...
    def create_suppliers(supplier_types, service_tags, count=15):
        ...
        suppliers = []
        supplier_names = [
            'مطبعة النجاح', 'شركة الإبداع للطباعة', 'مؤسسة التميز',
            'مطبعة الجودة العالية', 'شركة الفن الحديث', 'مكتب التصميم المتقدم',
            'مطبعة السرعة', 'شركة الدقة للطباعة', 'مؤسسة الاحتراف',
            'مطبعة التقنية الحديثة', 'شركة الإنتاج المتميز', 'مكتب الخدمات الشاملة',
            'مطبعة الجودة الفائقة', 'شركة الحلول المتكاملة', 'مؤسسة الخبرة'
        ]
    ...
    def create_paper_types_and_sizes():
        ...
        for name, width, height, unit in paper_sizes_data:
            paper_size, created = PaperSize.objects.get_or_create(
                name=name,
                defaults={'width': width, 'height': height, 'unit': unit}
            )
    ...
    def create_specialized_services(suppliers, supplier_types):
        ...
        service_names = [
            'تغطية لامع', 'تغطية مطفي', 'قص بالليزر', 'تجليد حلزوني',
            'تجليد مثالي', 'طباعة أوفست', 'طباعة رقمية', 'تصميم إبداعي'
        ]
    ...
    @staticmethod
    def create_pricing_orders(customers, suppliers, paper_types, paper_sizes, users, count=20):
    ...
    def create_complete_test_environment():
        ...
        # إنشاء الموردين
        suppliers = PricingTestDataFactory.create_suppliers(supplier_types, service_tags, 15)
        ...
        # إنشاء خدمات الورق
        paper_services = PricingTestDataFactory.create_paper_services(suppliers, paper_types, paper_sizes)
        print(f"تم إنشاء {len(paper_services)} خدمة ورق")
        
        # إنشاء خدمات الطباعة الرقمية
        digital_services = PricingTestDataFactory.create_digital_printing_services(suppliers, paper_sizes)
        print(f"تم إنشاء {len(digital_services)} خدمة طباعة رقمية")
        
        # إنشاء خدمات متخصصة
        specialized_services = PricingTestDataFactory.create_specialized_services(suppliers, supplier_types)
        print(f"تم إنشاء {len(specialized_services)} خدمة متخصصة")
        
        # إنشاء طلبات تسعير
        orders = PricingTestDataFactory.create_pricing_orders(customers, suppliers, paper_types, paper_sizes, users, 20)
