...
class PricingScenarioTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.paper_service_tag = SupplierServiceTag.objects.create(
            name='ورق عادي',
            code='paper_normal',
            description='خدمات الورق العادي'
        )
        ...
        self.paper_type_glossy = PaperType.objects.create(
            name='ورق لامع',
            code='GLOSSY',
            description='ورق لامع للطباعة الفاخرة'
        )
    ...
    def test_scenario_small_business_cards(self):
        ...
        self.assertGreater(total_cost, paper_cost + printing_cost, "التكلفة الإجمالية يجب أن تشمل الضريبة")
    ...
    def test_scenario_large_catalog_printing(self):
        ...
        self.assertGreater(paper_cost, printing_cost * 0.3, "تكلفة الورق يجب أن تكون نسبة معقولة من تكلفة الطباعة")
        
        # اختبار خصومات الكمية (إذا كانت متوفرة)
        unit_cost = total_cost / order.quantity
        self.assertLess(unit_cost, Decimal('5.00'), "تكلفة الوحدة يجب أن تقل في الكميات الكبيرة")
