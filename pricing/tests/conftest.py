...
def paper_size():
    ...
    return PaperSize.objects.create(
        name="A4", width=21.0, height=29.7, description="مقاس A4 قياسي"
    )
...
def coating_type():
    ...
    return CoatingType.objects.create(
        name="لامينيشن لامع", description="طلاء لامع", price_per_sheet=Decimal("0.25")
    )
...
def print_direction():
    ...
    return PrintDirection.objects.create(
        name="طباعة وجه واحد", description="طباعة على وجه واحد فقط"
    )
...
def extra_expense(pricing_order):
    ...
    return ExtraExpense.objects.create(
        order=pricing_order, description="مصروف شحن", amount=Decimal("50.00")
    )
...
def order_comment(pricing_order, user):
    ...
    return OrderComment.objects.create(
        order=pricing_order, comment="تعليق تجريبي على الطلب", created_by=user
    )
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "slow: اختبارات بطيئة التنفيذ")
    config.addinivalue_line("markers", "integration: اختبارات التكامل")
    config.addinivalue_line("markers", "unit: اختبارات الوحدة")
    config.addinivalue_line("markers", "api: اختبارات الـ API")
    config.addinivalue_line("markers", "view: اختبارات الواجهات")
    config.addinivalue_line("markers", "service: اختبارات الخدمات")
    config.addinivalue_line("markers", "model: اختبارات النماذج")
    config.addinivalue_line("markers", "form: اختبارات النماذج")
    config.addinivalue_line("markers", "javascript: اختبارات JavaScript")
    config.addinivalue_line("markers", "selenium: اختبارات Selenium")
    config.addinivalue_line("markers", "performance: اختبارات الأداء")
    config.addinivalue_line("markers", "security: اختبارات الأمان")
...
class TestDataFactory:
    ...
    def create_paper_sizes(count=5):
        ...
        for i in range(min(count, len(sizes))):
            ...
            paper_size = PaperSize.objects.create(name=name, width=width, height=height)
