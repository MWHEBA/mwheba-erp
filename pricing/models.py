...
class PaperWeight(models.Model):
    ...
    gsm = models.PositiveIntegerField(_('الوزن (جرام)'), validators=[MinValueValidator(50)])
    description = models.TextField(_('الوصف'), blank=True)
    is_active = models.BooleanField(_('نشط'), default=True)
    is_default = models.BooleanField(_('افتراضي'), default=False)
    created_at = models.DateTimeField(_('تاريخ الإنشاء'), auto_now_add=True)
...
class VATSetting(models.Model):
    ...
    percentage = models.DecimalField(_('النسبة المئوية'), max_digits=5, decimal_places=2, default=Decimal('15.00'))
    description = models.TextField(_('الوصف'), blank=True)
    effective_date = models.DateField(_('تاريخ السريان'), auto_now_add=True)
    created_by = models.ForeignKey(User, on_delete=models.PROTECT, related_name='vat_settings', verbose_name=_('تم الإنشاء بواسطة'))
    created_at = models.DateTimeField(_('تاريخ الإنشاء'), auto_now_add=True)
...
class PricingOrder(models.Model):
    ...
    order_number = models.CharField(_('رقم الطلب'), max_length=20, unique=True, editable=False)
    client = models.ForeignKey(Client, on_delete=models.CASCADE, related_name='orders', verbose_name=_('العميل'))
    ...
    # تحسينات ربط العملاء
    client_contact_person = models.CharField(_('الشخص المسؤول'), max_length=100, blank=True)
    ...
    client_type = models.CharField(_('نوع العميل'), max_length=20, choices=CLIENT_TYPES, default='regular')
    ...
    # الحقول المشتركة
    has_internal_content = models.BooleanField(_('يحتوي على محتوى داخلي'), default=False)
    
    # معلومات المنتج والورق والطباعة
    product_type = models.ForeignKey('ProductType', on_delete=models.PROTECT, related_name='orders', verbose_name=_('نوع المنتج'), null=True, blank=True)
    paper_type = models.ForeignKey(PaperType, on_delete=models.PROTECT, related_name='orders', verbose_name=_('نوع الورق'))
    paper_size = models.ForeignKey('ProductSize', on_delete=models.PROTECT, related_name='orders', verbose_name=_('مقاس المنتج'))
    print_direction = models.ForeignKey(PrintDirection, on_delete=models.PROTECT, related_name='orders', verbose_name=_('اتجاه الطباعة'))
    print_sides = models.ForeignKey(PrintSide, on_delete=models.PROTECT, related_name='orders', verbose_name=_('جوانب الطباعة'))
    ...
    custom_width = models.DecimalField(_('العرض المخصص (سم)'), max_digits=8, decimal_places=2, null=True, blank=True)
    custom_height = models.DecimalField(_('الطول المخصص (سم)'), max_digits=8, decimal_places=2, null=True, blank=True)
    
    # معلومات المونتاج
    montage_info = models.TextField(_('معلومات المونتاج'), blank=True)
    
    # معلومات التصميم
    design_price = models.DecimalField(_('سعر التصميم'), max_digits=10, decimal_places=2, default=Decimal('0.00'))
    internal_design_price = models.DecimalField(_('سعر التصميم الداخلي'), max_digits=10, decimal_places=2, default=Decimal('0.00'))
    
    # خدمات مابعد الطباعة والتغطية
    coating_type = models.ForeignKey(CoatingType, on_delete=models.PROTECT, related_name='orders', verbose_name=_('نوع التغطية'), null=True, blank=True)
    coating_service = models.ForeignKey('supplier.SpecializedService', on_delete=models.PROTECT, related_name='coating_orders', verbose_name=_('خدمة التغطية'), null=True, blank=True)
    ...
    supplier = models.ForeignKey(Supplier, on_delete=models.PROTECT, related_name='orders', verbose_name=_('المورد'), null=True, blank=True)
    ...
    # التكاليف والأسعار
    material_cost = models.DecimalField(_('تكلفة الخامات'), max_digits=12, decimal_places=2, default=Decimal('0.00'))
    printing_cost = models.DecimalField(_('تكلفة الطباعة'), max_digits=12, decimal_places=2, default=Decimal('0.00'))
    finishing_cost = models.DecimalField(_('تكلفة خدمات الطباعة'), max_digits=12, decimal_places=2, default=Decimal('0.00'))
    plates_cost = models.DecimalField(_('تكلفة الزنكات'), max_digits=12, decimal_places=2, default=Decimal('0.00'))
    extra_cost = models.DecimalField(_('تكاليف إضافية'), max_digits=12, decimal_places=2, default=Decimal('0.00'))
    total_cost = models.DecimalField(_('إجمالي التكلفة'), max_digits=12, decimal_places=2, default=Decimal('0.00'))
    
    profit_margin = models.DecimalField(_('هامش الربح (%)'), max_digits=5, decimal_places=2, default=Decimal('20.00'))
    sale_price = models.DecimalField(_('سعر البيع'), max_digits=12, decimal_places=2, default=Decimal('0.00'))
        ...
        try:
            if self.order_type == 'digital':
                ...
                digital_service = DigitalPrintingDetails.objects.filter(
                    supplier=self.supplier,
                    paper_size=self.paper_size,
                    color_type='color' if (self.colors_front + self.colors_back) > 1 else 'bw'
                ).first()
            ...
            cost = base_cost * (Decimal('1.0') + color_multiplier) * quantity_discount * self.quantity
            self.printing_cost = cost
            return cost
...
class InternalContent(models.Model):
    ...
    order = models.OneToOneField(PricingOrder, on_delete=models.CASCADE, related_name='internal_content', verbose_name=_('الطلب'))
    paper_type = models.ForeignKey(PaperType, on_delete=models.PROTECT, related_name='internal_contents', verbose_name=_('نوع الورق'))
    paper_size = models.ForeignKey(PaperSize, on_delete=models.PROTECT, related_name='internal_contents', verbose_name=_('مقاس المنتج'))
    page_count = models.PositiveIntegerField(_('عدد الصفحات'), validators=[MinValueValidator(1)])
    print_sides = models.ForeignKey(PrintSide, on_delete=models.PROTECT, related_name='internal_contents', verbose_name=_('جوانب الطباعة'))
    colors_front = models.PositiveIntegerField(_('عدد ألوان الوجه الأمامي'), default=4)
    colors_back = models.PositiveIntegerField(_('عدد ألوان الوجه الخلفي'), default=0)
    material_cost = models.DecimalField(_('تكلفة الخامات'), max_digits=12, decimal_places=2, default=Decimal('0.00'))
    printing_cost = models.DecimalField(_('تكلفة الطباعة'), max_digits=12, decimal_places=2, default=Decimal('0.00'))
...
class OrderFinishing(models.Model):
    ...
    order = models.ForeignKey(PricingOrder, on_delete=models.CASCADE, related_name='finishing_services', verbose_name=_('الطلب'))
    finishing_type = models.ForeignKey(FinishingType, on_delete=models.PROTECT, related_name='order_services', verbose_name=_('نوع خدمات مابعد الطباعة'), null=True, blank=True)
    supplier_service = models.ForeignKey('supplier.SpecializedService', on_delete=models.PROTECT, related_name='order_services', verbose_name=_('خدمة المورد'), null=True, blank=True)
    quantity = models.PositiveIntegerField(_('الكمية'), validators=[MinValueValidator(1)])
...
class ExtraExpense(models.Model):
    ...
    order = models.ForeignKey(PricingOrder, on_delete=models.CASCADE, related_name='extra_expenses', verbose_name=_('الطلب'))
...
class CtpPlates(models.Model):
    ...
    order = models.ForeignKey(PricingOrder, on_delete=models.CASCADE, related_name='ctp_plates', verbose_name=_('الطلب'))
    is_internal = models.BooleanField(_('للمحتوى الداخلي'), default=False)
    supplier = models.ForeignKey(Supplier, on_delete=models.PROTECT, related_name='order_plates', verbose_name=_('المورد'))
    plate_size = models.ForeignKey(PlateSize, on_delete=models.PROTECT, related_name='order_plates', verbose_name=_('مقاس الزنك'))
    plates_count = models.PositiveIntegerField(_('عدد الزنكات'), validators=[MinValueValidator(1)])
    plate_price = models.DecimalField(_('سعر الزنك'), max_digits=10, decimal_places=2)
    transportation_cost = models.DecimalField(_('تكلفة الانتقالات'), max_digits=10, decimal_places=2, default=Decimal('0.00'))
    total_cost = models.DecimalField(_('التكلفة الإجمالية'), max_digits=12, decimal_places=2)
    notes = models.TextField(_('ملاحظات'), blank=True)
    ...
    def __str__(self):
        return f"{_('زنكات')} - {self.order.order_number} - {self.plates_count} {_('زنك')}"
    
    def save(self, *args, **kwargs):
        # حساب إجمالي التكلفة
        self.total_cost = (self.plates_count * self.plate_price) + self.transportation_cost
        super().save(*args, **kwargs)
...
class OrderComment(models.Model):
    ...
    order = models.ForeignKey(PricingOrder, on_delete=models.CASCADE, related_name='comments', verbose_name=_('الطلب'))
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='order_comments', verbose_name=_('المستخدم'))
...
class OrderSupplier(models.Model):
    ...
    order = models.ForeignKey(PricingOrder, on_delete=models.CASCADE, related_name='order_suppliers', verbose_name=_('الطلب'))
    supplier = models.ForeignKey(Supplier, on_delete=models.CASCADE, related_name='supplier_orders', verbose_name=_('المورد'))
    role = models.CharField(_('دور المورد'), max_length=20, choices=SUPPLIER_ROLES, default='secondary')
    ...
    estimated_cost = models.DecimalField(_('التكلفة المقدرة'), max_digits=12, decimal_places=2, default=Decimal('0.00'))
    quoted_price = models.DecimalField(_('السعر المعروض'), max_digits=12, decimal_places=2, default=Decimal('0.00'))
...
class PricingQuotation(models.Model):
    ...
    pricing_order = models.OneToOneField(PricingOrder, on_delete=models.CASCADE, related_name='quotation', verbose_name=_('طلب التسعير'))
    quotation_number = models.CharField(_('رقم العرض'), max_length=20, unique=True, editable=False)
    ...
    payment_terms = models.TextField(_('شروط الدفع'), default='50% مقدم، 50% عند التسليم')
    delivery_terms = models.TextField(_('شروط التسليم'), default='التسليم خلال 7-10 أيام عمل')
    ...
    sent_via = models.CharField(_('طريقة الإرسال'), max_length=20, choices=SENT_VIA_CHOICES, default='email')
    ...
    final_price = models.DecimalField(_('السعر النهائي'), max_digits=12, decimal_places=2, default=Decimal('0.00'))
    ...
    # معلومات المستخدم
    created_by = models.ForeignKey(User, on_delete=models.PROTECT, related_name='created_quotations', verbose_name=_('تم الإنشاء بواسطة'))
    ...
    def save(self, *args, **kwargs):
        ...
        if self.discount_percentage > 0:
            self.discount_amount = self.pricing_order.sale_price * (self.discount_percentage / 100)
...
class PricingApprovalWorkflow(models.Model):
    ...
    # حدود الموافقة
    min_amount = models.DecimalField(_('الحد الأدنى للمبلغ'), max_digits=12, decimal_places=2, default=Decimal('0.00'))
    max_amount = models.DecimalField(_('الحد الأقصى للمبلغ'), max_digits=12, decimal_places=2, null=True, blank=True)
    ...
    # إعدادات إضافية
    auto_approve_below_limit = models.BooleanField(_('موافقة تلقائية تحت الحد الأدنى'), default=False)
    require_both_approvers = models.BooleanField(_('يتطلب موافقة المعتمدين'), default=False)
    
    # تواريخ
    created_at = models.DateTimeField(_('تاريخ الإنشاء'), auto_now_add=True)
    updated_at = models.DateTimeField(_('تاريخ التحديث'), auto_now=True)
    created_by = models.ForeignKey(User, on_delete=models.PROTECT, related_name='created_workflows', verbose_name=_('تم الإنشاء بواسطة'))
...
class PricingApproval(models.Model):
    ...
    pricing_order = models.ForeignKey(PricingOrder, on_delete=models.CASCADE, related_name='approvals', verbose_name=_('طلب التسعير'))
    workflow = models.ForeignKey(PricingApprovalWorkflow, on_delete=models.CASCADE, verbose_name=_('تدفق الموافقة'))
    
    # معلومات الموافقة
    approver = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name=_('المعتمد'))
    approval_level = models.CharField(_('مستوى الموافقة'), max_length=20, choices=APPROVAL_LEVELS)
    ...
    notification_method = models.CharField(_('طريقة الإشعار'), max_length=20, choices=NOTIFICATION_METHODS, default='email')
    notification_sent_at = models.DateTimeField(_('تاريخ إرسال الإشعار'), null=True, blank=True)
    
    # معلومات إضافية
    requested_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='requested_approvals', verbose_name=_('طلب الموافقة'))
    priority = models.CharField(_('الأولوية'), max_length=10, choices=(
        ('low', _('منخفضة')),
        ('normal', _('عادية')),
        ('high', _('عالية')),
        ('urgent', _('عاجلة')),
...
class PricingReport(models.Model):
    ...
    generated_by = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name=_('تم الإنشاء بواسطة'))
    ...
    # فلاتر التقرير
    client_filter = models.ForeignKey('client.Customer', on_delete=models.CASCADE, null=True, blank=True, verbose_name=_('فلتر العميل'))
    supplier_filter = models.ForeignKey('supplier.Supplier', on_delete=models.CASCADE, null=True, blank=True, verbose_name=_('فلتر المورد'))
    order_type_filter = models.CharField(_('فلتر نوع الطلب'), max_length=10, choices=PricingOrder.ORDER_TYPES, blank=True)
    ...
    schedule_frequency = models.CharField(_('تكرار الجدولة'), max_length=10, choices=(
        ('daily', _('يومي')),
        ('weekly', _('أسبوعي')),
        ('monthly', _('شهري')),
...
class PricingKPI(models.Model):
    ...
    expired_quotations = models.PositiveIntegerField(_('العروض المنتهية الصلاحية'), default=0)
    
    # مؤشرات مالية (مستقلة)
    total_quoted_value = models.DecimalField(_('إجمالي قيمة العروض'), max_digits=15, decimal_places=2, default=Decimal('0.00'))
    accepted_value = models.DecimalField(_('قيمة العروض المقبولة'), max_digits=15, decimal_places=2, default=Decimal('0.00'))
    rejected_value = models.DecimalField(_('قيمة العروض المرفوضة'), max_digits=15, decimal_places=2, default=Decimal('0.00'))
    average_order_value = models.DecimalField(_('متوسط قيمة الطلب'), max_digits=12, decimal_places=2, default=Decimal('0.00'))
    
    # مؤشرات الأداء
    success_rate = models.DecimalField(_('معدل النجاح %'), max_digits=5, decimal_places=2, default=Decimal('0.00'))
    average_response_time = models.PositiveIntegerField(_('متوسط وقت الرد (أيام)'), default=0)
    average_profit_margin = models.DecimalField(_('متوسط هامش الربح %'), max_digits=5, decimal_places=2, default=Decimal('0.00'))
    ...
    def calculate_daily_kpi(cls, date):
        ...
        pending_quotations = quotations.filter(status__in=['draft', 'sent', 'under_review']).count()
        expired_quotations = quotations.filter(status='expired').count()
        
        # المؤشرات المالية
        total_quoted_value = quotations.aggregate(total=Sum('final_price'))['total'] or Decimal('0.00')
        accepted_value = quotations.filter(status='accepted').aggregate(total=Sum('final_price'))['total'] or Decimal('0.00')
        rejected_value = quotations.filter(status='rejected').aggregate(total=Sum('final_price'))['total'] or Decimal('0.00')
        average_order_value = orders.aggregate(avg=Avg('sale_price'))['avg'] or Decimal('0.00')
        
        # حساب معدل النجاح
        success_rate = (accepted_quotations / total_quotations * 100) if total_quotations > 0 else Decimal('0.00')
        
        # متوسط هامش الربح
        average_profit_margin = orders.aggregate(avg=Avg('profit_margin'))['avg'] or Decimal('0.00')
        
        # العملاء والموردين
        new_clients = orders.values('client').distinct().count()
        active_suppliers = OrderSupplier.objects.filter(created_at__date=date).values('supplier').distinct().count()
...
class PricingSupplierSelection(models.Model):
    ...
    is_selected = models.BooleanField(
        _('مختار'), 
        default=False,
        help_text=_('هل تم اختيار هذا المورد للخدمة')
    )
    ...
    selection_reason = models.TextField(
        _('سبب الاختيار'), 
        blank=True,
        help_text=_('سبب اختيار أو رفض هذا المورد')
    )
    notes = models.TextField(
        _('ملاحظات'), 
        blank=True,
        help_text=_('ملاحظات إضافية حول التعامل مع المورد')
    )
