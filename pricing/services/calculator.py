...
class PricingCalculatorService:
    ...
    def calculate_sheets_needed(self):
        ...
        if self.order.has_internal_content and hasattr(self.order, "internal_content"):
            internal_sheets = (
                self.order.quantity * self.order.internal_content.page_count
            )
    ...
    def _calculate_digital_printing_cost(self):
        ...
        if digital_service:
            ...
            if hasattr(
                self.order.print_sides, "name"
            ) and self.order.print_sides.name in ["وجهين", "double"]:
                copies_needed *= 2
    ...
    def calculate_finishing_cost(self):
        ...
        try:
            ...
            if self.order.coating_service:
                coating_cost = (
                    self.order.coating_service.unit_price * self.order.quantity
                )
    ...
    def calculate_sale_price(self, total_cost):
        ...
        sale_price = total_cost * (
            Decimal("1.0") + (self.order.profit_margin / Decimal("100.0"))
        )
    ...
    def get_cost_breakdown(self):
        ...
        return {
            ...
            "unit_price": self.order.sale_price / self.order.quantity
