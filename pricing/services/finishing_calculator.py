...
class FinishingCalculatorService:
    ...
    def calculate_coating_cost(service_id, quantity):
        ...
        try:
            service = SpecializedService.objects.get(
                id=service_id, category__code="finishing", is_active=True
            )
            ...
            return {"success": False, "error": "خدمة التغطية غير موجودة"}
        except Exception as e:
            return {"success": False, "error": f"خطأ في حساب تكلفة التغطية: {str(e)}"}
    ...
    def calculate_folding_cost(service_id, quantity, folds_count=1):
        ...
        try:
            ...
            fold_multiplier = (
                Decimal(str(folds_count)) if folds_count > 1 else Decimal("1.0")
            )
            ...
            return {"success": False, "error": "خدمة الطي غير موجودة"}
        except Exception as e:
            return {"success": False, "error": f"خطأ في حساب تكلفة الطي: {str(e)}"}
    ...
    def calculate_die_cut_cost(service_id, quantity, cut_complexity="simple"):
        ...
        try:
            ...
            return {"success": False, "error": "خدمة القص غير موجودة"}
        except Exception as e:
            return {"success": False, "error": f"خطأ في حساب تكلفة القص: {str(e)}"}
    ...
    def calculate_spot_uv_cost(service_id, quantity, coverage_percentage=10):
        ...
        try:
            ...
            return {"success": False, "error": "خدمة الورنيش الموضعي غير موجودة"}
        except Exception as e:
            return {
                "success": False,
    ...
    @staticmethod
    def calculate_binding_cost(
        service_id, quantity, pages_count=1, binding_type="saddle_stitch"
    ):
        """حساب تكلفة التجليد"""
        try:
            service = SpecializedService.objects.get(
            ...
            # مضاعف عدد الصفحات
            pages_multiplier = Decimal("1.0") + (
                Decimal(str(pages_count)) / Decimal("100.0")
            )
            ...
            return {"success": False, "error": "خدمة التجليد غير موجودة"}
        except Exception as e:
            return {"success": False, "error": f"خطأ في حساب تكلفة التجليد: {str(e)}"}
        ...
        try:
            ...
            for service_data in services_data:
                ...
                if service_type == "coating":
                    result = FinishingCalculatorService.calculate_coating_cost(
                        service_id, quantity
                    )
                elif service_type == "folding":
                    folds_count = service_data.get("folds_count", 1)
                    result = FinishingCalculatorService.calculate_folding_cost(
                        service_id, quantity, folds_count
                    )
                elif service_type == "die_cut":
                    complexity = service_data.get("cut_complexity", "simple")
                    result = FinishingCalculatorService.calculate_die_cut_cost(
                        service_id, quantity, complexity
                    )
                elif service_type == "spot_uv":
                    coverage = service_data.get("coverage_percentage", 10)
                    result = FinishingCalculatorService.calculate_spot_uv_cost(
                        service_id, quantity, coverage
                    )
                    ...
                    result = FinishingCalculatorService.calculate_binding_cost(
                        service_id, quantity, pages_count, binding_type
                    )
                else:
                    continue
    ...
    def get_finishing_services_by_type(service_type):
        ...
        try:
            ...
            return {"success": True, "services": services_list}

        except Exception as e:
            return {"success": False, "error": f"خطأ في جلب خدمات التشطيب: {str(e)}"}
