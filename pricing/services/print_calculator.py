...
class PrintCalculatorService:
    ...
    def calculate_digital_printing_cost(
        supplier_id, paper_size_id, quantity, color_type="color", sides=1
    ):
    ...
    @staticmethod
    def calculate_offset_printing_cost(
        quantity, colors_front=4, colors_back=0, paper_size_factor=1.0
    ):
        ...
        try:
            ...
            color_multiplier = Decimal("1.0") + (
                Decimal(str(total_colors)) * Decimal("0.25")
            )
            ...
            return {
                ...
                "quantity_discount_percent": float(
                    (Decimal("1.0") - quantity_discount) * Decimal("100.0")
                ),
                "breakdown": {
    ...
    def calculate_plates_cost(
        supplier_id, plate_size_id, plates_count, transportation_included=True
    ):
        """حساب تكلفة الزنكات"""
        try:
            ...
            if not plate_service:
                return {"success": False, "error": "لم يتم العثور على سعر للزنك المحدد"}
            ...
            transportation_cost = (
                plate_service.transportation_cost
