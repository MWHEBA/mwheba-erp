        ...
        try:
            with transaction.atomic():
                if options['separate']:
                    ...
                    call_command('loaddata', 'pricing/fixtures/offset_machine_types.json', verbosity=0)
                    
                    self.stdout.write("📥 تحميل مقاسات ماكينات الأوفست...")
                    call_command('loaddata', 'pricing/fixtures/offset_sheet_sizes.json', verbosity=0)
                else:
                    # تحميل من ملف واحد
                    self.stdout.write("📥 تحميل جميع إعدادات ماكينات الأوفست...")
                    call_command('loaddata', 'pricing/fixtures/offset_settings_initial_data.json', verbosity=0)

                self.stdout.write(self.style.SUCCESS("✅ تم تحميل إعدادات ماكينات الأوفست بنجاح!"))
                self.stdout.write("\n📊 البيانات المحملة:")
                self.stdout.write("   • 2 نوع ماكينة أوفست (هايدلبرج SM52 و GTO52)")
                self.stdout.write("   • 3 مقاسات ورق (ربع فرخ، نصف فرخ، فرخ كامل)")
                
                self.stdout.write("\n🌐 يمكنك الآن زيارة:")
                self.stdout.write("   • http://127.0.0.1:8000/pricing/settings/offset-machine-types/")
                self.stdout.write("   • http://127.0.0.1:8000/pricing/settings/offset-sheet-sizes/")
