...
from supplier.models import (
    PaperServiceDetails, PlateServiceDetails, DigitalPrintingDetails
)
...
class SimplePricingCalculator:
    ...
    @staticmethod
    def calculate_paper_cost(paper_type_id, paper_size_id, weight, quantity, supplier_id=None):
        ...
        try:
            ...
            if not paper_service:
                return {
                    'success': False,
                    'error': 'لم يتم العثور على خدمة ورق مناسبة'
                }
            ...
            return {
                'success': False,
                'error': f'خطأ في حساب تكلفة الورق: {str(e)}'
            }
    
    @staticmethod
    def calculate_digital_printing_cost(paper_size_id, color_type, quantity, supplier_id=None):
    ...
    def calculate_total_project_cost(paper_data, printing_data, finishing_data=None):
        ...
        try:
            ...
            # حساب تكلفة الورق
            if paper_data:
                paper_result = SimplePricingCalculator.calculate_paper_cost(**paper_data)
            ...
            if printing_data:
                ...
                if press_type == 'digital':
                    printing_result = SimplePricingCalculator.calculate_digital_printing_cost(
    ...
    def get_available_suppliers_for_service(service_type):
        ...
        try:
            if service_type == 'paper':
                supplier_ids = PaperServiceDetails.objects.filter(
                    is_active=True
                ).values_list('supplier_id', flat=True).distinct()
            elif service_type == 'digital_printing':
                supplier_ids = DigitalPrintingDetails.objects.filter(
                    is_active=True
                ).values_list('supplier_id', flat=True).distinct()
            else:
                return []
