...
from .models import (
    PaperType, PaperSize, PrintSide, PrintDirection,
    CoatingType, FinishingType, PlateSize,
    PricingOrder, InternalContent, OrderFinishing, ExtraExpense, CtpPlates, OrderComment,
    VATSetting
)
...
from .forms import (
    PricingOrderForm, InternalContentForm, OrderFinishingForm, 
    ExtraExpenseForm, OrderCommentForm, CtpPlatesForm, PricingOrderApproveForm
)
...
class PricingOrderListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        headers = [
            {'key': 'order_number', 'label': 'رقم الطلب', 'sortable': True, 'class': 'text-center'},
            {'key': 'created_at', 'label': 'تاريخ الإنشاء', 'sortable': True, 'class': 'text-center', 'format': 'datetime_12h'},
            {'key': 'client', 'label': 'العميل', 'template': 'pricing/columns/client_column.html'},
            {'key': 'title', 'label': 'العنوان', 'sortable': True, 'class': 'text-start'},
            {'key': 'order_type', 'label': 'نوع الطباعة', 'template': 'pricing/columns/order_type_column.html'},
            {'key': 'quantity', 'label': 'الكمية', 'sortable': True, 'class': 'text-center'},
            {'key': 'sale_price', 'label': 'سعر البيع', 'sortable': True, 'class': 'text-center', 'format': 'currency', 'decimals': 2},
            {'key': 'status', 'label': 'الحالة', 'template': 'pricing/columns/status_column.html'},
            {'key': 'created_by.get_full_name', 'label': 'المنشئ', 'sortable': True, 'class': 'text-center'},
        ]
        ...
        context['breadcrumb_items'] = [
            ...
            {'title': 'التسعير', 'url': reverse('pricing:pricing_dashboard'), 'icon': 'fas fa-calculator'},
            {'title': 'قائمة الطلبات', 'url': '', 'icon': 'fas fa-list', 'active': True},
        ]
        
        # تعريف أزرار الإجراءات للجدول
        order_actions = [
            {'url': 'pricing:pricing_detail', 'icon': 'fa-eye', 'label': 'عرض', 'class': 'action-view'},
            {'url': 'pricing:pricing_edit', 'icon': 'fa-edit', 'label': 'تعديل', 'class': 'action-edit', 'condition': 'user.is_staff or user == item.created_by'},
            {'url': 'pricing:pricing_approve', 'icon': 'fa-check', 'label': 'اعتماد', 'class': 'action-approve', 'condition': 'user.is_staff and item.status == "pending"'},
        ]
...
class PricingOrderCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_initial(self):
        ...
        try:
            ...
            if hasattr(PrintDirection, 'objects') and PrintDirection.objects:
                default_print_direction = PrintDirection.objects.filter(is_default=True).first()
        ...
        if 'pricing_form_data' in self.request.session:
            ...
            for key, value in session_data.items():
                ...
                if key in ['quantity', 'colors_front', 'colors_back', 'zinc_plates_count', 'internal_page_count', 'ctp_plates_count']:
                    try:
                        if value and str(value).strip():
                            initial[key] = int(value)
                    except (ValueError, AttributeError):
                        pass
                elif key in ['custom_size_width', 'custom_size_height', 'open_size_width', 'open_size_height', 
                           'paper_price', 'material_cost', 'printing_cost', 'finishing_cost', 
                           'extra_cost', 'profit_margin', 'sale_price', 'ctp_plate_price', 'ctp_transportation',
                           'design_price']:
                    try:
                        if value and str(value).strip():
                ...
                elif key in ['paper_type', 'paper_size', 'print_direction', 'print_sides', 'coating_type',
                           'supplier', 'press', 'order_type', 'ctp_supplier', 'ctp_plate_size']:
                    try:
                        if value and str(value).strip():
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest' and not request.POST.get('regular_submit'):
            ...
            return JsonResponse({'status': 'success', 'message': 'تم حفظ البيانات في الجلسة'})
    ...
    def form_valid(self, form):
        ...
        if ctp_supplier_id and ctp_plate_size_id and ctp_plates_count and ctp_plate_price:
            try:
                supplier = Supplier.objects.get(pk=ctp_supplier_id)
                plate_size = PlateSize.objects.get(pk=ctp_plate_size_id) if hasattr(PlateSize, 'objects') else None
                ...
                CtpPlates.objects.create(
                    ...
                    total_cost=(Decimal(ctp_plate_price) * int(ctp_plates_count)) + Decimal(ctp_transportation or '0.00'),
        ...
        additional_data = {
            ...
            'custom_size': {'width': str(custom_size_width), 'height': str(custom_size_height)} if custom_size_width and custom_size_height else None,
            'open_size': {'width': str(open_size_width), 'height': str(open_size_height)} if open_size_width and open_size_height else None,
    ...
    def create_finishing_services(self, finishing_services, order_object=None):
        ...
        for service_type, service_data in finishing_services.items():
            ...
            try:
                finishing_type = FinishingType.objects.get(name=finishing_types[service_type])
            ...
            try:
                ...
                if supplier_id:
                    try:
                        ...
                        print(f"تحذير: لم يتم العثور على مورد الخدمة لـ {service_type}: {e}")
                ...
                import traceback
                traceback.print_exc()
                messages.error(self.request, f"خطأ في إنشاء خدمة خدمات مابعد الطباعة {finishing_types[service_type]}: {str(e)}")
    ...
    def form_invalid(self, form):
        ...
        messages.error(self.request, "هناك أخطاء في النموذج. يرجى التحقق من البيانات المدخلة.")
    ...
    def get_context_data(self, **kwargs):
        ...
        context['plate_sizes'] = PlateSize.objects.all() if hasattr(PlateSize, 'objects') else []
        ...
        if 'pricing_form_data' in self.request.session:
            context['session_data'] = json.dumps(self.request.session['pricing_form_data'])
        
        # إضافة موردي خدمات الطباعة إلى السياق
        context['finishing_suppliers'] = Supplier.objects.filter(is_active=True)  # مؤقت - جميع الموردين
        ...
        context['breadcrumb_items'] = [
            {'title': 'الرئيسية', 'url': reverse('core:dashboard'), 'icon': 'fas fa-home'},
            {'title': 'التسعير', 'url': reverse('pricing:pricing_dashboard'), 'icon': 'fas fa-calculator'},
            {'title': 'قائمة الطلبات', 'url': reverse('pricing:pricing_order_list'), 'icon': 'fas fa-list'},
    ...
    def get_paper_type_weights_json(self):
        ...
        for pt in paper_types:
            try:
                ...
                base_name = pt.name.split()[0].strip() if hasattr(pt, 'name') and pt.name else f"نوع {pt.id}"
        ...
        for key, data in grouped_paper_types.items():
            pt_id = str(data['id'])
            paper_type_weights[pt_id] = sorted(data['weights'])  # ترتيب الأوزان تصاعديًا
...
class PricingOrderUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def test_func(self):
        ...
        return (
            ...
            self.request.user == order.created_by
        )
    ...
    def form_valid(self, form):
        ...
        if form.instance.description and form.instance.description.startswith('{'):
            ...
            try:
                ...
                if isinstance(existing_data, dict) and isinstance(additional_data, dict):
        ...
        if order.description and order.description.startswith('{'):
            try:
                ...
                if 'custom_size' in additional_data and additional_data['custom_size']:
                    initial['custom_size_width'] = additional_data['custom_size'].get('width')
                    initial['custom_size_height'] = additional_data['custom_size'].get('height')
                
                if 'open_size' in additional_data and additional_data['open_size']:
                    initial['open_size_width'] = additional_data['open_size'].get('width')
                    initial['open_size_height'] = additional_data['open_size'].get('height')
                ...
                if 'internal_page_count' in additional_data:
                    initial['internal_page_count'] = additional_data['internal_page_count']
...
def calculate_cost(request):
    ...
    if request.method == 'POST':
        try:
            ...
            quantity = int(quantity_str) if quantity_str and str(quantity_str).strip() else 0
            ...
            colors_front = int(colors_front_str) if colors_front_str and str(colors_front_str).strip() else 0
            
            colors_back_str = data.get('colors_back', '0')
            colors_back = int(colors_back_str) if colors_back_str and str(colors_back_str).strip() else 0
            ...
            if paper_type_id and quantity:
                try:
                    ...
                    print(f"تكلفة الورق: {material_cost} (الكمية: {quantity}, السعر: {paper_price}, الجرام: {paper_weight})")
            ...
            if supplier_id and quantity:
                try:
                    ...
                    total_colors = colors_front + (colors_back if print_sides == '2' else 0)
                    ...
                    # حساب تكلفة الطباعة
                    printing_cost = (quantity / 1000) * base_price_per_1000 * max(1, total_colors)
                    ...
                    print(f"تكلفة الطباعة: {printing_cost} (الكمية: {quantity}, الألوان: {total_colors}, السعر الأساسي: {base_price_per_1000})")
            ...
            print(f"المحتوى المستلم: {request.body[:100]}")  # طباعة جزء من المحتوى للتشخيص
            return JsonResponse({'success': False, 'error': 'خطأ في تحليل البيانات المرسلة'})
        except Exception as e:
...
def get_plate_price(request):
    ...
    if request.method == 'GET':
        try:
            ...
            plate_service = PlateServiceDetails.find_plate_service(supplier_id, plate_size_id)
...
def get_paper_weights(request):
    ...
    if request.method == 'GET':
        try:
            ...
            if not supplier_id:
                return JsonResponse({'success': False, 'error': 'Missing supplier_id parameter'})
            ...
            if sheet_type:
                paper_services_query = paper_services_query.filter(sheet_type=sheet_type)
            
            # استخراج الأوزان الفريدة
            paper_services = paper_services_query.exclude(gsm__isnull=True).values('gsm').distinct()
            ...
            weights = sorted(weights, key=lambda x: int(x['gsm']) if x['gsm'].isdigit() else 0)
            ...
            try:
                ...
                for ps in paper_services:
                    ...
                    if sheet_type and sheet_type not in unique_sheet_types:
                        unique_sheet_types.add(sheet_type)  # إضافة المقاس إلى المجموعة لمنع التكرار
                        # الحصول على اسم العرض من الثنائيات المعرفة في النموذج
                        display_name = dict(PaperServiceDetails.SHEET_TYPE_CHOICES).get(sheet_type, sheet_type)
                        sheet_types.append({
    ...
    if request.method == 'GET':
        try:
            ...
            try:
                ...
                query &= Q(service__paper_type__name__istartswith=base_name) | Q(service__paper_type__id=paper_type_id)
                
                # نستبعد القيم الفارغة لبلد المنشأ
                paper_services = PaperServiceDetails.objects.filter(query).exclude(
                    Q(country_of_origin='') | Q(country_of_origin__isnull=True)
                ).values('country_of_origin').distinct()
                ...
                if not origins:
                    ...
                    paper_services = PaperServiceDetails.objects.filter(flexible_query).exclude(
                        Q(country_of_origin='') | Q(country_of_origin__isnull=True)
                    ).values('country_of_origin').distinct()
...
def get_paper_price(request):
    ...
    print(f"معلومات طلب سعر الورق: نوع={paper_type_id}, جرام={gsm}, مورد={supplier_id}, مقاس={sheet_type}, بلد={country_of_origin}")
    ...
    try:
        ...
        if matching_paper_type:
            ...
            print(f"تم العثور على نوع ورق مطابق: {matching_paper_type.name}, السعر: {price}")
            ...
            if supplier_id:
                try:
                    ...
                    if matching_service:
                        ...
                        sheet_type_display = dict(PaperServiceDetails.SHEET_TYPE_CHOICES).get(sheet_type, sheet_type)
    ...
    if request.method == 'GET':
        try:
            ...
            try:
                ...
                print(f"تم العثور على الماكينة: {press.name}, السعر: {press.unit_price}")
                
                return JsonResponse({
                    'success': True,
                    'unit_price': float(press.unit_price),
                    'price': float(press.unit_price),
                    'price_per_1000': float(press.unit_price)  # للتوافق مع الكود القديم
                })
            ...
            try:
                ...
                try:
                    ...
                    details.update({
                        ...
                        'partial_price': float(offset_details.partial_price) if offset_details.partial_price else None,
                    })
                    ...
                    if hasattr(offset_details, 'max_width') and offset_details.max_width:
                        size_details['max_width'] = offset_details.max_width
                    if hasattr(offset_details, 'max_height') and offset_details.max_height:
    ...
    if request.method == 'GET':
        try:
            ...
            try:
                ...
                # طباعة معلومات الماكينة للتصحيح
                print(f"تم العثور على مقاس الماكينة: {press.name}, العرض: {width}, الارتفاع: {height}")
...
def get_paper_suppliers(request):
    ...
    if request.method == 'GET':
        try:
            ...
            suppliers = Supplier.objects.filter(
                is_active=True
            ).values('id', 'name')  # مؤقت - جميع الموردين
...
def pricing_dashboard(request):
    ...
    this_month_orders = PricingOrder.objects.filter(created_at__gte=current_month).count()
    
    # إحصائيات الإيرادات
    total_revenue = PricingOrder.objects.filter(
        status__in=['approved', 'executed']
    ).aggregate(Sum('sale_price'))['sale_price__sum'] or 0
    
    this_month_revenue = PricingOrder.objects.filter(
    ...
    try:
        ...
        ).filter(orders_count__gt=0).order_by('-total_value')[:5]
    except:
        top_clients = []
    
    # الطلبات الحديثة
    recent_orders = PricingOrder.objects.select_related('client', 'created_by').order_by('-created_at')[:10]
...
def quotation_list(request):
    ...
    quotations = PricingOrder.objects.filter(
        status__in=['approved', 'executed']
    ...
    # تعريف headers للجدول الموحد
    headers = [
        {'key': 'order_number', 'label': 'رقم العرض', 'sortable': True, 'class': 'text-center'},
        {'key': 'created_at', 'label': 'تاريخ الإنشاء', 'sortable': True, 'class': 'text-center', 'format': 'datetime_12h'},
        {'key': 'client', 'label': 'العميل', 'template': 'pricing/columns/client_column.html'},
        {'key': 'title', 'label': 'العنوان', 'sortable': True, 'class': 'text-start'},
        {'key': 'order_type', 'label': 'نوع الطباعة', 'template': 'pricing/columns/order_type_column.html'},
    ...
    context = {
        ...
        'breadcrumb_items': [
            {'title': 'الرئيسية', 'url': reverse('core:dashboard'), 'icon': 'fas fa-home'},
            {'title': 'التسعير', 'url': reverse('pricing:pricing_dashboard'), 'icon': 'fas fa-calculator'},
            {'title': 'عروض الأسعار', 'url': '', 'icon': 'fas fa-file-invoice-dollar', 'active': True},
        ],
...
def supplier_pricing_list(request):
    ...
    context = {
        ...
        'breadcrumb_items': [
            {'title': 'الرئيسية', 'url': reverse('core:dashboard'), 'icon': 'fas fa-home'},
            {'title': 'التسعير', 'url': reverse('pricing:pricing_dashboard'), 'icon': 'fas fa-calculator'},
            {'title': 'أسعار الموردين', 'url': '', 'icon': 'fas fa-building', 'active': True},
        ],
        'suppliers': suppliers,
        'paper_services': [],  # مؤقت
        'plate_services': []   # مؤقت
...
def price_comparison(request):
    ...
    context = {
        ...
        'breadcrumb_items': [
            {'title': 'الرئيسية', 'url': reverse('core:dashboard'), 'icon': 'fas fa-home'},
            {'title': 'التسعير', 'url': reverse('pricing:pricing_dashboard'), 'icon': 'fas fa-calculator'},
            {'title': 'مقارنة الأسعار', 'url': '', 'icon': 'fas fa-chart-line', 'active': True},
        ],
        'suppliers': suppliers,
...
def profitability_report(request):
    ...
    executed_orders = PricingOrder.objects.filter(status='executed').annotate(
        profit=F('sale_price') - F('total_cost')
    ).order_by('-created_at')
...
def pricing_analytics(request):
    ...
    orders_by_month = PricingOrder.objects.extra(
        select={'month': "strftime('%%Y-%%m', created_at)"}
    ).values('month').annotate(count=Count('id')).order_by('month')
    
    top_clients = PricingOrder.objects.values('client__name').annotate(
...
def settings_home(request):
    ...
    return render(request, 'pricing/settings/settings_home.html', {
        'active_menu': 'pricing',
        ...
        'breadcrumb_items': [
            {'title': 'الرئيسية', 'url': reverse('core:dashboard'), 'icon': 'fas fa-home'},
            {'title': 'التسعير', 'url': reverse('pricing:pricing_dashboard'), 'icon': 'fas fa-calculator'},
            {'title': 'الإعدادات', 'url': '', 'icon': 'fas fa-cogs', 'active': True},
        ],
    })
...
def print_direction_list(request):
    ...
    context = {
        ...
        'breadcrumb_items': [
            {'title': 'الرئيسية', 'url': reverse('core:dashboard'), 'icon': 'fas fa-home'},
            {'title': 'التسعير', 'url': reverse('pricing:pricing_dashboard'), 'icon': 'fas fa-calculator'},
            {'title': 'الإعدادات', 'url': reverse('pricing:settings_home'), 'icon': 'fas fa-cog'},
...
def print_side_list(request):
    ...
    return render(request, 'pricing/settings/print_side/list.html', {
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                ...
                return render(request, 'pricing/settings/coating_type/form_modal.html', {
                    'form': form,
...
def coating_type_delete(request, pk):
    ...
    return render(request, 'pricing/settings/coating_type/delete_modal.html', {
...
def finishing_type_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f'تم إنشاء نوع التشطيب "{finishing_type.name}" بنجاح')
    ...
    return render(request, 'pricing/settings/finishing_types/form_modal.html', {
        'form': form,
...
def finishing_type_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            finishing_type = form.save()
            messages.success(request, f'تم تحديث نوع التشطيب "{finishing_type.name}" بنجاح')
...
def finishing_type_delete(request, pk):
    ...
    return render(request, 'pricing/settings/finishing_types/delete_modal.html', {
        'finishing_type': finishing_type,
...
def offset_machine_type_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                messages.success(request, f'تم إنشاء نوع الماكينة "{machine_type.name}" بنجاح')
                ...
                if 'UNIQUE constraint failed' in str(e):
                    form.add_error('code', 'هذا الكود موجود بالفعل. يرجى استخدام كود مختلف.')
                else:
    ...
    return render(request, 'pricing/settings/offset_machine_type/form_modal.html', {
        'form': form,
        ...
        if form.is_valid():
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                ...
                return render(request, 'pricing/settings/offset_machine_type/form_modal.html', {
                    ...
                    'action_url': reverse('pricing:offset_machine_type_edit', kwargs={'pk': pk})
                })
...
def offset_machine_type_delete(request, pk):
    ...
    return render(request, 'pricing/settings/offset_machine_type/delete_modal.html', {
        'machine_type': machine_type,
        'action_url': reverse('pricing:offset_machine_type_delete', kwargs={'pk': pk})
    })
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                ...
                # في حالة عدم وجود AJAX، أعد عرض النموذج مع الأخطاء
                return render(request, 'pricing/settings/offset_sheet_size/form_modal.html', {
                    'form': form,
...
def offset_sheet_size_delete(request, pk):
    ...
    return render(request, 'pricing/settings/offset_sheet_size/delete_modal.html', {
        'sheet_size': sheet_size,
        'action_url': reverse('pricing:offset_sheet_size_delete', kwargs={'pk': pk})
    })
...
def get_plate_price(request):
    ...
    try:
        ...
        total_cost = plates_cost + float(plate_service.setup_cost) + float(plate_service.transportation_cost)
...
def get_press_price(request):
    ...
    try:
        ...
        try:
            ...
            return JsonResponse({
                'success': False,
                'error': 'لم يتم العثور على سعر للطباعة المحددة'
            })
...
def get_paper_sheet_types(request):
    ...
    try:
        ...
        return JsonResponse({
            'success': True,
            'data': [{'value': choice[0], 'label': choice[1]} for choice in sheet_types]
        })
...
def get_paper_origins(request):
    ...
    try:
        ...
        return JsonResponse({
            'success': True,
            'data': [{'value': choice[0], 'label': choice[1]} for choice in origins]
        })
...
def get_plate_sizes(request):
    ...
    try:
        ...
        if not supplier_id:
            ...
            sizes = PlateSize.objects.filter(is_active=True).values('id', 'name', 'width', 'height').order_by('name')
            return JsonResponse({
                'success': True,
                'plate_sizes': [{
                    'id': size['id'],
        ...
        for service in plate_services:
            if service.plate_size:
                ...
                size_display = dict(PlateServiceDetails.PLATE_SIZE_CHOICES).get(service.plate_size, service.plate_size)
...
def get_paper_suppliers(request):
    ...
    try:
        ...
        suppliers = query.select_related('service__supplier').values(
            'service__supplier__id', 'service__supplier__name'
        ).distinct().order_by('service__supplier__name')
...
def get_suppliers_by_service(request):
    ...
    try:
        ...
        suppliers = SpecializedService.objects.filter(
            category__code=service_type,
            is_active=True
            ...
            if order_type == 'digital':
                ...
                if plate_data:
                    ...
                    if plate_response.get('success'):
                        printing_cost += Decimal(str(plate_response['data']['total_cost']))
        
        # حساب تكلفة خدمات التشطيب
        finishing_services = data.get('finishing_services', [])
        for service in finishing_services:
            service_cost = Decimal(str(service.get('unit_price', 0))) * int(service.get('quantity', 1))
        ...
        return JsonResponse({
            ...
            'data': {
                ...
                'breakdown': {
                    ...
                    'price_per_unit': float(final_price / quantity) if quantity > 0 else 0,
                    'profit_amount': float(sale_price - total_cost),
                    'profit_percentage': float((sale_price - total_cost) / total_cost * 100) if total_cost > 0 else 0
                }
            }
        })
...
def coating_services_api(request):
    ...
    try:
        ...
        services = query.values(
            'id', 'name', 'description', 'unit_price', 'minimum_quantity',
            'supplier__id', 'supplier__name'
    ...
    try:
        ...
        if not supplier_id:
            ...
            for machine in offset_machines:
                presses.append({
                    ...
                    'name': f'{machine.manufacturer} {machine.name}' if machine.manufacturer else machine.name,
                    'type': 'offset',
        ...
        if not order_type or order_type == 'offset':
            ...
            for service in offset_services:
                if service.machine_type:
                    ...
                    machine_name = service.service.name if service.service.name else dict(OffsetPrintingDetails.MACHINE_TYPE_CHOICES).get(service.machine_type, service.machine_type)
                    
                    presses.append({
                        'id': f'offset_service_{service.id}',
                        'name': machine_name,
                        'type': 'offset',
                        'price_per_1000': float(service.price_per_1000) if hasattr(service, 'price_per_1000') and service.price_per_1000 else 100.0
                    })
        ...
        if not order_type or order_type == 'digital':
            ...
            for service in digital_services:
                if service.machine_type:
                    # استخدام اسم الخدمة بدلاً من نوع الماكينة
                    machine_name = service.service.name if service.service.name else dict(DigitalPrintingDetails.MACHINE_TYPE_CHOICES).get(service.machine_type, service.machine_type)
                    
                    presses.append({
                        'id': f'digital_service_{service.id}',
                        'name': machine_name,
                        'type': 'digital',
                        'price_per_1000': float(service.price_per_sheet) if hasattr(service, 'price_per_sheet') and service.price_per_sheet else 150.0
                    })
...
def get_press_details(request):
    ...
    try:
        ...
        if not press_id:
            return JsonResponse({
                'success': False,
                'error': 'معرف المطبعة مطلوب'
            })
        
        # بيانات مؤقتة - يمكن ربطها بنظام المطابع لاحقاً
        press_details = {
            '1': {'name': 'مطبعة أوفست كبيرة', 'type': 'offset', 'colors': 4, 'speed': 10000},
            '2': {'name': 'مطبعة أوفست متوسطة', 'type': 'offset', 'colors': 2, 'speed': 8000},
            '3': {'name': 'مطبعة رقمية ملونة', 'type': 'digital', 'colors': 4, 'speed': 2000},
            '4': {'name': 'مطبعة رقمية أبيض وأسود', 'type': 'digital', 'colors': 1, 'speed': 3000},
        }
...
def calculate_cost_api(request):
    ...
    try:
        ...
        # حساب تكلفة الورق
        paper_cost = 0
        if data.get('paper_type') and data.get('paper_size') and data.get('weight') and data.get('quantity'):
...
from .models import OrderSupplier, PricingQuotation, PricingApprovalWorkflow, PricingApproval, PricingReport, PricingKPI
...
class OrderSupplierListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if order_id:
            return OrderSupplier.objects.filter(order_id=order_id).select_related('order', 'supplier')
        return OrderSupplier.objects.select_related('order', 'supplier').order_by('-created_at')
...
class OrderSupplierCreateView(LoginRequiredMixin, CreateView):
    ...
    fields = ['supplier', 'role', 'service_type', 'description', 'estimated_cost', 
              'quoted_price', 'contact_person', 'phone', 'email', 'notes']
    
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        order_id = self.kwargs.get('order_id')
        context['order'] = get_object_or_404(PricingOrder, id=order_id)
        context['order_suppliers'] = OrderSupplier.objects.filter(order_id=order_id).select_related('supplier')
        context['suppliers'] = Supplier.objects.filter(is_active=True)
        
        # حساب إحصائيات التكاليف
        order_suppliers = context['order_suppliers']
        context['total_estimated_cost'] = sum(os.estimated_cost for os in order_suppliers)
        context['total_quoted_price'] = sum(os.quoted_price for os in order_suppliers)
        context['price_difference'] = context['total_quoted_price'] - context['total_estimated_cost']
...
class PricingQuotationListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = PricingQuotation.objects.select_related('pricing_order', 'created_by').order_by('-created_at')
...
class PricingQuotationCreateView(LoginRequiredMixin, CreateView):
    ...
    fields = ['pricing_order', 'valid_until', 'follow_up_date', 'payment_terms', 
    ...
    def generate_quotation_number(self):
        ...
        last_quotation = PricingQuotation.objects.filter(
...
class PricingQuotationUpdateView(LoginRequiredMixin, UpdateView):
    ...
    model = PricingQuotation
    template_name = 'pricing/quotation_form.html'
    fields = ['valid_until', 'follow_up_date', 'status', 'payment_terms', 
              'delivery_terms', 'warranty_terms', 'special_conditions', 
              'sent_to_person', 'sent_via', 'discount_percentage', 
...
class PricingApprovalWorkflowCreateView(LoginRequiredMixin, CreateView):
    ...
    fields = ['name', 'description', 'min_amount', 'max_amount', 'primary_approver', 
              'secondary_approver', 'email_notifications', 'whatsapp_notifications', 
              'auto_approve_below_limit', 'require_both_approvers']
...
class PricingApprovalDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['can_approve'] = (
            self.object.approver == self.request.user and 
...
class PricingReportListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return PricingReport.objects.select_related('generated_by').order_by('-created_at')
...
class PricingKPIListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['pending_approvals'] = PricingApproval.objects.filter(status='pending').count()
...
def pricing_dashboard(request):
    ...
    context = {
        ...
        'active_workflows': PricingApprovalWorkflow.objects.filter(is_active=True).count(),
        
        # الطلبات الحديثة
        'recent_orders': PricingOrder.objects.select_related('client', 'created_by')[:5],
        
        # العروض الحديثة
        'recent_quotations': PricingQuotation.objects.select_related('pricing_order', 'created_by')[:5],
        ...
        'breadcrumb_items': [
            {'title': 'الرئيسية', 'url': reverse('core:dashboard'), 'icon': 'fas fa-home'},
            {'title': 'التسعير', 'url': reverse('pricing:pricing_dashboard'), 'icon': 'fas fa-calculator'},
            {'title': 'لوحة التحكم', 'url': '', 'icon': 'fas fa-tachometer-alt', 'active': True},
        ],
    }
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                ...
                # في حالة عدم وجود AJAX، أعد عرض النموذج مع الأخطاء
                return render(request, 'pricing/settings/digital_machine_type/form_modal.html', {
                    'form': form,
                    'title': 'إضافة نوع ماكينة جديد',
        ...
        if form.is_valid():
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                ...
                return render(request, 'pricing/settings/digital_machine_type/form_modal.html', {
                    'form': form,
                    'title': f'تعديل {machine_type.name}',
                    'action_url': reverse('pricing:digital_machine_type_edit', kwargs={'pk': pk})
                })
...
def digital_machine_type_delete(request, pk):
    ...
    return render(request, 'pricing/settings/digital_machine_type/delete_modal.html', {
        'machine_type': machine_type,
        'action_url': reverse('pricing:digital_machine_type_delete', kwargs={'pk': pk})
    })
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                ...
                # في حالة عدم وجود AJAX، أعد عرض النموذج مع الأخطاء
                return render(request, 'pricing/settings/digital_sheet_size/form_modal.html', {
                    'form': form,
                    'title': 'إضافة مقاس جديد',
...
def digital_sheet_size_delete(request, pk):
    ...
    return render(request, 'pricing/settings/digital_sheet_size/delete_modal.html', {
        'sheet_size': sheet_size,
        'action_url': reverse('pricing:digital_sheet_size_delete', kwargs={'pk': pk})
    })
...
def paper_weight_create(request):
    ...
    return render(request, 'pricing/settings/paper_weights/form_modal.html', {
        'form': form,
...
def paper_weight_delete(request, pk):
    ...
    return render(request, 'pricing/settings/paper_weights/delete_modal.html', {
        'weight': weight,
...
def paper_type_create(request):
    ...
    return render(request, 'pricing/settings/paper_types/form_modal.html', {
        'form': form,
...
def paper_type_delete(request, pk):
    ...
    return render(request, 'pricing/settings/paper_types/delete_modal.html', {
...
def paper_size_create(request):
    ...
    return render(request, 'pricing/settings/paper_sizes/form_modal.html', {
        'form': form,
...
def paper_size_delete(request, pk):
    ...
    return render(request, 'pricing/settings/paper_sizes/delete_modal.html', {
...
def paper_origin_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f'تم إنشاء منشأ الورق "{paper_origin.name}" بنجاح')
    ...
    return render(request, 'pricing/settings/paper_origins/form_modal.html', {
        'form': form,
...
def paper_origin_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            paper_origin = form.save()
            messages.success(request, f'تم تحديث منشأ الورق "{paper_origin.name}" بنجاح')
...
def paper_origin_delete(request, pk):
    ...
    return render(request, 'pricing/settings/paper_origins/delete_modal.html', {
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                ...
                # في حالة عدم وجود AJAX، أعد عرض النموذج مع الأخطاء
                return render(request, 'pricing/settings/product_types/form_modal.html', {
                    'form': form,
...
def product_type_delete(request, pk):
    ...
    return render(request, 'pricing/settings/product_types/delete_modal.html', {
        'product_type': product_type,
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                ...
                # في حالة عدم وجود AJAX، أعد عرض النموذج مع الأخطاء
                return render(request, 'pricing/settings/product_sizes/form_modal.html', {
                    'form': form,
...
def product_size_delete(request, pk):
    ...
    return render(request, 'pricing/settings/product_sizes/delete_modal.html', {
