...
def format_currency(amount, currency_symbol=None, decimal_places=2, show_symbol=True, smart=True):
    ...
    if smart and amount is not None:
        try:
            ...
            pass  # استخدم decimal_places الأصلي
...
def format_number(
    number, decimal_places=None, thousand_separator=",", decimal_separator="."
):
    """
    ...
    integer_part, decimal_part = (
        str(number).split(".") if "." in str(number) else (str(number), "0")
    )
...
def generate_report(
    model, report_type, start_date=None, end_date=None, filters=None, user=None
):
    """
