...
def validate_file_extension(value, allowed_extensions=None):
    ...
    if ext not in allowed_extensions:
        raise ValidationError(
            _(f'نوع الملف غير مسموح. الأنواع المسموحة: {", ".join(allowed_extensions)}')
        )
...
def validate_file_size(value, max_size=None):
    ...
    if value.size > max_size:
        ...
        raise ValidationError(
            _(f"حجم الملف كبير جدًا. يجب أن يكون أقل من {max_size_mb} ميجابايت")
        )
...
def validate_image_dimensions(
    ...
    try:
        ...
        if min_width and width < min_width:
            raise ValidationError(
                _(f"عرض الصورة صغير جدًا. يجب أن يكون على الأقل {min_width} بكسل")
            )

        # التحقق من الحد الأدنى للارتفاع
        if min_height and height < min_height:
            raise ValidationError(
                _(f"ارتفاع الصورة صغير جدًا. يجب أن يكون على الأقل {min_height} بكسل")
            )

        # التحقق من الحد الأقصى للعرض
        if max_width and width > max_width:
            raise ValidationError(
                _(f"عرض الصورة كبير جدًا. يجب أن يكون أقل من {max_width} بكسل")
            )

        # التحقق من الحد الأقصى للارتفاع
        if max_height and height > max_height:
            raise ValidationError(
                _(f"ارتفاع الصورة كبير جدًا. يجب أن يكون أقل من {max_height} بكسل")
            )
        ...
        if aspect_ratio:
            ...
            if abs(current_ratio - aspect_ratio) > allowed_ratio_diff:
                raise ValidationError(
                    _(
                        f"نسبة أبعاد الصورة غير مناسبة. النسبة المطلوبة هي {aspect_ratio}"
                    )
                )
