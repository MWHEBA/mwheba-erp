...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options["dry_run"]:
            ...
            self.stdout.write(
                self.style.WARNING("تحذير: سيتم حذف أنواع الحسابات غير الضرورية!")
            )
    ...
    def preview_deletion(self, essential_types):
        ...
        for acc_type in types_to_delete.order_by("code"):
            ...
            warning = (
                f" ⚠️  [{accounts_count} حساب مرتبط]" if accounts_count > 0 else ""
            )
        ...
        self.stdout.write(
            f"  - النسبة المتبقية: {(types_to_keep.count() / AccountType.objects.count() * 100):.1f}%"
        )
    ...
    def simplify_account_types(self, essential_types):
        ...
        try:
            with transaction.atomic():
                ...
                self.stdout.write(
                    f"  - تم التبسيط بنسبة: {(deleted_count / (deleted_count + remaining_count) * 100):.1f}%"
                )
            ...
            self.stdout.write(self.style.ERROR(f"❌ حدث خطأ أثناء التبسيط: {str(e)}"))
            raise
    ...
    def migrate_accounts(self, types_with_accounts, essential_types):
        ...
        for old_type, accounts_count in types_with_accounts:
            ...
            if not new_type_code:
                ...
                new_type_code = category_defaults.get(
                    old_type.category, "CURRENT_ASSET"
                )
