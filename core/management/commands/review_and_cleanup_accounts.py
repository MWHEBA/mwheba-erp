...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['dry_run']:
            ...
            self.stdout.write(
                self.style.WARNING(
                    'سيتم حذف الحسابات المكررة وغير المرغوبة!'
                )
            )
    ...
    def review_accounts(self):
        ...
        self.stdout.write(self.style.SUCCESS(f'📊 إجمالي الحسابات: {all_accounts.count()}\n'))
        ...
        for account in all_accounts:
            ...
            if account.code not in correct_codes:
                ...
                for correct_name, correct_code in correct_accounts_map.items():
                    if correct_name.lower() in name_lower or name_lower in correct_name.lower():
                        # وجدنا حساب قديم له بديل صحيح
                        try:
                            correct_account = ChartOfAccounts.objects.get(code=correct_code)
                            duplicates.append({
                                'old': account,
                                'new': correct_account
                            })
        ...
        if duplicates:
            ...
            for dup in duplicates:
                entries_count = JournalEntryLine.objects.filter(account=dup['old']).count()
        ...
        if unwanted_accounts:
            ...
            for account in unwanted_accounts:
                ...
                self.stdout.write(
                    f'  - [{account.code}] {account.name}{warning}'
                )
        else:
        ...
        self.stdout.write(f'  - حسابات نشطة: {all_accounts.filter(is_active=True).count()}')
    ...
    def cleanup_accounts(self):
        ...
        try:
            with transaction.atomic():
                ...
                if duplicates:
                    ...
                    for dup in duplicates:
                        ...
                        sale_payments = SalePayment.objects.filter(financial_account=old_account)
                        ...
                        purchase_payments = PurchasePayment.objects.filter(financial_account=old_account)
                        ...
                        # نقل الحركات النقدية
                        cash_movements = CashMovement.objects.filter(account=old_account)
                        ...
                        if old_account.opening_balance and old_account.opening_balance != 0:
                            # إضافة الرصيد القديم للرصيد الجديد
                            new_account.opening_balance = (new_account.opening_balance or 0) + old_account.opening_balance
                            new_account.save()
                            self.stdout.write(
                                self.style.SUCCESS(
                ...
                self.stdout.write(f'  - حسابات تم حذفها: {deleted_count + len(duplicates)}')
            ...
            self.stdout.write(
                self.style.ERROR(f'❌ حدث خطأ أثناء التنظيف: {str(e)}')
            )
            raise
