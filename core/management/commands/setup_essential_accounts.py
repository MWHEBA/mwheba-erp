        ...
        try:
            with transaction.atomic():
                ...
                for account_data in accounts:
                    ...
                    if ChartOfAccounts.objects.filter(code=account_data['code']).exists():
                        self.stdout.write(f'  ⏭️  [{account_data["code"]}] {account_data["name"]} - موجود بالفعل')
                        skipped_count += 1
                        continue
                    
                    try:
                        # الحصول على نوع الحساب
                        account_type = AccountType.objects.get(code=account_data['type_code'])
            ...
            self.stdout.write(
                self.style.ERROR(f'❌ حدث خطأ أثناء الإضافة: {str(e)}')
            )
            raise
