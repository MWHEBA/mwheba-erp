...
class Command(BaseCommand):
    ...
    def show_current_accounts(self):
        ...
        for account in accounts:
            ...
            balance = (
                f"{account.opening_balance:,.2f}" if account.opening_balance else "0.00"
            )
    ...
    def suggest_accounts(self):
        ...
        for type_code, accounts in suggestions.items():
            try:
                ...
                for suggestion in accounts:
                    ...
                    exists = ChartOfAccounts.objects.filter(
                        code=suggestion["code"]
                    ).exists()

                    priority_color = {"عالية": "🔴", "متوسطة": "🟡", "منخفضة": "🟢"}.get(
                        suggestion["priority"], "⚪"
                    )

                    if exists:
                        self.stdout.write(
                            f'  ✅ [{suggestion["code"]}] {suggestion["name"]} - موجود'
                        )
                    else:
                        self.stdout.write(
        ...
        self.stdout.write(
            "4. حدد الحسابات النهائية (is_leaf=True) للحسابات التي ستستخدم في القيود"
        )
