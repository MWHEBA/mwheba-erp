...
from product.models import Product, ProductImage, ProductVariant, Stock, StockMovement, SerialNumber
from sale.models import Sale, SaleItem, SalePayment, SaleReturn, SaleReturnItem
from purchase.models import Purchase, PurchaseItem, PurchasePayment, PurchaseReturn, PurchaseReturnItem, PurchaseOrder, PurchaseOrderItem
...
try:
    ...
    BalanceSnapshot = AccountBalanceCache = BalanceAuditLog = BalanceReconciliation = DummyModel
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['dry_run']:
            ...
            self.stdout.write(
                self.style.WARNING(
                    'تحذير: سيتم حذف جميع البيانات التشغيلية نهائياً!'
                )
            )
