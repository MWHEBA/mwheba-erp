    ...
    def show_current_balances(self):
        ...
        for account in accounts:
            ...
            if current_balance != 0:
                icon = '💰' if account.is_cash_account else '🏦' if account.is_bank_account else '📊'
                self.stdout.write(
        ...
        self.stdout.write('  - الرصيد الافتتاحي = 0 (يُستخدم فقط للرصيد قبل بداية النظام)')
        self.stdout.write('  - لتصفير الأرصدة الافتتاحية: python manage.py recalculate_balances --reset')
