...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['dry_run']:
            ...
            self.stdout.write(
                self.style.WARNING(
                    f'سيتم {action} الحسابات اليتيمة!'
                )
            )
    ...
    def preview_cleanup(self):
        ...
        active_types = set(AccountType.objects.filter(is_active=True).values_list('id', flat=True))
        ...
        if orphan_accounts:
            ...
            for account in orphan_accounts:
                try:
                    ...
                    type_name = 'نوع محذوف'
                self.stdout.write(
                    f'  [{account.code}] {account.name} ← {type_name}'
                )
        ...
        if accounts_with_entries:
            ...
            for account, count in accounts_with_entries:
                self.stdout.write(
                    f'  [{account.code}] {account.name} - {count} قيد'
                )
            self.stdout.write(
        ...
        self.stdout.write(f'  - حسابات سليمة: {all_accounts.count() - len(all_problem_accounts)}')
        ...
        try:
            with transaction.atomic():
                ...
                if not problem_accounts:
                    self.stdout.write(
                        self.style.SUCCESS('✅ لا توجد حسابات يتيمة!')
                    )
                    return
            ...
            self.stdout.write(
                self.style.ERROR(f'❌ حدث خطأ أثناء {action}: {str(e)}')
            )
            raise
