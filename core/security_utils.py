...
def safe_error_response(error: Exception, user_message: str = "حدث خطأ غير متوقع") -> JsonResponse:
    """
...
def safe_html_clean(html_content: str) -> str:
    ...
    clean_content = re.sub(r'<(object|embed)\b[^>]*>.*?</\1>', '', clean_content, flags=re.DOTALL | re.IGNORECASE)
    
    # إزالة javascript: URLs
    clean_content = re.sub(r'javascript:[^"\']*', '', clean_content, flags=re.IGNORECASE)
...
def validate_input(data: Any, field_name: str, max_length: int = None) -> bool:
    ...
    if isinstance(data, str):
        if max_length and len(data) > max_length:
            logger.warning(f"Input too long for field {field_name}: {len(data)} > {max_length}")
            return False
        ...
        for pattern in dangerous_patterns:
            if re.search(pattern, data, re.IGNORECASE):
                logger.warning(f"Dangerous pattern found in field {field_name}: {pattern}")
                return False
...
class SecureExceptionHandler:
    ...
    def handle_view_exception(view_name: str, error: Exception) -> JsonResponse:
        ...
        logger.error(f"Error in view {view_name}: {error_type} - {str(error)}", exc_info=True)
