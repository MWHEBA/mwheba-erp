...
class FileExtensionValidatorTest(TestCase):
    ...
    def test_valid_file_extensions(self):
        ...
        doc_file = SimpleUploadedFile(
            "test.doc", b"file content", content_type="application/msword"
        )
        ...
        png_file = SimpleUploadedFile(
            "test.png", b"file content", content_type="image/png"
        )
    ...
    def test_invalid_file_extensions(self):
        ...
        exe_file = SimpleUploadedFile(
            "test.exe", b"file content", content_type="application/x-msdownload"
        )
...
class ImageDimensionsValidatorTest(TestCase):
    ...
    def create_test_image(self, width, height, filename):
        ...
        setattr(
            self,
            filename.replace(".", "_"),
            SimpleUploadedFile(filename, image_io.read(), content_type="image/png"),
        )
    ...
    def test_valid_image_dimensions(self):
        ...
        try:
            ...
            validate_image_dimensions(
                self.small_square_png, min_width=50, min_height=50
            )

            # صورة متوسطة تلبي حدود النسبة
            validate_image_dimensions(
                self.medium_rectangle_png, min_width=800, min_height=600
            )

            # صورة كبيرة أقل من الحد الأقصى
            validate_image_dimensions(
                self.large_rectangle_png, max_width=2000, max_height=1500
            )
        except ValidationError:
    ...
    def test_invalid_image_dimensions(self):
        ...
        with self.assertRaises(ValidationError):
            validate_image_dimensions(
                self.small_square_png, min_width=200, min_height=200
            )

        # صورة أكبر من الحد الأقصى
        with self.assertRaises(ValidationError):
            validate_image_dimensions(
                self.large_rectangle_png, max_width=1000, max_height=600
            )
    ...
    def test_corrupt_image_file(self):
        ...
        corrupt_image = SimpleUploadedFile(
            "corrupt.png", b"Corrupt image data", content_type="image/png"
        )
