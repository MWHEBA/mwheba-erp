...
class MiddlewareTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user_profile = UserProfile.objects.create(
            user=self.user, phone="01234567890", timezone="Africa/Cairo"
        )
    ...
    def test_ajax_redirect_middleware(self):
        ...
        response = self.client.get(
            reverse("dashboard"), HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
    ...
    def test_session_cookie_settings(self):
        ...
        if session_cookie:
            ...
            if not settings.DEBUG:
                self.assertEqual(
                    session_cookie["secure"], settings.SESSION_COOKIE_SECURE
                )
...
class MaintenanceModeMiddlewareTest(TestCase):
    ...
    def test_maintenance_mode_enabled_regular_user(self):
        ...
        self.assertEqual(response.status_code, 503)
        self.assertEqual(
            response.content.decode(), "النظام في وضع الصيانة. يرجى المحاولة لاحقًا."
        )
...
class TimezoneMiddlewareTest(TestCase):
    ...
    def test_default_timezone(self):
        ...
        request.user = User.objects.create_user(
            username="defaultuser", email="default@example.com", password="default123"
        )
        request.session = {}
...
class AuditLogMiddlewareTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="audituser", email="audit@example.com", password="audit123"
        )
    ...
    def test_audit_log_creation(self):
        ...
        request.user = self.user
        request.META = {"REMOTE_ADDR": "127.0.0.1", "HTTP_USER_AGENT": "Mozilla/5.0"}
        ...
        log = AuditLog.objects.filter(
            user=self.user, url="/admin/users/", method="GET", ip_address="127.0.0.1"
        ).first()
    ...
    def test_audit_log_for_anonymous_user(self):
        ...
        # التحقق من إنشاء سجل المراقبة
        log = AuditLog.objects.filter(
            user__isnull=True, url="/login/", method="GET", ip_address="127.0.0.1"
        ).first()
...
class PerformanceMonitoringMiddlewareTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_superuser(
            username="perfuser", email="perf@example.com", password="perf123"
        )
