    ...
    try:
        if isinstance(value, str):
            for fmt in ('%Y-%m-%d', '%Y-%m-%d %H:%M:%S', '%d/%m/%Y'):
                try:
                    value = datetime.strptime(value.split('T')[0], fmt).date()
                    break
                except ValueError:
                    pass
        ...
        if isinstance(value, dt.date):
            today = timezone.now().date() if hasattr(timezone, 'now') else dt.date.today()
...
def format_age(age_months):
    """
    تنسيق العمر بالشهور إلى سنوات وشهور
    Usage: {{ age_months|format_age }}
    """
    try:
        ...
        if age_months < 12:
            if age_months == 1:
                ...
                return f"{age_months} أشهر"
            else:
                return f"{age_months} شهر"
        ...
        if years == 1:
            ...
            years_text = f"{years} سنوات"
        else:
            years_text = f"{years} سنة"
        
        # إذا لا توجد شهور إضافية
        if months == 0:
            return years_text
        ...
        if months == 1:
            ...
            months_text = f"{months} شهر"
        
        return f"{years_text} و{months_text}"
    except (ValueError, TypeError):
        return str(age_months)
...
def phone_to_whatsapp(phone):
    ...
    # إضافة رمز مصر إذا لم يكن موجود
...
def format_code_badge(code):
    ...
    Usage: {{ code|format_code_badge }}
    """
    if not code:
        return mark_safe('<span class="text-muted">-</span>')
    
    return mark_safe(f'<code class="bg-light px-2 py-1 rounded">{code}</code>')


# تم حذف فلتر smart_float من هنا - استخدم الفلتر الموحد من utils_extras
# {% load utils_extras %} واستخدم {{ value|smart_float }}
# هذا الفلتر كان بيحول القيمة لـ float فقط، مش بيعمل تنسيق
...
def remove_prefix(value, prefix):
    """
    إزالة بادئة من النص
    Usage: {{ text|remove_prefix:"حساسية:" }}
    """
