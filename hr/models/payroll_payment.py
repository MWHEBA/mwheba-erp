...
class PayrollPayment(models.Model):
    ...
    def clean(self):
        ...
        # النظام الجديد: payment_method هو account code
        # التحقق من وجود bank_reference للحسابات البنكية
        if self.payment_method and not self.bank_reference:
            try:
                from financial.models import ChartOfAccounts
                account = ChartOfAccounts.objects.filter(code=self.payment_method).first()
                if account and account.is_bank_account:
                    errors['bank_reference'] = 'مرجع البنك مطلوب للحسابات البنكية'
            except Exception:
                pass
