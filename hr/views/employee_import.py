...
def export_employees(employees, format_type):
    ...
    if format_type == 'csv':
        ...
        for row_num, emp in enumerate(employees, 2):
            ...
            ws.cell(row=row_num, column=2, value=emp.name or '')
            ws.cell(row=row_num, column=3, value=emp.national_id or '')
            ws.cell(row=row_num, column=4, value=emp.birth_date.strftime('%Y-%m-%d') if emp.birth_date else '')
            ws.cell(row=row_num, column=5, value=gender_map.get(emp.gender, emp.gender) if emp.gender else '')
            ws.cell(row=row_num, column=6, value=marital_map.get(emp.marital_status, emp.marital_status) if emp.marital_status else '')
            ws.cell(row=row_num, column=7, value=emp.department.name_ar if emp.department else '')
            ws.cell(row=row_num, column=8, value=emp.job_title.title_ar if emp.job_title else '')
            ws.cell(row=row_num, column=9, value=emp.hire_date.strftime('%Y-%m-%d') if emp.hire_date else '')
            ws.cell(row=row_num, column=10, value=status_map.get(emp.status, emp.status))
            ws.cell(row=row_num, column=11, value=emp.work_email or '')
            ws.cell(row=row_num, column=12, value=emp.mobile_phone or '')
            ws.cell(row=row_num, column=13, value=emp.address or '')
            ws.cell(row=row_num, column=14, value=emp.city or '')
...
def import_employees_from_csv(csv_file, user):
    ...
    try:
        ...
        for row_num, row in enumerate(reader, start=2):  # start=2 لأن الصف 1 هو الهيدر
            ...
            full_name = row.get('الاسم الكامل', '').strip()
            if not full_name:
                skipped_count += 1
                error_details.append(f"الصف {row_num}: الاسم الكامل مفقود")
                continue
            ...
            try:
                ...
                employee.name = row.get('الاسم الكامل', '').strip()
                ...
                # إنشاء موظف جديد - التحقق من الحقول الضرورية فقط
                national_id = row.get('الرقم القومي', '').strip()
                work_email = row.get('البريد الإلكتروني', '').strip() or None
                mobile_phone = row.get('رقم الهاتف', '').strip() or None
                address = row.get('العنوان', '').strip() or None
                city = row.get('المدينة', '').strip() or None
                
                # التحقق من الحقول الضرورية فقط
                if not all([national_id, birth_date, gender, department, job_title]):
                    # تخطي إذا كانت أي من الحقول الضرورية ناقصة
                    skipped_count += 1
                    missing_fields = []
                ...
                employee = Employee.objects.create(
                    ...
                    name=row.get('الاسم الكامل', '').strip(),
...
def import_employees_from_excel(excel_file, user):
    ...
    try:
        ...
        for row_num in range(2, ws.max_row + 1):
            ...
            # التحقق من الحقول المطلوبة
            full_name = row_data.get('الاسم الكامل', '').strip()
            if not full_name:
                skipped_count += 1
                error_details.append(f"الصف {row_num}: الاسم الكامل مفقود")
                continue
            ...
            try:
                employee = Employee.objects.get(employee_number=employee_number)
                # تحديث الموظف الموجود
                employee.name = row_data.get('الاسم الكامل', '').strip()
                employee.status = status
                ...
                work_email = row_data.get('البريد الإلكتروني', '').strip() or None
                mobile_phone = row_data.get('رقم الهاتف', '').strip() or None
                address = row_data.get('العنوان', '').strip() or None
                city = row_data.get('المدينة', '').strip() or None
                ...
                employee = Employee.objects.create(
                    employee_number=employee_number,
                    name=row_data.get('الاسم الكامل', '').strip(),
                    national_id=national_id,
                    birth_date=birth_date,
                    gender=gender,
                    marital_status=marital_status or 'single',
