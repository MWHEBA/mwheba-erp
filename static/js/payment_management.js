...
class PaymentManager {
    ...
    async editPayment(paymentId) {
        try {
            ...
            // جلب نموذج التعديل مع إعادة المحاولة
            const response = await this.fetchWithRetry(`/financial/payments/${paymentId}/edit/`, {
                method: 'GET',
                headers: {
    ...
    handleEditError(error) {
        const content = document.getElementById('editPaymentContent');
        let errorMessage = 'خطأ في تحميل نموذج التعديل';
        ...
        if (error.message.includes('لا يوجد اتصال')) {
            ...
            errorMessage = 'ليس لديك صلاحية لتعديل هذه الدفعة';
        }
    ...
    handleSaveError(error) {
        let errorMessage = 'خطأ في حفظ التغييرات';
        
        if (error.message.includes('لا يوجد اتصال')) {
            errorMessage = 'لا يوجد اتصال بالإنترنت';
        } else if (error.message.includes('لم يتم العثور على النموذج')) {
            errorMessage = 'خطأ في النموذج';
        }
