...
// إعدادات Toastr الافتراضية - ثابتة ومركزية
...
// تطبيق الإعدادات فوراً إذا كان toastr متاح
...
window.showNotification = function(message, type = 'info', title = null, customOptions = {}) {
    ...
    const config = {
        ...
        ...typeConfig[type],
        ...customOptions,
        positionClass: "toast-top-left" // فرض الموضع دائماً
    };

    const toastrType = window.mapToToastrType(type);
    
    // تطبيق الإعدادات مباشرة بدون حفظ/استعادة
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // الآن نحول فقط رسائل Django Messages
});
...
/**
 * حماية إضافية: مراقبة أي تغيير في container class
 */
if (typeof MutationObserver !== 'undefined') {
    const observer = new MutationObserver(function(mutations) {
        mutations.forEach(function(mutation) {
            if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
                ...
                if (container && !container.classList.contains('toast-top-left')) {
                    // إعادة تطبيق الموضع الصحيح
                    container.className = 'toast-top-left';
                }
            }
        });
    });
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (container) {
            ...
            // إذا لم يكن موجود، راقب body لحين إنشائه
