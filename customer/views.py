...
def customer_list(request):
    ...
    used_customer_types = Customer.objects.exclude(customer_type__isnull=True).exclude(customer_type='').values_list('customer_type', flat=True).distinct()
    customer_types = [choice for choice in Customer.CUSTOMER_TYPES if choice[0] in used_customer_types]
    ...
    if is_archive_view:
        header_buttons.append({
            "url": reverse("customer:customer_list"),
            "icon": "fa-users",
    ...
    if daftra_enabled:
        header_buttons.append({
            "onclick": "syncWithDaftra('customers')",
            "icon": "fa-sync",
            "text": "مزامنة دفترة",
    ...
    context = {
        **pagination_data,
        ...
        'breadcrumb_items': [
            ...
            {"title": "العملاء", "url": reverse("customer:customer_list") if is_archive_view else None, "active": not is_archive_view},
    ...
    return render_paginated_response(
        ...
        context,
        table_template_name="customer/partials/customer_table.html"
    )
        ...
        try:
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return JsonResponse({
                    ...
                    'action': res['action'],
                    'message': res['message'],
                    'redirect_url': reverse("customer:customer_list"),
                })
...
def customer_detail(request, pk):
    ...
    quick_action_buttons = [
        ...
        {
            "url": reverse("customer:customer_edit", kwargs={"pk": customer.pk}),
            "icon": "fas fa-edit",
...
def customer_change_account(request, pk):
    ...
    return render(request, "customer/customer_change_account.html", context)
...
def customer_create_account(request, pk):
    ...
    if customer.financial_account:
        ...
        return redirect("customer:customer_change_account", pk=customer.pk)
    ...
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        html = render_to_string("customer/customer_create_account_modal.html", {
            'customer': customer
        }, request=request)
