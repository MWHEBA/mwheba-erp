...
class SupplierForm(forms.ModelForm):
    ...
    class Meta:
        model = Supplier
        fields = [
            'name', 'code', 'phone', 'secondary_phone', 'email', 'whatsapp', 'website',
            'contact_person', 'address', 'city', 'country', 'tax_number',
            'delivery_time', 'min_order_amount', 'payment_terms', 'working_hours', 'is_preferred',
            'pricing_notes', 'discount_policy', 'is_active'
        ]
        widgets = {
            ...
            'phone': forms.TextInput(attrs={'class': 'form-control', 'dir': 'ltr', 'placeholder': '+20123456789'}),
            'secondary_phone': forms.TextInput(attrs={'class': 'form-control', 'dir': 'ltr', 'placeholder': '+20123456789'}),
            'email': forms.EmailInput(attrs={'class': 'form-control', 'dir': 'ltr', 'placeholder': 'supplier@example.com'}),
            'whatsapp': forms.TextInput(attrs={'class': 'form-control', 'dir': 'ltr', 'placeholder': '+20123456789'}),
            'website': forms.URLInput(attrs={'class': 'form-control', 'dir': 'ltr', 'placeholder': 'https://example.com'}),
            'contact_person': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'اسم الشخص المسؤول'}),
            ...
            'delivery_time': forms.NumberInput(attrs={'class': 'form-control', 'min': '1', 'placeholder': 'عدد الأيام'}),
            'min_order_amount': forms.NumberInput(attrs={'class': 'form-control', 'step': '0.01', 'min': '0', 'placeholder': '0.00'}),
            'payment_terms': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'مثال: نقداً، آجل 30 يوم'}),
            'working_hours': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'مثال: من 9 صباحاً إلى 5 مساءً'}),
            'is_preferred': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'pricing_notes': forms.Textarea(attrs={'class': 'form-control', 'rows': 3, 'placeholder': 'ملاحظات خاصة بالتسعير'}),
            'discount_policy': forms.Textarea(attrs={'class': 'form-control', 'rows': 3, 'placeholder': 'تفاصيل سياسة الخصومات'}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['supplier_types'] = forms.ModelMultipleChoiceField(
            queryset=SupplierType.objects.filter(is_active=True).order_by('display_order'),
        ...
        if ChartOfAccounts:
            ...
            if suppliers_account:
                ...
                qualified_accounts = ChartOfAccounts.objects.filter(
                    models.Q(id=suppliers_account.id) |  # الحساب الرئيسي نفسه
                    models.Q(parent=suppliers_account) |  # الحسابات الفرعية المباشرة
                    models.Q(parent__parent=suppliers_account)  # الحسابات الفرعية من المستوى الثاني
    ...
    def clean_financial_account(self):
        ...
        if account:
            ...
            if suppliers_account and account:
                is_valid = (
                    account.id == suppliers_account.id or  # الحساب الرئيسي نفسه
                    account.parent == suppliers_account or  # فرعي مباشر
                    (account.parent and account.parent.parent == suppliers_account)  # فرعي من المستوى الثاني
                )
