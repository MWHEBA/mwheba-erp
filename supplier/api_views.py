...
from .models import (
    Supplier, SupplierType, SpecializedService, ServicePriceTier,
    ...
    try:
        ...
        if category_code == 'offset_printing':
            ...
            try:
                ...
                machine_types = OffsetMachineType.objects.filter(is_active=True).order_by('name')
                sheet_sizes = OffsetSheetSize.objects.filter(is_active=True).order_by('name')
                
                if machine_types.exists() and sheet_sizes.exists():
                    form_choices.update({
                        'machine_types': [(mt.code if mt.code else f'mt_{mt.id}', mt.name) for mt in machine_types],
                        'sheet_sizes': [(ss.code if ss.code else f'ss_{ss.id}', f"{ss.name} ({int(ss.width_cm)}×{int(ss.height_cm)} سم)") for ss in sheet_sizes]
                    })
                else:
            ...
            try:
                ...
                machine_types = DigitalMachineType.objects.filter(is_active=True).order_by('name')
                paper_sizes = DigitalSheetSize.objects.filter(is_active=True).order_by('name')
                
                if machine_types.exists() and paper_sizes.exists():
                    form_choices.update({
                        'machine_types': [(mt.code if mt.code else f'mt_{mt.id}', mt.name) for mt in machine_types],
                        'paper_sizes': [(ps.code if ps.code else f'ps_{ps.id}', f"{ps.name} ({int(ps.width_cm)}×{int(ps.height_cm)} سم)") for ps in paper_sizes]
                    })
                else:
    ...
    try:
        with transaction.atomic():
            ...
            if not supplier_id or not service_type:
                return JsonResponse({'error': 'بيانات المورد ونوع الخدمة مطلوبة'}, status=400)
            ...
            return JsonResponse({
                ...
                'redirect_url': reverse('supplier:supplier_detail', args=[supplier_id])
            })
    ...
    try:
        with transaction.atomic():
            ...
            return JsonResponse({
                ...
                'redirect_url': reverse('supplier:supplier_detail', args=[service.supplier.id])
            })
...
def get_field_mapping_api(request, service_type):
    ...
    try:
        ...
        return JsonResponse({
            'error': f'خطأ في جلب خريطة الحقول: {str(e)}'
        }, status=500)
    ...
    try:
        ...
        if category_code == 'digital_printing' and hasattr(service, 'digital_details'):
            ...
            service_data.update({
                ...
                'set_price': float(details.set_price) if details.set_price else None,
            })
