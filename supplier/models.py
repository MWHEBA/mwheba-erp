...
class SupplierType(models.Model):
    ...
    code = models.CharField(_('الرمز'), max_length=20, choices=TYPE_CHOICES, unique=True)
    slug = models.SlugField(_('الرابط'), max_length=100, unique=True, blank=True)
    description = models.TextField(_('وصف'), blank=True)
    icon = models.CharField(_('أيقونة'), max_length=50, blank=True, help_text=_('اسم الأيقونة من Font Awesome'))
    color = models.CharField(_('لون'), max_length=7, default='#007bff', help_text=_('لون بصيغة HEX'))
...
class SupplierServiceTag(models.Model):
    ...
    name = models.CharField(_('اسم الخدمة'), max_length=100, unique=True,
                          help_text=_('اسم الخدمة المقدمة من المورد'))
    category = models.CharField(_('فئة الخدمة'), max_length=20, choices=SERVICE_CATEGORIES,
                              help_text=_('تصنيف الخدمة حسب النوع'))
    description = models.TextField(_('وصف الخدمة'), blank=True,
                                 help_text=_('وصف تفصيلي للخدمة'))
    color_code = models.CharField(_('رمز اللون'), max_length=7, blank=True, default='#6c757d',
    ...
    display_order = models.PositiveSmallIntegerField(_('ترتيب العرض'), default=0,
                                                   help_text=_('ترتيب عرض الخدمة في القوائم'))
    is_active = models.BooleanField(_('نشط'), default=True,
                                  help_text=_('هل الخدمة متاحة للاستخدام'))
    ...
    estimated_duration = models.PositiveIntegerField(_('المدة المقدرة (أيام)'), default=1,
    ...
    def get_supplier_types_display(self):
        ...
        return ", ".join([supplier_type.name for supplier_type in self.supplier_types.all()])
...
class Supplier(models.Model):
    ...
    supplier_types = models.ManyToManyField(SupplierType, blank=True, related_name='suppliers', verbose_name=_('أنواع المورد'))
    primary_type = models.ForeignKey(SupplierType, on_delete=models.SET_NULL, null=True, blank=True, related_name='primary_suppliers', verbose_name=_('النوع الأساسي'))
    ...
    delivery_time = models.IntegerField(_('مدة التسليم'), default=7, help_text=_('مدة التسليم الافتراضية بالأيام'))
    min_order_amount = models.DecimalField(_('الحد الأدنى للطلب'), max_digits=10, decimal_places=2, default=0, help_text=_('الحد الأدنى لمبلغ الطلب'))
    
    # معلومات إضافية للتسعير
    pricing_notes = models.TextField(_('ملاحظات التسعير'), blank=True, help_text=_('ملاحظات خاصة بالتسعير والخدمات'))
    payment_terms = models.CharField(_('شروط الدفع'), max_length=100, blank=True, help_text=_('شروط الدفع المفضلة'))
    discount_policy = models.TextField(_('سياسة الخصومات'), blank=True, help_text=_('تفاصيل سياسة الخصومات للمورد'))
    ...
    # معلومات التشغيل
    working_hours = models.CharField(_('ساعات العمل'), max_length=100, blank=True, help_text=_('مثال: من 9 صباحاً إلى 5 مساءً'))
    is_preferred = models.BooleanField(_('مورد مفضل'), default=False, help_text=_('هل هذا مورد مفضل للشركة؟'))
    
    # تقييم المورد
    supplier_rating = models.DecimalField(_('تقييم المورد'), max_digits=3, decimal_places=1, default=0, help_text=_('تقييم من 0 إلى 5'))
    ...
    def actual_balance(self):
        ...
        total_purchase_payments = PurchasePayment.objects.filter(
            purchase__supplier=self
        ).aggregate(
            total=Sum('amount')
        )['total'] or 0
    ...
    def get_contact_methods(self):
        ...
        if self.secondary_phone:
            methods.append({'type': 'phone', 'value': self.secondary_phone, 'label': _('هاتف ثانوي')})
        if self.whatsapp:
            methods.append({'type': 'whatsapp', 'value': self.whatsapp, 'label': _('واتساب')})
        if self.email:
            methods.append({'type': 'email', 'value': self.email, 'label': _('بريد إلكتروني')})
        if self.website:
            methods.append({'type': 'website', 'value': self.website, 'label': _('موقع إلكتروني')})
        return methods
    ...
    def is_available_for_order(self):
        ...
        return (
            self.is_active and 
            self.get_specialized_services_count() > 0 and
            self.supplier_types.filter(is_active=True).exists()
        )
    ...
    def get_service_categories(self):
        ...
        try:
            ...
            SupplierServiceCategory = apps.get_model('supplier', 'SupplierServiceCategory')
...
class SpecializedService(models.Model):
    ...
    supplier = models.ForeignKey(Supplier, on_delete=models.CASCADE, related_name='specialized_services')
    category = models.ForeignKey(SupplierType, on_delete=models.PROTECT, verbose_name=_('فئة الخدمة'))
    name = models.CharField(_('اسم الخدمة'), max_length=255)
    description = models.TextField(_('وصف الخدمة'), blank=True)
    
    # معلومات السعر الأساسية
    setup_cost = models.DecimalField(_('تكلفة التجهيز'), max_digits=10, decimal_places=2, default=0)
    ...
    def get_price_for_quantity(self, quantity):
        ...
        first_tier = self.price_tiers.filter(is_active=True).order_by('min_quantity').first()
...
class PaperServiceDetails(models.Model):
    ...
    service = models.OneToOneField(SpecializedService, on_delete=models.CASCADE, related_name='paper_details')
    paper_type = models.CharField(_('نوع الورق'), max_length=20, choices=PAPER_TYPE_CHOICES)
    gsm = models.PositiveIntegerField(_('وزن الورق (جرام)'))
    sheet_size = models.CharField(_('مقاس الفرخ'), max_length=20, choices=SHEET_SIZE_CHOICES)
    
    # أبعاد مخصصة
    custom_width = models.DecimalField(_('العرض المخصص (سم)'), max_digits=8, decimal_places=2, null=True, blank=True)
    custom_height = models.DecimalField(_('الارتفاع المخصص (سم)'), max_digits=8, decimal_places=2, null=True, blank=True)
    ...
    price_per_sheet = models.DecimalField(_('سعر الفرخ'), max_digits=10, decimal_places=2)
...
class DigitalPrintingDetails(models.Model):
    ...
    service = models.OneToOneField(SpecializedService, on_delete=models.CASCADE, related_name='digital_details')
    machine_type = models.CharField(_('نوع الماكينة'), max_length=30, choices=MACHINE_TYPE_CHOICES)
    machine_model = models.CharField(_('موديل الماكينة'), max_length=100, blank=True)
    paper_handling = models.CharField(_('نوع التغذية'), max_length=20, choices=PAPER_HANDLING_CHOICES)
    paper_size = models.CharField(_('مقاس الورق'), max_length=20, choices=PAPER_SIZE_CHOICES)
    
    # أسعار الطباعة الأساسية (يمكن تجاوزها بالشرائح السعرية)
    price_per_page_bw = models.DecimalField(_('سعر الصفحة أبيض/أسود'), max_digits=10, decimal_places=2)
    price_per_page_color = models.DecimalField(_('سعر الصفحة ملونة'), max_digits=10, decimal_places=2)
    
    # استخدام الشرائح السعرية
    has_price_tiers = models.BooleanField(_('استخدام شرائح سعرية'), default=True, help_text=_('إذا كان نشطاً، سيتم استخدام الشرائح السعرية بدلاً من الأسعار الثابتة'))
    
    # إمكانيات الماكينة
    duplex_printing = models.BooleanField(_('طباعة على الوجهين'), default=True)
    variable_data_printing = models.BooleanField(_('طباعة بيانات متغيرة'), default=False)
    supports_heavy_paper = models.BooleanField(_('يدعم الورق السميك'), default=False)
    max_paper_weight_gsm = models.PositiveIntegerField(_('أقصى وزن ورق (جرام)'), default=300)
    
    # معلومات الإنتاج
    pages_per_minute_bw = models.PositiveIntegerField(_('صفحة/دقيقة أبيض وأسود'), null=True, blank=True)
    pages_per_minute_color = models.PositiveIntegerField(_('صفحة/دقيقة ملون'), null=True, blank=True)
    setup_time_minutes = models.PositiveIntegerField(_('وقت الإعداد (دقائق)'), default=5)
...
class FinishingServiceDetails(models.Model):
    ...
    service = models.OneToOneField(SpecializedService, on_delete=models.CASCADE, related_name='finishing_details')
    finishing_type = models.CharField(_('نوع الخدمة'), max_length=20, choices=FINISHING_TYPE_CHOICES)
    calculation_method = models.CharField(_('طريقة الحساب'), max_length=20, choices=CALCULATION_METHOD_CHOICES)
    
    # قيود المقاسات
    min_size_cm = models.DecimalField(_('أصغر مقاس (سم)'), max_digits=8, decimal_places=2, null=True, blank=True)
    max_size_cm = models.DecimalField(_('أكبر مقاس (سم)'), max_digits=8, decimal_places=2, null=True, blank=True)
    
    # الأسعار
    price_per_unit = models.DecimalField(_('سعر الوحدة'), max_digits=10, decimal_places=2)
    setup_time_minutes = models.PositiveIntegerField(_('وقت التجهيز (دقائق)'), default=0)
    
    # معلومات التسليم
    turnaround_time_hours = models.PositiveIntegerField(_('وقت التسليم (ساعات)'), default=24)
...
class ServicePriceTier(models.Model):
    ...
    service = models.ForeignKey(SpecializedService, on_delete=models.CASCADE, related_name='price_tiers')
    tier_name = models.CharField(_('اسم الشريحة'), max_length=50, help_text=_('مثال: 1-20، 21-50'))
    min_quantity = models.PositiveIntegerField(_('الحد الأدنى للكمية'))
    max_quantity = models.PositiveIntegerField(_('الحد الأقصى للكمية'), null=True, blank=True, help_text=_('اتركه فارغاً للكمية المفتوحة'))
    price_per_unit = models.DecimalField(_('سعر الوحدة'), max_digits=10, decimal_places=2)
    floor_price = models.DecimalField(_('السعر للأرضيات'), max_digits=10, decimal_places=2, null=True, blank=True, help_text=_('السعر الخاص للأرضيات (اختياري)'))
...
class OffsetPrintingDetails(models.Model):
    ...
    service = models.OneToOneField(SpecializedService, on_delete=models.CASCADE, related_name='offset_details')
    machine_type = models.CharField(_('نوع الماكينة'), max_length=30, choices=MACHINE_TYPE_CHOICES)
    sheet_size = models.CharField(_('مقاس الفرخ'), max_length=20, choices=SHEET_SIZE_CHOICES)
    
    # أبعاد مخصصة
    custom_width_cm = models.DecimalField(_('العرض المخصص (سم)'), max_digits=8, decimal_places=2, null=True, blank=True)
    custom_height_cm = models.DecimalField(_('الارتفاع المخصص (سم)'), max_digits=8, decimal_places=2, null=True, blank=True)
    
    # تكاليف الإعداد
    color_calibration_cost = models.DecimalField(_('تكلفة معايرة الألوان'), max_digits=10, decimal_places=2, default=0)
    
    # التسعير بالتراج
    impression_cost_per_1000 = models.DecimalField(_('تكلفة الفرخ لكل 1000'), max_digits=10, decimal_places=2)
    special_impression_cost = models.DecimalField(_('سعر التراج مخصوص'), max_digits=10, decimal_places=2, default=0, blank=True)
    break_impression_cost = models.DecimalField(_('سعر كسر التراج'), max_digits=10, decimal_places=2, default=0, blank=True)
...
class PlateServiceDetails(models.Model):
    ...
    service = models.OneToOneField(SpecializedService, on_delete=models.CASCADE, related_name='plate_details')
    plate_size = models.CharField(_('مقاس الزنك'), max_length=20, choices=PLATE_SIZE_CHOICES)
    ...
    # التسعير
    price_per_plate = models.DecimalField(_('سعر الزنك الواحد'), max_digits=10, decimal_places=2)
    set_price = models.DecimalField(_('سعر الطقم (4 زنكات)'), max_digits=10, decimal_places=2, null=True, blank=True)
...
class OutdoorPrintingDetails(models.Model):
    ...
    service = models.OneToOneField(SpecializedService, on_delete=models.CASCADE, related_name='outdoor_details')
    material_type = models.CharField(_('نوع المادة'), max_length=30, choices=MATERIAL_TYPE_CHOICES)
    printing_type = models.CharField(_('نوع الطباعة'), max_length=30, choices=PRINTING_TYPE_CHOICES)
    
    # أقصى مقاسات
    max_width_cm = models.DecimalField(_('أقصى عرض (سم)'), max_digits=8, decimal_places=2)
    max_height_cm = models.DecimalField(_('أقصى ارتفاع (سم)'), max_digits=8, decimal_places=2, null=True, blank=True)
    
    # التسعير
    price_per_sqm = models.DecimalField(_('سعر المتر المربع'), max_digits=10, decimal_places=2)
    min_order_sqm = models.DecimalField(_('الحد الأدنى (متر مربع)'), max_digits=8, decimal_places=2, default=1)
    
    # خدمات إضافية
    includes_hemming = models.BooleanField(_('يشمل الحياكة'), default=False)
    hemming_cost_per_meter = models.DecimalField(_('تكلفة الحياكة للمتر'), max_digits=10, decimal_places=2, default=0)
    includes_grommets = models.BooleanField(_('يشمل الحلقات'), default=False)
    grommet_cost_each = models.DecimalField(_('تكلفة الحلقة الواحدة'), max_digits=10, decimal_places=2, default=0)
    
    # معلومات التسليم
    production_time_days = models.PositiveIntegerField(_('وقت الإنتاج (أيام)'), default=3)
    includes_installation = models.BooleanField(_('يشمل التركيب'), default=False)
    installation_cost_per_sqm = models.DecimalField(_('تكلفة التركيب للمتر المربع'), max_digits=10, decimal_places=2, default=0)
...
class LaserServiceDetails(models.Model):
    ...
    service = models.OneToOneField(SpecializedService, on_delete=models.CASCADE, related_name='laser_details')
    laser_type = models.CharField(_('نوع الليزر'), max_length=20, choices=LASER_TYPE_CHOICES)
    service_type = models.CharField(_('نوع الخدمة'), max_length=30, choices=SERVICE_TYPE_CHOICES)
    material_type = models.CharField(_('نوع المادة'), max_length=20, choices=MATERIAL_TYPE_CHOICES)
    
    # قيود المقاسات
    max_width_cm = models.DecimalField(_('أقصى عرض (سم)'), max_digits=8, decimal_places=2)
    max_height_cm = models.DecimalField(_('أقصى ارتفاع (سم)'), max_digits=8, decimal_places=2)
    max_thickness_mm = models.DecimalField(_('أقصى سمك (مم)'), max_digits=6, decimal_places=2)
    
    # التسعير
    price_per_minute = models.DecimalField(_('سعر الدقيقة'), max_digits=10, decimal_places=2, null=True, blank=True)
    price_per_cm = models.DecimalField(_('سعر السنتيمتر'), max_digits=10, decimal_places=2, null=True, blank=True)
    price_per_piece = models.DecimalField(_('سعر القطعة'), max_digits=10, decimal_places=2, null=True, blank=True)
    setup_cost = models.DecimalField(_('تكلفة الإعداد'), max_digits=10, decimal_places=2, default=0)
    
    # معلومات إضافية
    min_order_value = models.DecimalField(_('الحد الأدنى للطلب'), max_digits=10, decimal_places=2, default=0)
    turnaround_time_hours = models.PositiveIntegerField(_('وقت التسليم (ساعات)'), default=24)
    includes_design = models.BooleanField(_('يشمل التصميم'), default=False)
    design_cost_per_hour = models.DecimalField(_('تكلفة التصميم للساعة'), max_digits=10, decimal_places=2, default=0)
...
class VIPGiftDetails(models.Model):
    ...
    service = models.OneToOneField(SpecializedService, on_delete=models.CASCADE, related_name='vip_gift_details')
    gift_category = models.CharField(_('فئة الهدية'), max_length=30, choices=GIFT_CATEGORY_CHOICES)
    customization_type = models.CharField(_('نوع التخصيص'), max_length=30, choices=CUSTOMIZATION_TYPE_CHOICES)
    ...
    customization_cost = models.DecimalField(_('تكلفة التخصيص'), max_digits=10, decimal_places=2, default=0)
    packaging_cost = models.DecimalField(_('تكلفة التغطية'), max_digits=10, decimal_places=2, default=0)
    
    # قيود الطلب
    min_order_quantity = models.PositiveIntegerField(_('الحد الأدنى للكمية'), default=1)
    max_customization_chars = models.PositiveIntegerField(_('أقصى عدد أحرف للتخصيص'), null=True, blank=True)
    
    # معلومات التسليم
    production_time_days = models.PositiveIntegerField(_('وقت الإنتاج (أيام)'), default=7)
    ...
    delivery_cost = models.DecimalField(_('تكلفة التوصيل'), max_digits=10, decimal_places=2, default=0)
    includes_setup_service = models.BooleanField(_('يشمل خدمة الإعداد'), default=False)
    setup_service_cost = models.DecimalField(_('تكلفة خدمة الإعداد'), max_digits=10, decimal_places=2, default=0)
