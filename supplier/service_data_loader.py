...
from .field_registry import FIELD_REGISTRY, get_field_config, get_service_fields, get_field_choices
...
class ServiceDataLoader:
    ...
    def load_service_data(self, service_id):
        ...
        if service.category.code != self.service_type:
            raise ValueError(f"Service type mismatch: expected {self.service_type}, got {service.category.code}")
    ...
    def _save_related_details(self, service):
        ...
        """
        try:
            if self.service_type == 'offset_printing' and hasattr(service, 'offset_details'):
                service.offset_details.save()
            elif self.service_type == 'digital_printing' and hasattr(service, 'digital_details'):
