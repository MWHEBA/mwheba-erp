...
from .models import (
    ...
    ServicePriceTier, PaperServiceDetails, DigitalPrintingDetails,
    FinishingServiceDetails, OffsetPrintingDetails, PlateServiceDetails,
    OutdoorPrintingDetails, LaserServiceDetails, VIPGiftDetails
)
...
def price_comparison(request):
    ...
    if category_code:
        try:
            selected_category = SupplierType.objects.get(code=category_code, is_active=True)
            services = SpecializedService.objects.filter(
                category=selected_category,
                is_active=True,
                supplier__is_active=True
            ).select_related('supplier', 'category').prefetch_related('price_tiers')
...
def service_calculator(request):
    ...
    if service_id:
        try:
            service = SpecializedService.objects.select_related('supplier', 'category').get(
                id=service_id, 
                is_active=True
            )
...
def supplier_services_comparison(request, supplier_id):
    ...
    context = {
        ...
        'breadcrumb_items': [
            {'title': 'الرئيسية', 'url': reverse('core:dashboard'), 'icon': 'fas fa-home'},
            {'title': 'الموردين', 'url': reverse('supplier:supplier_list'), 'icon': 'fas fa-truck'},
            {'title': supplier.name, 'url': reverse('supplier:supplier_detail', args=[supplier.id]), 'icon': 'fas fa-user'},
...
def category_analysis(request, category_code):
    ...
    for service in services:
        supplier_type = service.supplier.primary_type.name if service.supplier.primary_type else 'غير محدد'
        supplier_types_distribution[supplier_type] = supplier_types_distribution.get(supplier_type, 0) + 1
    ...
    context = {
        ...
        'breadcrumb_items': [
            {'title': 'الرئيسية', 'url': reverse('core:dashboard'), 'icon': 'fas fa-home'},
            {'title': 'الموردين', 'url': reverse('supplier:supplier_list'), 'icon': 'fas fa-truck'},
            {'title': 'مقارنة الأسعار', 'url': reverse('supplier:price_comparison'), 'icon': 'fas fa-balance-scale'},
