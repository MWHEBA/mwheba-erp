...
def suppliers_by_type(request):
    ...
    for supplier_type in supplier_types:
        ...
        active_suppliers = supplier_type.suppliers.filter(is_active=True).order_by(
            "name"
        )
        ...
        try:
            total_services = SpecializedService.objects.filter(
                supplier__in=active_suppliers, is_active=True
            ).count()
        except:
            total_services = 0
...
def service_comparison(request):
    ...
    services = (
        SpecializedService.objects.filter(category=category, is_active=True)
