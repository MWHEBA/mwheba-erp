...
def services_home(request):
    ...
    service_types = (
        ...
        .annotate(
            services_count=Count(
                "specializedservice", filter=Q(specializedservice__is_active=True)
            ),
            suppliers_count=Count(
                "specializedservice__supplier",
                filter=Q(
                    specializedservice__is_active=True,
                    specializedservice__supplier__is_active=True,
                ),
                distinct=True,
            ),
            avg_price=Avg(
                "specializedservice__setup_cost",
                filter=Q(specializedservice__is_active=True),
            ),
        )
        .filter(services_count__gt=0)  # فقط الأنواع التي لها خدمات
    ...
    total_suppliers = (
        Supplier.objects.filter(is_active=True, specialized_services__is_active=True)
        .distinct()
        .count()
    )
    ...
    # متوسط التقييم العام
    avg_rating = 0
    active_services = SpecializedService.objects.filter(is_active=True).select_related(
        "supplier"
    )
...
def category_detail(request, slug):
    ...
    services = (
        SpecializedService.objects.filter(is_active=True, category=category)
    ...
    if category.code == "offset_printing":
        category_service_headers = [
            ...
            },
            {
                ...
                "sortable": False,
                "class": "text-center",
                "template": "components/cells/service_actions_crud.html",
                "width": "15%",
            },
        ]
    else:
    ...
    context = {
        ...
        "breadcrumb_items": [
            ...
            },
            {
                "title": category.name,
                "active": True,
                "icon": category.icon or "fas fa-cogs",
            },
        ],
    }
