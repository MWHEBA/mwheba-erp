    ...
    if created and not getattr(instance, "_skip_accounting", False):
        ...
        try:
            with transaction.atomic():
                ...
                if journal_entry:
                    logger.info(
                        f"تم إنشاء قيد محاسبي لحركة المخزون: {instance.reference_number}"
                    )

        except Exception as e:
            logger.error(
                f"خطأ في إنشاء قيد محاسبي لحركة المخزون {instance.reference_number}: {str(e)}"
            )
...
def delete_accounting_entry_for_stock_movement(sender, instance, **kwargs):
    ...
    try:
        if hasattr(instance, "journal_entry") and instance.journal_entry:
            ...
            logger.info(
                f"تم حذف القيد المحاسبي لحركة المخزون: {instance.reference_number}"
            )

    except Exception as e:
        logger.error(
            f"خطأ في حذف القيد المحاسبي لحركة المخزون {instance.reference_number}: {str(e)}"
        )
