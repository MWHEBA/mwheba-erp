...
try:
    from ..models.warehouse import StockTransfer, StockSnapshot
    ...
    try:
        ...
        stock_stats = Stock.objects.aggregate(
            ...
            low_stock_count=Count(
                "id", filter=Q(quantity__lte=F("min_stock_level"))
            ),
        ...
        low_stock_products = Stock.objects.filter(
            quantity__lte=F("min_stock_level"), product__is_active=True
...
def reorder_point_report(request):
    ...
    context = {
        ...
        "warehouses": Warehouse.objects.filter(is_active=True),
        "statuses": [
            {"id": "out_of_stock", "name": "نفذ المخزون"},
...
def enhanced_inventory_report(request):
    ...
    try:
        ...
        products = Product.objects.select_related('category', 'unit').prefetch_related(
            'stock_set__warehouse',
            'components',  # للمنتجات المجمعة
            'used_in_bundles'  # للمكونات
        ...
        if product_type == 'bundles':
            ...
            products = products.filter(used_in_bundles__isnull=False).distinct()
        elif product_type == 'regular':
            products = products.filter(is_bundle=False, used_in_bundles__isnull=True)
        
        # إعداد البيانات للجدول
        inventory_data = []
        for product in products:
            # حساب المخزون الإجمالي
            total_stock = product.stock_set.aggregate(
                total=Sum('quantity')
            )['total'] or 0
            
            # معلومات إضافية للمنتجات المجمعة
            bundle_info = {}
            if product.is_bundle:
                ...
                bundle_info = {
                    ...
                    'stock_difference': calculated_stock - total_stock if calculated_stock else 0
                }
            
            # معلومات المكونات
            component_info = {}
            if product.used_in_bundles.exists():
                bundles_count = product.used_in_bundles.count()
                component_info = {
                    'used_in_bundles_count': bundles_count,
                    'bundles_list': list(product.used_in_bundles.values_list('name', flat=True)[:3])
                }
            ...
            inventory_data.append({
                ...
                'stock_status': 'high' if total_stock > 50 else 'low' if total_stock > 0 else 'out'
            })
        ...
        stats = {
            ...
            'component_products': products.filter(used_in_bundles__isnull=False).distinct().count(),
        ...
        context = {
            ...
            'breadcrumb_items': [
                ...
                {'title': 'تقرير المخزون المحسن', 'active': True}
            ],
            'header_buttons': [
                {
        ...
        return render(request, 'product/enhanced_inventory_report.html', context)
        
    except Exception as e:
        messages.error(request, f'خطأ في تحميل تقرير المخزون: {str(e)}')
...
def bundle_stock_alerts_api(request):
    ...
    try:
        # المنتجات المجمعة بمخزون منخفض أو نفذ
        bundle_alerts = []
        
        bundles = Product.objects.filter(is_bundle=True, is_active=True).prefetch_related('components__component_product')
        ...
        for bundle in bundles:
            ...
            if calculated_stock == 0:
                ...
                alert_level = 'warning'
            elif calculated_stock <= 10:
                alert_level = 'info'
            
            if alert_level != 'normal':
                # تحديد المكونات المسببة للمشكلة
                problematic_components = []
                for component in bundle.components.all():
                    comp_stock = component.component_product.calculated_stock
                    required = component.required_quantity
                    available_bundles = comp_stock // required if required > 0 else 0
                    
                    if available_bundles <= 10:
        ...
        return JsonResponse({
            ...
            'total_alerts': len(bundle_alerts)
        })
