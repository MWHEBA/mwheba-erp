...
def get_product_sales_statistics(product):
    ...
    try:
        ...
        sale_items = SaleItem.objects.filter(
            product=product, sale__status="confirmed"
        ...
        # إحصائيات أساسية
        total_sold_quantity = (
            sale_items.aggregate(Sum("quantity"))["quantity__sum"] or 0
        )
        total_sales_value = sale_items.aggregate(Sum("total"))["total__sum"] or 0
        total_sales_count = sale_items.count()
        average_sale_price = (
            sale_items.aggregate(Avg("unit_price"))["unit_price__avg"] or 0
        )
        ...
        for i in range(5, -1, -1):  # من 5 إلى 0 (آخر 6 شهور)
            ...
            month_data = sale_items.filter(
                sale__date__gte=month_start, sale__date__lt=month_end
            ).aggregate(
        ...
        best_selling_month = (
            max(monthly_sales, key=lambda x: x["quantity"]) if monthly_sales else None
        )
        max_monthly_quantity = (
            max([month["quantity"] for month in monthly_sales]) if monthly_sales else 0
        )
        ...
        return {
            ...
            "period_dates": f"{current_period.start_date.strftime('%Y/%m/%d')} - {current_period.end_date.strftime('%Y/%m/%d')}"
            if current_period
            else None,
        }
...
def product_delete(request, pk):
    ...
    has_sale_items = (
        SaleItem is not None and SaleItem.objects.filter(product=product).exists()
    )
    has_purchase_items = (
    ...
    if request.method == "POST":
        try:
            ...
            if has_dependencies:
                ...
                messages.success(
                    request,
                    f'تم إلغاء تنشيط المنتج "{name}" بنجاح. لم يتم حذفه لارتباطه بعمليات سابقة',
                )
            else:
...
def brand_list(request):
    ...
    brand_headers = [
        ...
        },
        {
            ...
            "sortable": True,
            "class": "text-center",
            "template": "components/cells/website_link.html",
            "width": "150px",
        },
        {
            "key": "products_count",
    ...
    brand_actions = [
        {
            "url": "product:brand_detail",
            "icon": "fa-eye",
            "label": "عرض",
            "class": "action-view",
        },
        {
            "url": "product:brand_edit",
            "icon": "fa-edit",
            "label": "تعديل",
            "class": "action-edit",
        },
        {
            "url": "product:brand_delete",
            "icon": "fa-trash",
            "label": "حذف",
            "class": "action-delete",
        },
...
def stock_list(request):
    ...
    stocks = Stock.objects.all().select_related(
        "product", "product__category", "product__brand", "product__unit", "warehouse"
    )
...
def stock_movement_list(request):
    ...
    movement_headers = [
        ...
        },
        {
            ...
            "sortable": True,
            "class": "text-center",
            "template": "components/cells/movement_type.html",
            "width": "120px",
        },
        {
            "key": "quantity",
    ...
    movement_actions = [
        {
            "url": "product:stock_movement_detail",
            "icon": "fa-eye",
            "label": "عرض",
            "class": "action-view",
        },
    ...
    try:
        ...
        with transaction.atomic():
            ...
            try:
                ...
                if sales_count > 0:
                    Sale.objects.filter(warehouse=source_warehouse).update(
                        warehouse=target_warehouse
                    )
                    logger.info(
                        f"تم نقل {sales_count} فاتورة مبيعات من {source_warehouse.name} إلى {target_warehouse.name}"
                    )
            except ImportError:
                pass
    ...
    try:
        ...
        with transaction.atomic():
            ...
            try:
                ...
                if sales_count > 0:
                    logger.warning(
                        f"سيتم حذف {sales_count} فاتورة مبيعات مرتبطة بالمخزن {warehouse.name}"
                    )
