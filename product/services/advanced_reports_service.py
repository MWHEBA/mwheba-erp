...
class AdvancedReportsService:
    ...
    def abc_analysis(warehouse=None, period_months=12):
        ...
        try:
            ...
            for item in products_data:
                ...
                item['value_percentage'] = float((item['total_value_sold'] / total_value) * 100)
    ...
    def inventory_turnover_analysis(warehouse=None, period_months=12):
        ...
        try:
            ...
            # حساب الإحصائيات
            if turnover_data:
                avg_turnover = sum(item['turnover_ratio'] for item in turnover_data) / len(turnover_data)
                fast_count = len([item for item in turnover_data if item['turnover_category'] == 'سريع'])
                medium_count = len([item for item in turnover_data if item['turnover_category'] == 'متوسط'])
                slow_count = len([item for item in turnover_data if item['turnover_category'] == 'بطيء'])
            else:
                avg_turnover = 0
        ...
        try:
            ...
            for stock in queryset:
                if stock.quantity > 0:
                    ...
                    if last_movement:
                        days_since_last_movement = (today - last_movement.movement_date.date()).days
                    else:
                        # إذا لم توجد حركة صادرة، استخدم تاريخ إنشاء المنتج
                        days_since_last_movement = (today - stock.product.created_at.date()).days
                    ...
                    aging_data.append({
                        ...
                        'current_stock': stock.quantity,
                        'average_cost': stock.average_cost,
                        'inventory_value': float(stock.quantity * stock.average_cost),
                        'days_since_last_movement': days_since_last_movement,
                        'last_movement_date': last_movement.movement_date.date() if last_movement else None,
            ...
            # حساب الإحصائيات
            if aging_data:
                new_count = len([item for item in aging_data if item['age_category'] == 'جديد'])
                medium_count = len([item for item in aging_data if item['age_category'] == 'متوسط'])
                old_count = len([item for item in aging_data if item['age_category'] == 'قديم'])
                stagnant_count = len([item for item in aging_data if item['age_category'] == 'راكد'])
                
                total_value = sum(item['inventory_value'] for item in aging_data)
                stagnant_value = sum(item['inventory_value'] for item in aging_data if item['age_category'] == 'راكد')
            else:
            ...
            return {
                ...
                'summary': {
                    ...
                    'stagnant_percentage': round((stagnant_value / total_value * 100), 2) if total_value > 0 else 0
                },
                'generated_at': timezone.now()
            }
        ...
        try:
            ...
            for stock in queryset:
                ...
                if daily_consumption > 0:
                    # طلب لمدة 30 يوم
                    suggested_order_qty = max(0, (daily_consumption * 30) - stock.quantity)
                else:
                ...
                reorder_data.append({
                    ...
                    'priority': priority,
                    'days_remaining': int(stock.quantity / daily_consumption) if daily_consumption > 0 else 999
                })
            ...
            # حساب الإحصائيات
            if reorder_data:
                out_of_stock = len([item for item in reorder_data if item['status'] == 'نفد'])
                need_reorder = len([item for item in reorder_data if item['status'] == 'يحتاج طلب'])
                under_watch = len([item for item in reorder_data if item['status'] == 'مراقبة'])
                normal = len([item for item in reorder_data if item['status'] == 'طبيعي'])
            else:
