...
from ..models.expiry_tracking import (
    ProductBatch,
    BatchConsumption,
    ExpiryAlert,
    ExpiryRule,
...
class ExpiryService:
        ...
        try:
            with transaction.atomic():
                ...
                if ProductBatch.objects.filter(
                    batch_number=batch_number, product=product
                ).exists():
                    raise ValueError(
                        f"رقم الدفعة {batch_number} موجود مسبقاً لهذا المنتج"
                    )
    ...
    def get_available_batches(product, warehouse=None, exclude_expired=True):
        ...
        try:
            queryset = ProductBatch.objects.filter(
                product=product, status="active", current_quantity__gt=0
            )
    ...
    def check_expiry_alerts():
        ...
        try:
            ...
            active_batches = ProductBatch.objects.filter(
                status="active", current_quantity__gt=0, expiry_date__isnull=False
            )
    ...
    def _check_batch_alerts(batch):
        ...
        try:
            ...
            rules = ExpiryRule.objects.filter(is_active=True)
            ...
            for rule in rules:
                if (
                    ...
                    or batch.product.category == rule.product_category
                ):
                    if not rule.warehouse or batch.warehouse == rule.warehouse:
            ...
            for rule in applicable_rules:
                ...
                if alert_type:
                    ...
                    existing_alert = ExpiryAlert.objects.filter(
                        batch=batch, alert_type=alert_type, alert_date=today
                    ).exists()
    ...
    def get_expiry_report(warehouse=None, days_ahead=90):
        ...
        try:
            ...
            total_value_at_risk = sum(
                batch.current_quantity * batch.unit_cost for batch in queryset
            )

            expired_value = sum(
                batch.current_quantity * batch.unit_cost for batch in expired_batches
            )
    ...
    def auto_expire_batches():
        ...
        try:
            ...
            expired_batches = ProductBatch.objects.filter(
                status="active", expiry_date__lt=today
            )
    ...
    def get_batch_history(batch):
        ...
        """
        try:
            consumptions = BatchConsumption.objects.filter(batch=batch).order_by(
                "-consumed_at"
            )

            from ..models.reservation_system import BatchReservation

            reservations = BatchReservation.objects.filter(batch=batch).order_by(
                "-reserved_at"
            )

            alerts = ExpiryAlert.objects.filter(batch=batch).order_by("-created_at")
