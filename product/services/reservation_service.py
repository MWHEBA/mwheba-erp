...
from ..models.reservation_system import StockReservation, ReservationFulfillment, ReservationRule
...
class ReservationService:
    ...
    def create_reservation(
        ...
        try:
            with transaction.atomic():
                ...
                available_quantity = ReservationService.get_available_quantity(product, warehouse)
    ...
    def get_available_quantity(product, warehouse):
        ...
        try:
            ...
            active_reservations = StockReservation.objects.filter(
                ...
                total_reserved=models.Sum('quantity_reserved') - models.Sum('quantity_fulfilled')
            )['total_reserved'] or 0
    ...
    @staticmethod
    def fulfill_reservation(reservation_id, quantity, user=None, inventory_movement=None):
        """
        تنفيذ كمية من الحجز
        """
        try:
            with transaction.atomic():
                ...
                if reservation.status != 'active':
                    raise ValueError(f"لا يمكن تنفيذ حجز بحالة: {reservation.get_status_display()}")
    ...
    def get_reservation_summary(warehouse=None):
        ...
        try:
            ...
            summary = queryset.aggregate(
                ...
                active_reservations=models.Count('id', filter=models.Q(status='active')),
                fulfilled_reservations=models.Count('id', filter=models.Q(status='fulfilled')),
                cancelled_reservations=models.Count('id', filter=models.Q(status='cancelled')),
                expired_reservations=models.Count('id', filter=models.Q(status='expired')),
            ...
            if summary['total_quantity_reserved'] > 0:
                fulfillment_rate = (summary['total_quantity_fulfilled'] / 
                                  summary['total_quantity_reserved']) * 100
            else:
                fulfillment_rate = 0
        ...
        try:
            ...
            for reservation in reservations:
                ...
                allocation_results.append({
                    ...
                    'status': 'allocated' if can_allocate == remaining_needed else 'partial'
                })
            ...
            return {'total_allocated': 0, 'remaining_stock': available_quantity, 'allocations': []}
            ...
            with transaction.atomic():
                for item in order_items:
                    ...
                    rules = ReservationRule.objects.filter(
                        is_active=True,
                        rule_type='auto_reserve_on_order'
                    )
                    ...
                    if should_reserve:
                        try:
                            reservation = ReservationService.create_reservation(
                                ...
                                user=user,
                                sale_order_id=order_id if order_type == 'sale_order' else None,
                                purchase_order_id=order_id if order_type == 'purchase_order' else None,
                            ...
                            logger.warning(f"فشل في إنشاء حجز تلقائي للمنتج {product.name}: {e}")
                            continue
            
            logger.info(f"تم إنشاء {len(reservations_created)} حجز تلقائي للطلب {order_id}")
