            ...
            with transaction.atomic():
                ...
                for zone_data in zones_data:
                    ...
                    zone = LocationZone.objects.create(
                        ...
                        requires_authorization=zone_data.get(
        ...
        try:
            with transaction.atomic():
                ...
                existing_location = ProductLocation.objects.filter(
                    product=product, shelf=shelf, level=level, position=position
                ).first()

                if existing_location:
                    raise ValueError(
                        f"الموقع موجود مسبقاً: {existing_location.full_location_code}"
                    )
    ...
    def get_product_locations(product, warehouse=None):
        ...
        try:
            ...
            return queryset.order_by(
                "location_type", "shelf__aisle__sequence", "shelf__sequence"
            )
    ...
    def get_location_utilization(warehouse):
        ...
        try:
            ...
            total_locations = ProductLocation.objects.filter(
                shelf__aisle__zone__warehouse=warehouse, is_active=True
            ).count()
            ...
            full_locations = (
                ...
                .exclude(max_quantity__isnull=True)
                .count()
            )

            # حساب النسب
            utilization_rate = (
                (occupied_locations / total_locations * 100)
                if total_locations > 0
                else 0
            )
            capacity_rate = (
                (full_locations / total_locations * 100) if total_locations > 0 else 0
            )
    ...
    def get_location_tasks(warehouse=None, status=None, assigned_to=None):
        ...
        try:
            queryset = LocationTask.objects.select_related(
                "location__shelf__aisle__zone", "product", "assigned_to", "created_by"
            )
        ...
        try:
            ...
            for item in pick_list:
                ...
                for location in locations:
                    ...
                    available_qty = min(
                        location.current_quantity, quantity_needed - quantity_collected
                    )
