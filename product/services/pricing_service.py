...
class PricingService:
            ...
            with transaction.atomic():
                ...
                supplier_price, created = SupplierProductPrice.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        'created_by': user,
                        'is_default': not SupplierProductPrice.objects.filter(product=product).exists(),
                        'notes': notes
                    }
                )
                ...
                logger.info(f"تم تحديث سعر المنتج {product.name} للمورد {supplier.name} إلى {new_price}")
        ...
        try:
            with transaction.atomic():
                ...
                if not supplier_price:
                    logger.warning(f"لا يوجد سعر للمنتج {product.name} عند المورد {supplier.name}")
                    return False
                ...
                PricingService._update_product_main_cost_price(product, supplier_price.cost_price)
                
                logger.info(f"تم تعيين {supplier.name} كمورد افتراضي للمنتج {product.name}")
                return True
    ...
    def get_price_comparison(product: Product) -> List[Dict[str, Any]]:
        ...
        try:
            supplier_prices = SupplierProductPrice.objects.filter(
                product=product,
                is_active=True
            ).select_related('supplier').order_by('-is_default', 'cost_price')
            
            comparison = []
            main_price = product.cost_price or Decimal('0')
            
            for sp in supplier_prices:
                price_diff = sp.cost_price - main_price if main_price > 0 else Decimal('0')
                price_diff_percent = (price_diff / main_price * 100) if main_price > 0 else Decimal('0')
    ...
    @staticmethod
    def get_price_history(product: Product, supplier: Optional[Supplier] = None, limit: int = 50) -> List[PriceHistory]:
        """
        ...
        for update_data in updates:
            try:
                ...
                if result:
                    ...
                    results['failed'] += 1
                    results['errors'].append(f"فشل تحديث {product.name} - {supplier.name}")
    ...
    @staticmethod
    def get_cheapest_suppliers(product: Product, limit: int = 5) -> List[SupplierProductPrice]:
        """
        الحصول على أرخص الموردين لمنتج معين
        """
        try:
            return list(SupplierProductPrice.objects.filter(
                product=product,
                is_active=True
            ).select_related('supplier').order_by('cost_price')[:limit])
    ...
    def get_supplier_products_summary(supplier: Supplier) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_price = supplier_prices.aggregate(
    ...
    def _update_product_main_cost_price(product: Product, new_price: Decimal):
        ...
        try:
            if product.cost_price != new_price:
                ...
                logger.info(f"تم تحديث سعر التكلفة الرئيسي للمنتج {product.name} إلى {new_price}")
        except Exception as e:
    ...
    def get_price_comparison(product):
        ...
        try:
            prices = SupplierProductPrice.objects.filter(
                product=product
            ).select_related('supplier').order_by('cost_price')
            ...
            return {
                ...
                'default_supplier': default_supplier.supplier if default_supplier else None
            }
