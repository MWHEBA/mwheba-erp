"""
خدمة إدارة المخزون وتتبع الحركات - محدثة للنموذج الموحد
"""
                    ...
                    try:
                        ...
                        if created or True:  # إعادة حساب دائماً
                            ...
                            movements = InventoryMovement.objects.filter(
                                product=product, warehouse=warehouse, date__date=date
                            )
                        ...
                        logger.error(
                            f"خطأ في إنشاء لقطة المخزون للمنتج {product.name}: {e}"
                        )
                        continue
    ...
    def get_inventory_report(warehouse=None, category=None, low_stock_only=False):
        ...
        try:
            ...
            stats = queryset.aggregate(
                ...
                total_value=models.Sum(models.F("quantity") * models.F("average_cost")),
                low_stock_count=models.Count(
                    "id", filter=models.Q(quantity__lte=models.F("product__min_stock"))
                ),
                out_of_stock_count=models.Count("id", filter=models.Q(quantity=0)),
            )
            ...
            return {"stocks": [], "stats": {}, "error": str(e)}

    @staticmethod
    def get_movement_report(
        product=None, warehouse=None, date_from=None, date_to=None, movement_type=None
    ):
        """
        تقرير حركات المخزون
        """
        try:
            ...
            stats = queryset.aggregate(
                ...
                total_in=models.Sum(
                    "quantity", filter=models.Q(movement_type__in=["in", "return_in"])
                ),
            ...
            return {"movements": [], "stats": {}, "error": str(e)}
