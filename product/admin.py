...
class ProductAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_('التسعير'), {'fields': ('cost_price', 'selling_price', 'tax_rate', 'discount_rate', 'default_supplier')}),
...
class SupplierProductPriceAdmin(admin.ModelAdmin):
    ...
    """
    list_display = ('product', 'supplier', 'cost_price', 'is_default', 'last_purchase_date', 
                   'last_purchase_quantity', 'is_active', 'created_at')
    list_filter = ('is_default', 'is_active', 'supplier', 'last_purchase_date')
    search_fields = ('product__name', 'product__sku', 'supplier__name', 'supplier__code')
    readonly_fields = ('created_at', 'updated_at', 'created_by')
    autocomplete_fields = ['product', 'supplier']
    
    fieldsets = (
        (None, {
            'fields': ('product', 'supplier', 'cost_price')
        }),
        (_('الحالة'), {
...
class PriceHistoryAdmin(admin.ModelAdmin):
    ...
    """
    list_display = ('get_product_name', 'get_supplier_name', 'old_price', 'new_price', 
                   'change_amount', 'change_percentage', 'change_reason', 'change_date', 'changed_by')
    list_filter = ('change_reason', 'change_date', 'supplier_product_price__supplier')
    search_fields = ('supplier_product_price__product__name', 'supplier_product_price__supplier__name',
                    'purchase_reference', 'notes')
    readonly_fields = ('change_date', 'changed_by', 'change_amount', 'change_percentage')
    date_hierarchy = 'change_date'
    
    fieldsets = (
        (None, {
            'fields': ('supplier_product_price', 'old_price', 'new_price')
        }),
        (_('التغيير'), {
            'fields': ('change_amount', 'change_percentage', 'change_reason')
        }),
        (_('المرجع'), {
            'fields': ('purchase_reference', 'notes')
        }),
        (_('معلومات النظام'), {
            'fields': ('change_date', 'changed_by'),
            'classes': ('collapse',)
        }),
    )
