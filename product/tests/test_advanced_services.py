...
class AdvancedReportsServiceTestCase(TestCase):
    ...
    @patch('product.services.advanced_reports_service.AdvancedReportsService._get_sales_data')
    def test_abc_analysis(self, mock_sales_data):
        ...
        top_product = next(p for p in products if p['product_id'] == self.products[0].id)
    ...
    def test_stock_aging_analysis(self):
        ...
        with patch.object(self.reports_service, '_get_last_movement_dates') as mock_dates:
    ...
    def test_reorder_point_analysis(self):
        ...
        with patch.object(self.reports_service, '_calculate_lead_time') as mock_lead_time:
            with patch.object(self.reports_service, '_get_average_daily_usage') as mock_usage:
