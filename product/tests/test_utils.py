...
class TestDataFactory:
    ...
    def create_user(username=None, **kwargs):
        ...
        email = kwargs.get('email', f"{username}_{TestDataFactory._random_string(4)}@example.com")
    ...
    @staticmethod
    def create_product(name=None, sku=None, category=None, brand=None, 
                      unit=None, created_by=None, **kwargs):
        """إنشاء منتج للاختبار"""
        if not name:
    ...
    @staticmethod
    def create_stock_reservation(product=None, warehouse=None, 
...
class TestAssertions:
    ...
    def assert_stock_quantity(test_case, product, warehouse, expected_quantity):
        ...
        try:
            ...
            test_case.fail(f"لا يوجد مخزون للمنتج {product.name} في المستودع {warehouse.name}")
    ...
    def assert_reservation_exists(test_case, product, warehouse, reserved_for):
        ...
        test_case.assertIsNotNone(
            reservation,
            f"لا يوجد حجز نشط للمنتج {product.name} للطلب {reserved_for}"
        )
        return reservation
    ...
    def assert_batch_expiry_status(test_case, batch, expected_status):
        ...
        if expected_status == 'expired':
            test_case.assertTrue(batch.is_expired, f"الدفعة {batch.batch_number} يجب أن تكون منتهية الصلاحية")
        elif expected_status == 'near_expiry':
            test_case.assertTrue(batch.is_near_expiry, f"الدفعة {batch.batch_number} يجب أن تكون قريبة من انتهاء الصلاحية")
        elif expected_status == 'good':
            test_case.assertFalse(batch.is_expired, f"الدفعة {batch.batch_number} يجب ألا تكون منتهية الصلاحية")
            test_case.assertFalse(batch.is_near_expiry, f"الدفعة {batch.batch_number} يجب ألا تكون قريبة من انتهاء الصلاحية")
...
class TestScenarios:
    ...
    def setup_basic_inventory_scenario():
        ...
        user = TestDataFactory.create_user(username=f'inventory_manager_{TestDataFactory._random_string(4)}')
    ...
    def setup_supplier_pricing_scenario():
        ...
        user = TestDataFactory.create_user(username=f'pricing_manager_{TestDataFactory._random_string(4)}')
    ...
    def setup_expiry_tracking_scenario():
        ...
        user = TestDataFactory.create_user(username=f'expiry_manager_{TestDataFactory._random_string(4)}')
        warehouse = TestDataFactory.create_warehouse(manager=user)
        product = TestDataFactory.create_product(created_by=user)
    ...
    def setup_reservation_scenario():
        ...
        user = TestDataFactory.create_user(username=f'reservation_manager_{TestDataFactory._random_string(4)}')
        warehouse = TestDataFactory.create_warehouse(manager=user)
        product = TestDataFactory.create_product(created_by=user)
