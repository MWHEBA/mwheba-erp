...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for movement in all_movements:
            if movement.reference_number and movement.reference_number.startswith(
                "PURCHASE-"
            ):
        ...
        # حذف الحركات المكررة
        duplicate_count = StockMovement.objects.filter(
            id__in=movements_to_delete
        ).count()
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"تم حذف {duplicate_count} حركة مكررة وإعادة حساب أرصدة المخزون بنجاح"
            )
        )
