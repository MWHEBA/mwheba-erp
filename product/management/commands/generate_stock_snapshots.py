...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--days", type=int, default=1, help="عدد الأيام السابقة لإنشاء لقطات لها"
        )
        parser.add_argument(
            "--force", action="store_true", help="إعادة إنشاء اللقطات الموجودة"
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"بدء إنشاء لقطات المخزون"))
        ...
        for i in range(days):
            ...
            try:
                snapshots_count = InventoryService.generate_daily_snapshots(
                    current_date
                )
        ...
        if total_snapshots == 0:
            self.stdout.write(
                self.style.WARNING(
                    "\n💡 نصيحة: تأكد من وجود منتجات ومخزنات نشطة في النظام"
                )
            )
