...
class LocationZone(models.Model):
    ...
    code = models.CharField(_("كود المنطقة"), max_length=20)

    zone_type = models.CharField(
        _("نوع المنطقة"), max_length=20, choices=ZONE_TYPE, default="normal"
    )
    ...
    # قيود الوصول
    requires_authorization = models.BooleanField(_("يتطلب تصريح"), default=False)
...
class LocationAisle(models.Model):
    ...
    name = models.CharField(_("اسم الممر"), max_length=50)

    code = models.CharField(_("كود الممر"), max_length=10)
...
class LocationShelf(models.Model):
    ...
    name = models.CharField(_("اسم الرف"), max_length=50)

    code = models.CharField(_("كود الرف"), max_length=10)
...
class ProductLocation(models.Model):
    ...
    level = models.PositiveIntegerField(
        _("المستوى"), default=1, validators=[MinValueValidator(1)]
    )
    ...
    location_type = models.CharField(
        _("نوع الموقع"), max_length=20, choices=LOCATION_TYPE, default="primary"
    )
    ...
    max_quantity = models.PositiveIntegerField(
        _("الحد الأقصى للكمية"), null=True, blank=True
    )

    min_quantity = models.PositiveIntegerField(_("الحد الأدنى للكمية"), default=0)
...
class LocationMovement(models.Model):
    ...
    movement_type = models.CharField(
        _("نوع الحركة"), max_length=20, choices=MOVEMENT_TYPE
    )
    ...
    moved_at = models.DateTimeField(_("تاريخ النقل"), auto_now_add=True)
    ...
    def __str__(self):
        from_loc = (
            self.from_location.full_location_code if self.from_location else "خارجي"
        )
...
class LocationTask(models.Model):
    ...
    task_type = models.CharField(_("نوع المهمة"), max_length=20, choices=TASK_TYPE)
    ...
    quantity = models.PositiveIntegerField(_("الكمية"), null=True, blank=True)

    priority = models.PositiveIntegerField(_("الأولوية"), choices=PRIORITY, default=3)

    status = models.CharField(
        _("الحالة"), max_length=20, choices=TASK_STATUS, default="pending"
    )
