...
class ProductStock(models.Model):
    ...
    reserved_quantity = models.PositiveIntegerField(
        _("الكمية المحجوزة"), default=0, help_text=_("الكمية المحجوزة للطلبات المعلقة")
    )
    ...
    last_movement_date = models.DateTimeField(
        _("تاريخ آخر حركة"), null=True, blank=True
    )
    ...
    def __str__(self):
        return (
            f"{self.product.name} - {self.warehouse.name} ({self.available_quantity})"
        )
    ...
    def update_average_cost(self, new_quantity, new_cost):
        ...
        if new_quantity > 0:
            total_value = (self.quantity * self.average_cost) + (
                new_quantity * new_cost
            )
            total_quantity = self.quantity + new_quantity
            self.average_cost = (
                total_value / total_quantity if total_quantity > 0 else new_cost
            )
...
class StockSnapshot(models.Model):
    ...
    warehouse = models.ForeignKey(
        "product.Warehouse", on_delete=models.CASCADE, verbose_name=_("المخزن")
    )
    product = models.ForeignKey(
        "product.Product", on_delete=models.CASCADE, verbose_name=_("المنتج")
    )
    quantity = models.PositiveIntegerField(_("الكمية"))
    average_cost = models.DecimalField(
