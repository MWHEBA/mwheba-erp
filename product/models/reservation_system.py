...
class StockReservation(models.Model):
    ...
    priority = models.PositiveIntegerField(
        _('الأولوية'),
        default=5,
        help_text=_('1 = أولوية عالية، 10 = أولوية منخفضة')
    )
    ...
    def fulfill_quantity(self, quantity, user=None):
        ...
        if quantity > self.quantity_remaining:
            raise ValueError(f"الكمية المطلوبة ({quantity}) أكبر من المتبقية ({self.quantity_remaining})")
...
class ReservationRule(models.Model):
    ...
    # إعدادات القاعدة
    auto_reserve_enabled = models.BooleanField(
        _('تفعيل الحجز التلقائي'),
        default=False
    )
