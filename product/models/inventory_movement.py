    ...
    def reverse(self, user, reason=None):
        ...
        try:
            ...
            reverse_movement = InventoryMovement.objects.create(
                ...
                notes=f"عكس الحركة {self.movement_number}" + (f" - {reason}" if reason else ""),
...
class InventoryAdjustmentItem(models.Model):
    ...
    system_quantity = models.PositiveIntegerField(
        _('الكمية النظرية'), 
        help_text=_('الكمية حسب النظام')
    )
    actual_quantity = models.PositiveIntegerField(
        _('الكمية الفعلية'), 
        help_text=_('الكمية المعدودة فعلياً')
    )
    quantity_difference = models.IntegerField(
        _('فرق الكمية'), 
        help_text=_('الفرق بين الفعلي والنظري')
    )
    unit_cost = models.DecimalField(
        _('تكلفة الوحدة'), 
        max_digits=12, 
    ...
    def apply_adjustment(self):
        ...
        try:
            ...
            movement_type = 'adjustment_in' if self.quantity_difference > 0 else 'adjustment_out'
