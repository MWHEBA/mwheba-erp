...
class InventoryMovement(models.Model):
    ...
    # دمج جميع الأغراض
    PURPOSE_TYPES = RECEIPT_PURPOSE_TYPES + ISSUE_PURPOSE_TYPES
    ...
    voucher_type = models.CharField(
        ...
        default="none",
        help_text=_("نوع الإذن (استلام/صرف/لا يوجد)"),
    )
    ...
    purpose_type = models.CharField(
        ...
        blank=True,
        null=True,
        help_text=_("غرض الإذن (يحدد الحساب المحاسبي المقابل)"),
    )
    ...
    contra_account = models.ForeignKey(
        ...
        null=True,
        blank=True,
        verbose_name=_("الحساب المقابل"),
        related_name="inventory_movements_contra",
        help_text=_("الحساب المحاسبي المقابل (يتم تحديده تلقائياً حسب الغرض)"),
    )
    ...
    party_name = models.CharField(
        ...
        max_length=200,
        blank=True,
        help_text=_("اسم الجهة المستلمة أو المصدرة"),
    )
    ...
    reference_document = models.CharField(
        ...
        max_length=100,
        blank=True,
        help_text=_("رقم أو اسم المستند المرجعي"),
    )
    ...
    received_by_name = models.CharField(
        ...
        max_length=200,
        blank=True,
        help_text=_("اسم الشخص المستلم (لأذون الاستلام)"),
    )
...
class InventoryAdjustment(models.Model):
    ...
    warehouse = models.ForeignKey(
        "Warehouse", on_delete=models.PROTECT, verbose_name=_("المخزن")
    )
...
class InventoryAdjustmentItem(models.Model):
    ...
    product = models.ForeignKey(
        "Product", on_delete=models.PROTECT, verbose_name=_("المنتج")
    )
