...
class ProductBatch(models.Model):
    ...
    # معلومات الدفعة
    batch_number = models.CharField(_("رقم الدفعة"), max_length=100, unique=True)
    ...
    initial_quantity = models.PositiveIntegerField(
        _("الكمية الأولية"), validators=[MinValueValidator(1)]
    )

    current_quantity = models.PositiveIntegerField(_("الكمية الحالية"), default=0)

    reserved_quantity = models.PositiveIntegerField(_("الكمية المحجوزة"), default=0)

    # تواريخ مهمة
    production_date = models.DateField(_("تاريخ الإنتاج"), null=True, blank=True)
    ...
    status = models.CharField(
        _("حالة الدفعة"), max_length=20, choices=BATCH_STATUS, default="active"
    )
    ...
    supplier_batch_number = models.CharField(
        _("رقم دفعة المورد"), max_length=100, blank=True, null=True
    )
    ...
    def consume_quantity(self, quantity, user=None):
        ...
        if quantity > self.available_quantity:
            raise ValueError(
                f"الكمية المطلوبة ({quantity}) أكبر من المتاحة ({self.available_quantity})"
            )
    ...
    def release_reservation(self, quantity, reservation_reference=None):
        ...
        if quantity > self.reserved_quantity:
            raise ValueError(
                f"الكمية المطلوبة ({quantity}) أكبر من المحجوزة ({self.reserved_quantity})"
            )
...
class BatchConsumption(models.Model):
    ...
    quantity_consumed = models.PositiveIntegerField(_("الكمية المستهلكة"))

    consumed_at = models.DateTimeField(_("تاريخ الاستهلاك"), auto_now_add=True)
...
class BatchReservation(models.Model):
    ...
    quantity_reserved = models.IntegerField(
        _("الكمية المحجوزة"), help_text=_("كمية موجبة للحجز، سالبة لإلغاء الحجز")
    )

    reservation_reference = models.CharField(
        _("مرجع الحجز"), max_length=100, blank=True, null=True
    )
...
class ExpiryAlert(models.Model):
    ...
    alert_type = models.CharField(_("نوع التنبيه"), max_length=20, choices=ALERT_TYPE)

    alert_date = models.DateField(_("تاريخ التنبيه"), default=timezone.now)

    days_to_expiry = models.IntegerField(_("أيام حتى انتهاء الصلاحية"))

    is_acknowledged = models.BooleanField(_("تم الاطلاع"), default=False)
    ...
    acknowledged_at = models.DateTimeField(_("تاريخ الاطلاع"), null=True, blank=True)
...
class ExpiryRule(models.Model):
    ...
    auto_block_expired = models.BooleanField(
        _("حظر المنتجات المنتهية الصلاحية تلقائياً"), default=True
    )
