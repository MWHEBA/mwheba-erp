...
class Category(models.Model):
    ...
    parent = models.ForeignKey('self', on_delete=models.CASCADE, blank=True, null=True,
                              related_name='children', verbose_name=_('التصنيف الأم'))
...
class Product(models.Model):
    ...
    brand = models.ForeignKey(Brand, on_delete=models.PROTECT, related_name='products',
                             verbose_name=_('النوع'), blank=True, null=True)
    description = models.TextField(_('الوصف'), blank=True, null=True)
    ...
    def get_all_supplier_prices(self):
        ...
        """
        try:
            return SupplierProductPrice.objects.filter(
                product=self, 
                is_active=True
    ...
    def update_cost_price_from_supplier(self, supplier, new_price, user, reason='manual_update', purchase_reference=None):
        """
        ...
        try:
            ...
            supplier_price, created = SupplierProductPrice.objects.get_or_create(
                ...
                defaults={
                    ...
                    'created_by': user,
                    'is_default': not SupplierProductPrice.objects.filter(product=self).exists()
                }
            )
...
class StockMovement(models.Model):
    ...
    # ربط بالقيد المحاسبي
    journal_entry = models.ForeignKey('financial.JournalEntry', on_delete=models.SET_NULL,
    ...
    def _create_journal_entry(self):
        ...
        try:
            ...
            journal_entry = JournalEntry.objects.create(
                date=self.timestamp.date() if hasattr(self.timestamp, 'date') else self.timestamp,
    ...
    def save(self, *args, **kwargs):
        ...
        if (is_new and not self.journal_entry and 
            not getattr(self, '_skip_update', False) and 
            self.document_type not in excluded_types):
...
class SupplierProductPrice(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        elif not SupplierProductPrice.objects.filter(
            product=self.product, 
            is_default=True
        ...
        if self.is_default and self.product.cost_price != self.cost_price:
            Product.objects.filter(pk=self.product.pk).update(cost_price=self.cost_price)
    ...
    def price_difference_percentage(self):
        ...
        if self.product.cost_price and self.product.cost_price > 0:
            return ((self.cost_price - self.product.cost_price) / self.product.cost_price) * 100
        return Decimal('0')
