...
from .forms import PurchaseForm, PurchaseItemForm, PurchasePaymentForm, PurchasePaymentEditForm, PurchaseReturnForm, PurchaseUpdateForm
...
def purchase_list(request):
    ...
    purchase_headers = [
        ...
        {'key': 'created_at', 'label': _('التاريخ والوقت'), 'sortable': True, 'class': 'text-center', 'format': 'datetime_12h'},
        ...
        {'key': 'payment_method', 'label': _('طريقة الدفع'), 'sortable': True, 'class': 'text-center', 'template': 'components/cells/purchase_payment_method.html'},
    ...
    purchase_actions = [
        ...
        {'url': 'purchase:purchase_edit', 'icon': 'fa-edit', 'label': _('تعديل'), 'class': 'action-edit', 'condition': 'not_fully_paid'},
        {'url': 'purchase:purchase_delete', 'icon': 'fa-trash', 'label': _('حذف'), 'class': 'action-delete', 'condition': 'not_fully_paid'},
        {'url': 'purchase:purchase_add_payment', 'icon': 'fa-money-bill', 'label': _('إضافة دفعة'), 'class': 'action-paid', 'condition': 'not_fully_paid'}
    ]
...
def purchase_create(request):
                    ...
                    if purchase.payment_method == 'cash':
                        ...
                        if financial_account_id:
                            try:
                                ...
                                financial_account = ChartOfAccounts.objects.get(id=financial_account_id)
                                ...
                                if journal_entry:
                                    ...
                                    payment.save(update_fields=['financial_transaction', 'financial_status'])
                                    logger.info(f"✅ تم إنشاء دفعة تلقائية وقيد محاسبي للفاتورة النقدية: {purchase.number}")
                            ...
                            messages.warning(request, 'تحذير: لم يتم اختيار خزينة للفاتورة النقدية')
    ...
    purchase_orders = PurchaseOrder.objects.filter(status='pending').order_by('-date', '-id')
...
def purchase_detail(request, pk):
    ...
    context = {
        ...
        'breadcrumb_items': [
            ...
            {'title': purchase.supplier.name, 'url': reverse('supplier:supplier_detail', args=[purchase.supplier.pk]), 'icon': 'fas fa-truck'},
...
def payment_detail(request, pk):
    ...
    context = {
        ...
        'breadcrumb_items': [
            ...
            {'title': 'المشتريات', 'url': reverse('purchase:purchase_list'), 'icon': 'fas fa-shopping-basket'},
            {'title': purchase.supplier.name, 'url': reverse('supplier:supplier_detail', args=[purchase.supplier.pk]), 'icon': 'fas fa-truck'},
            {'title': f'فاتورة {purchase.number}', 'url': reverse('purchase:purchase_detail', kwargs={'pk': purchase.pk}), 'icon': 'fas fa-file-invoice'},
            {'title': f'دفعة #{payment.id}', 'active': True, 'icon': 'fas fa-money-bill-wave'}
        ],
    }
...
def purchase_return_list(request):
    ...
    """
    returns = PurchaseReturn.objects.all().select_related('purchase', 'purchase__supplier').order_by('-date', '-id')
    
    # تعريف أعمدة جدول مرتجعات المشتريات
    return_headers = [
        {'key': 'id', 'label': '#', 'sortable': True, 'class': 'text-center', 'width': '50px'},
        {'key': 'purchase.invoice_number', 'label': 'رقم الفاتورة', 'sortable': True, 'class': 'text-center', 'width': '120px'},
        {'key': 'purchase.supplier.name', 'label': 'المورد', 'sortable': True, 'class': 'text-start'},
        {'key': 'date', 'label': 'تاريخ المرتجع', 'sortable': True, 'class': 'text-center', 'format': 'date', 'width': '120px'},
        {'key': 'total_amount', 'label': 'إجمالي المرتجع', 'sortable': True, 'class': 'text-center', 'format': 'currency', 'width': '120px'},
        {'key': 'status', 'label': 'الحالة', 'sortable': True, 'class': 'text-center', 'template': 'components/cells/return_status.html', 'width': '100px'},
    ]
    
    # أزرار الإجراءات
    return_actions = [
        {'url': 'purchase:purchase_return_detail', 'icon': 'fa-eye', 'label': 'عرض', 'class': 'action-view'},
    ]
...
def post_payment(request, payment_id):
    ...
    try:
        with transaction.atomic():
            ...
            from financial.services.payment_integration_service import payment_integration_service
            ...
            if integration_result['success']:
                ...
                messages.success(request, f'تم ترحيل الدفعة بنجاح - القيد: {integration_result.get("journal_entry_number")}')
            else:
                messages.error(request, f'فشل الترحيل: {integration_result.get("message")}')
...
def edit_payment(request, payment_id):
    ...
    if request.method == 'POST':
        form = PurchasePaymentEditForm(request.POST, instance=payment, purchase=payment.purchase)
        if form.is_valid():
            try:
