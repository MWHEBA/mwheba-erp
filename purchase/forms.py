...
class PurchaseForm(forms.ModelForm):
    ...
    financial_account = forms.ModelChoiceField(
        ...
        widget=forms.Select(attrs={
            'class': 'form-control',
            'id': 'id_financial_account'
        })
    )
    
    class Meta:
        model = Purchase
        fields = ['supplier', 'warehouse', 'purchase_order', 'date', 'number', 'discount', 'payment_method', 'financial_account', 'notes']
        widgets = {
            'date': forms.DateInput(attrs={'type': 'date'}),
            'notes': forms.Textarea(attrs={'rows': 2}),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            self.fields['financial_account'].queryset = self.fields['financial_account'].queryset.none()
...
class PurchasePaymentForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ['financial_account', 'amount', 'payment_date', 'payment_method', 'reference_number', 'notes']
        widgets = {
            'payment_date': forms.DateInput(attrs={'type': 'date', 'class': 'form-control'}),
            'payment_method': forms.Select(attrs={'class': 'form-control'}),
            'amount': forms.NumberInput(attrs={'class': 'form-control', 'step': '0.01'}),
            'reference_number': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'رقم المرجع (اختياري)'}),
            'notes': forms.Textarea(attrs={'rows': 2, 'class': 'form-control', 'placeholder': 'ملاحظات (اختياري)'}),
        }
    ...
    def clean_financial_account(self):
        ...
        if financial_account:
            ...
            if not (financial_account.is_cash_account or financial_account.is_bank_account):
    ...
    def clean(self):
        ...
        if payment_method and financial_account:
            ...
            elif payment_method in ['bank_transfer', 'check'] and not financial_account.is_bank_account:
                raise ValidationError({
