        ...
        try:
            ...
            if purchase_price > product.cost_price:
                ...
                if product.cost_price > 0:
                    profit_margin = (product.selling_price - product.cost_price) / product.cost_price
                else:
...
def create_financial_transaction_for_purchase(sender, instance, created, **kwargs):
    ...
    if created and instance.status == 'confirmed':
        try:
            ...
            if journal_entry:
                logger.info(f"✅ تم إنشاء قيد محاسبي للمشتريات: {journal_entry.number} - فاتورة {instance.number}")
            else:
                logger.warning(f"⚠️ فشل في إنشاء قيد محاسبي للمشتريات - فاتورة {instance.number}")
                
        except Exception as e:
            import logging
            logger = logging.getLogger(__name__)
            logger.error(f"❌ خطأ في إنشاء قيد محاسبي للمشتريات: {str(e)} - فاتورة {instance.number}")
