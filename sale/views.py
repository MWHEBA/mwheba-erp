...
from .forms import SaleForm, SaleItemForm, SalePaymentForm, SalePaymentEditForm, SaleReturnForm
...
def sale_list(request):
    ...
    sale_headers = [
        ...
        {'key': 'payment_method', 'label': _('طريقة الدفع'), 'sortable': True, 'class': 'text-center', 'template': 'components/cells/sale_payment_method.html'},
    ...
    sale_actions = [
        ...
        {'url': 'sale:sale_edit', 'icon': 'fa-edit', 'label': _('تعديل'), 'class': 'action-edit', 'condition': 'not_fully_paid'},
        {'url': 'sale:sale_delete', 'icon': 'fa-trash', 'label': _('حذف'), 'class': 'action-delete', 'condition': 'not_fully_paid'},
        {'url': 'sale:sale_add_payment', 'icon': 'fa-money-bill', 'label': _('إضافة دفعة'), 'class': 'action-paid', 'condition': 'not_fully_paid'}
    ]
                    ...
                    for item in sale.items.all():
                        ...
                        if not StockMovement.objects.filter(
                            ...
                            if stock.quantity < item.quantity:
                                raise ValueError(f'الكمية المطلوبة غير متوفرة في المخزون للمنتج {item.product.name}. الكمية المتوفرة: {stock.quantity}')
                        ...
                        if financial_account_id:
                            try:
                                ...
                                if journal_entry:
                                    ...
                                    logger.info(f"✅ تم إنشاء دفعة تلقائية وقيد محاسبي للفاتورة النقدية: {sale.number}")
...
def sale_detail(request, pk):
    ...
    context = {
        ...
        'breadcrumb_items': [
            ...
            {'title': 'المبيعات', 'url': reverse('sale:sale_list'), 'icon': 'fas fa-shopping-cart'},
            {'title': sale.customer.name, 'url': reverse('client:customer_detail', args=[sale.customer.pk]), 'icon': 'fas fa-user-tie'},
...
def sale_return_list(request):
    ...
    """
    returns = SaleReturn.objects.all().select_related('sale', 'sale__customer').order_by('-date', '-id')
    
    # تعريف أعمدة جدول مرتجعات المبيعات
    return_headers = [
        {'key': 'id', 'label': '#', 'sortable': True, 'class': 'text-center', 'width': '50px'},
        {'key': 'sale.invoice_number', 'label': 'رقم الفاتورة', 'sortable': True, 'class': 'text-center', 'width': '120px'},
        {'key': 'sale.customer.name', 'label': 'العميل', 'sortable': True, 'class': 'text-start'},
    ...
    # أزرار الإجراءات
    return_actions = [
        {'url': 'sale:sale_return_detail', 'icon': 'fa-eye', 'label': 'عرض', 'class': 'action-view'},
    ]
