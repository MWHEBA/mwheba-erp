...
def create_financial_transaction_for_sale(sender, instance, created, **kwargs):
    ...
    if created and instance.status == 'confirmed':
        try:
            ...
            if journal_entry:
                logger.info(f"✅ تم إنشاء قيد محاسبي للمبيعات: {journal_entry.number} - فاتورة {instance.number}")
            else:
                logger.warning(f"⚠️ فشل في إنشاء قيد محاسبي للمبيعات - فاتورة {instance.number}")
                
        except Exception as e:
            import logging
            logger = logging.getLogger(__name__)
            logger.error(f"❌ خطأ في إنشاء قيد محاسبي للمبيعات: {str(e)} - فاتورة {instance.number}")
