...
class SalePayment(MonetaryTransactionMixin, PaymentAuditMixin, models.Model):
    """
    نموذج مدفوعات الفواتير
    """
    ...
    payment_exchange_rate = models.DecimalField(
        ...
        help_text=_("سعر صرف عملة السداد مقابل العملة الأساسية وقت التحصيل"),
    )
    ...
    amount_functional = models.DecimalField(
        ...
        decimal_places=2,
        default=Decimal("0.00"),
        help_text=_("القيمة المعادلة بالجنيه المصري المودعة بالخزينة"),
    )
    ...
    def can_be_synced(self):
        ...
        return (
            ...
            and self.financial_status in ["pending", "failed"]
            and self.amount > 0
        )
    ...
    def unpost(self, user=None, reason=""):
        ...
        try:
            ...
            old_data = {
                ...
                "financial_transaction_id": self.financial_transaction.id
            ...
            self.log_payment_action(
                action="unpost",
                user=user,
                description=f"إلغاء ترحيل دفعة مبيعات - القيد المحذوف: {journal_entry_number}",
                reason=reason,
                old_values=old_data,
                new_values=new_data,
            ...
            if user:
                self.log_payment_action(
                    action="unpost",
                    user=user,
                    description=f"فشل في إلغاء ترحيل دفعة مبيعات: {str(e)}",
                    reason=reason,
                    status="failed",
                    error_message=str(e),
                )
    ...
    def update_payment_data(self, data, user=None):
        ...
        self.log_payment_action(
            action="update",
            user=user,
            description="تحديث بيانات دفعة مبيعات",
        ...
        if was_posted:
            ...
            result = PaymentIntegrationService.process_payment(self, "sale", user)
            if result["success"]:
                self.status = "posted"
                ...
                self.log_payment_action(
                    action="post",
                    user=user,
                    description="إعادة ترحيل دفعة مبيعات بعد التعديل",
                    reason="إعادة ترحيل بعد التعديل",
                    journal_entry_id=self.financial_transaction.id
                    if self.financial_transaction
                    else None,
                )
    ...
    def delete(self, *args, **kwargs):
        ...
        try:
            customer_payment = CustomerPayment.objects.get(
                reference_number=f"SALE-{self.sale.number}-PAY-{self.id}"
            )
